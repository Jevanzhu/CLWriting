...
[![Test](https://img.shields.io/badge/tests-549%20all%20green-4FC08D?logo=vitest&logoColor=white)](#-项目状态)
[![Deps](https://img.shields.io/badge/runtime%20deps-0-e879f9)](#%EF%B8%8F-技术栈)
[![License](https://img.shields.io/badge/license-MIT-blue)](LICENSE)
[![Status](https://img.shields.io/badge/status-v1.0%20alpha%20%2F%20beta%20hardening-orange)](#-项目状态)
...
| **自动连写** | `auto` 将多章产出攒进 `待定稿/`，作者回来批量审稿、逐章定稿或整批回滚。 |
...
| **导出 / 迁移** | `export` 输出干净正文；`import` 从 v0.2 正文轻量迁移，长短篇自动分流。 |
...
| 数据流转 | `export` / `import` | 定稿导出 / v0.2 正文导入。 |
...
**v1.0 alpha（beta 收口中）**：Node + TypeScript 从零重写，与 v0.2 Python 版无代码继承关系。
...
- **69 个测试文件 / 549 个测试全绿**，`tsc --noEmit` 通过，构建通过。
...
- 当前 beta 留点：50 章规模验证进行中；`health --metrics` 已接入宿主漏记软提示与预算校准提示，auto 待定稿记账链路已回归覆盖，token 真值回填仍待实测校准。
