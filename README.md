...
[![Test](https://img.shields.io/badge/tests-1386%20all%20green-4FC08D?logo=vitest&logoColor=white)](#-项目状态)
...
npm test                   # 1386 单测
npm run test:e2e           # Playwright e2e（mock 驱动，24 specs）
...
| 测试 | vitest（1386 单测）+ Playwright（24 e2e） |
...
- **161 个测试文件 / 1386 单测全绿 + 24 个 e2e 全绿**，`tsc --noEmit` 通过，`build:all` 构建通过。
