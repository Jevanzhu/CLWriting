...
npm test                   # 1388 单测
npm run test:e2e           # Playwright e2e（mock 驱动，24 specs / 35 用例）
...
| 测试 | vitest（1388 单测）+ Playwright（24 specs / 35 用例） |
...
- **161 个测试文件 / 1388 单测全绿 + 24 个 e2e spec / 35 用例全绿**，`tsc --noEmit` 通过，`build:all` 构建通过。
