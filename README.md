...
**在 Claude Code 里写中文小说的创作助手**

从一句构思到逐章成稿，story-craft 全程陪你——
搭框架 · 写正文 · 自动审稿 · 记住前文 · 沉淀经验

适合 1-10 万字的短篇/中篇，也能撑起长篇连载。
覆盖悬疑 · 言情 · 修仙 · 科幻 · 现实 · 知乎短篇等主流品类。

![License](https://img.shields.io/badge/license-GPL--3.0-blue)
![Python](https://img.shields.io/badge/python-≥3.10-3776AB?logo=python&logoColor=white)
![Claude Code](https://img.shields.io/badge/Claude_Code-plugin-8A2BE2)
![Status](https://img.shields.io/badge/tests-282_passing-brightgreen)
...
| | |
|---|---|
| 🏗️ **从零起项目** | 说清书名、题材、主角和卖点，框架自动搭好 |
| 📝 **逐章写正文** | 你给方向它写草稿，还记得住前文人物、伏笔、设定，绝不前后矛盾 |
| 🔍 **写完自动审稿** | 每章过一遍审查，挑出逻辑漏洞、节奏问题、人物跑偏、AI 味——没修不放行 |
| 🧠 **越写越懂你** | 把你认可的写作经验沉淀下来，自动用到后续章节（见 `/story-learn`） |
...
<details>
<summary><b>Python 依赖（可选，单人使用可跳过）</b></summary>
...
> 🧭 不确定下一步用哪个命令？随时 `/story-preflight`，它会告诉你该走哪条路。
...
- **📦 章节真源 + 投影** — 一章验收通过后写入 `.story/commits/chapter_NNN.commit.json` 作为唯一真源，再自动派生出状态、记忆、摘要、检索索引、向量库、Markdown 视图等多份"投影"，供后续查询和写作调用。
- **♻️ 可恢复** — 每章中间产物都留痕，写到一半中断也能从缺失的那一步继续。
...
你创建的故事项目里，所有数据都在项目目录的 `.story/` 下（合同、章节真源、记忆、索引等），正文在 `正文/`、设定在 `设定/`、大纲在 `大纲/`——都是你能直接打开看的文件。
...
|:---|:---:|
| Skill | **17 个 Skill** |
| Claude Code 命令 | **13 个 Claude Code commands** |
| Agent | **9 个 Agent** |
| CLI 子命令 | **20 个 CLI 顶层子命令** |
| 投影 | 6 个（`state` `memory` `summary` `index` `vector` `markdown_view`） |
