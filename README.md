...
[![Test](https://img.shields.io/badge/tests-552%20all%20green-4FC08D?logo=vitest&logoColor=white)](#-项目状态)
...
- **69 个测试文件 / 552 个测试全绿**，`tsc --noEmit` 通过，构建通过。
...
- 当前 beta 留点：50 章规模验证进行中；`health --metrics` 已接入宿主漏记软提示与预算校准提示，auto 待定稿记账链路已回归覆盖，`record-call --set-tokens` 已支持 token 真值事后回填。
