...
| **长篇自动连写** | `auto` 将长篇多章产出攒进 `待定稿/`，作者回来批量审稿、逐章定稿或整批回滚；短篇集主链按篇完成，暂不承诺批量自动连写。 |
...
- **70 个测试文件 / 557 个测试全绿**，`tsc --noEmit` 通过，构建通过；RC 中文路径专项已在 Ubuntu / macOS / Windows CI matrix 通过。
...
- RC 能力边界：`auto` 当前按长篇批量连写设计；短篇集已完成按篇起草、机检、三审、定稿和导入，短篇批量自动连写留给 beta 后再评估。
