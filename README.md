...
>
> **双轨设计**——长篇（200 万字不崩、不吃书）+ 短篇集（单篇情绪爆破、一反转撑全篇）。

**一本书 / 一个短篇集 = 一个 git 仓库**。AI 负责写和审，脚本负责数和记，作者只做三件事：**确认细纲 · 审稿 · 拍创作决策**。
...
| | |
|---|---|
| 🎯 **双轨分流** | `kind: long` 长篇 / `kind: short` 短篇集——从仓库布局、状态机到机检、三审、定稿，全程分轨而非打补丁 |
...
| 📒 **账本防吃书** | 每章定稿前核对「声明 vs 正文」是否一致，账实不符（如揭凶手但正文不写）会被设定校对逮住、定稿拒绝 |
...
**长篇**：`enter` → 起草 → `confirm` 细纲 → 写正文 → `check` 机检 → `review` 三审 → 作者裁决 → `finalize` 定稿 → 下一章。`auto` 可连写一批攒进 `待定稿/` 批量审。

**短篇**：`init --kind short` 建集 → `enter` 落态 7 出「篇」→ 单篇走 **P1–P4**（定情绪 + 反转 → 五段大纲 + 清单 → 正文 → 三审定稿）。一篇一个独立闭环，整集共享文风库。
