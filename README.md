...
[![Test](https://img.shields.io/badge/tests-2248%20all%20green-4FC08D?logo=vitest&logoColor=white)](#-项目状态)
...
| 测试 | vitest（2248 单测）+ Playwright（25 specs / 38 用例） |
...
- **245 个测试文件 / 2248 单测全绿 + 25 个 e2e spec / 38 用例**，`tsc --noEmit` 与 `vue-tsc` 双端通过，`build:all` 构建通过。
