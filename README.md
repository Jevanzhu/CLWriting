...
[![Test](https://img.shields.io/badge/tests-7703%20all%20green-4FC08D?logo=vitest&logoColor=white)](#开发)
...
Node 24+，TypeScript strict。前端 Vue 3 + Pinia + Vite，编辑器 CodeMirror 6，桌面壳 Electron；存储是 node:sqlite（RAG 索引）加 JSON/YAML 配置；AI 侧三个协议适配器（Anthropic、OpenAI Chat、OpenAI Responses）统一走 runTask 编排，重试、超时、用量都归它管；测试 vitest（7703 单测）+ Playwright（33 specs / 54 用例）（常规命令跑 51，3 个发布 smoke 用例需 CLWRITING_E2E_RELEASE 环境变量）。
