...
- **经验学习闭环**：`/story-learn` 把三类可复用写作经验统一沉淀到 `.story/project_learning.json`——人工标注、从审查历史自动提炼（`query learning-suggestions` 聚合跨章反复问题与 blocker，经人工确认后入库）、以及参考作品拆解的可迁移技法（`learn --source import`）；入库经验按严重度排序、自动去重合并，并注入后续章节的写作 checklist，在 `/story-*-write` 中生效。
...
底层 CLI 仅有一个可选三方依赖 `filelock`，用于多进程并发写入时的状态文件保护。**单人单进程使用可跳过**（缺失时自动降级为仅原子替换，不影响功能）；若有并发写入同一项目的场景，请安装：

```bash
python3 -m pip install -r requirements.txt
```

安装与否可随时用 `/story-* ... health` 查看（`runtime.filelock_available` 字段）。
