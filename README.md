...
[![Test](https://img.shields.io/badge/tests-2433%20all%20green-4FC08D?logo=vitest&logoColor=white)](#-项目状态)
...
| 测试 | vitest（2433 单测）+ Playwright（26 specs / 39 用例） |
...
- **265 个测试文件 / 2433 单测全绿 + 26 个 e2e spec / 39 用例**，`tsc --noEmit` 与 `vue-tsc` 双端通过，`build:all` 构建通过。
