...
项目治理与协作规矩（文档操作链、测试分层、评审命名、入库判据等纪律条）正本 = 项目根 `CLAUDE.md`——单机私有（`.gitignore` 排除、不入库），`AGENTS.md` 指向它，单机生效；入库正本已随 2026-09-19 治理落点批取消（沿革与复原锚 = git `bd49238f`）；同日 Archive README（归档规则/批记）亦随作者拍板撤除，历批明细改由 git 历史兜底。
