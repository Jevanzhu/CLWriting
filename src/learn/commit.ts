/**
 * learn 候选入库 —— 作者审核挑选后的候选写入文风条目库（文风系统重整）。
 *
...
/** 让出粒度——每满 100 条让一次（400 上限即最多 3-4 次让出，单次
 *  同步段回落到百 ms 量级内）。 */
const COMMIT_YIELD_EVERY = 100

/**
 * 内容指纹去重——「场景+正文」sha256。双击提交/重放同一批
...
async function commitDeduped<T>(
  ...
  for (let i = 0; i < picks.length; i++) {
    const pick = picks[i]!
    // 每满 100 条让出一次——时机在恰处理完第 100/200/... 条之后、
    ...
    if (hit !== undefined && hit !== '') {
      ...
      // 指纹只含 kind+场景+正文——同批同场景同正文但技法指令不同
...
/**
 * 样章候选入库（作者挑选后调用）。
 * 转 async（周期让出，见文件头注）——持久化语义与返回值形状不变。
 * @returns 入库的文件路径列表（相对书仓库；起重复内容返回既有条目路径）
 */
export async function commitSamples(
  ...
  return commitDeduped(
    ...
    // 被吞条目的技法指令摘要（空则回退出处，去重 warn 可定位）
    (p) => p.技法指令 || p.出处,
    yieldFn,
  )
}
