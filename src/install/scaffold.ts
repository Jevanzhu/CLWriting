...
export function renderBookAgentsMd(opts: BookScaffoldOpts): string {
  if (opts.kind === 'short') {
    return [
      ...
      '',
      '- **P1 定情绪+反转**：作者拍板目标情绪 + 一句话梗概 + 核心反转 → 写入工作区细纲；确认细纲绑哈希',
      '- **P2 排五段大纲**：开头钩子(约300字) / 铺垫(30-40%) / 升级(20-30%) / 反转(10-15%) / 余韵(5-10%)；反转线索表(≥3铺垫)写入 `清单.md`；确认细纲',
      '- **P3 写正文**：按段写，文风样章贴笔感；机检（身体部位词≤5 / 「像」≤10 / 节数守恒 / 开头零环境）',
      '- **P4 三审定稿**：三审满审三视角（钩子审/情绪反转审/设定收尾审）→ 通过 → 按篇原子提交（pc: 前缀）',
      '',
