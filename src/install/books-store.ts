...
// 锁等待异步孪生（acquireCrossProcessLockAsync：setTimeout 轮询，
...
// （拆分波1）：字面量单源——books-resolve.ts findWorkDir 上溯判存 / books.ts
// writeActive 等消费点共用，不随拆分复制
export const CLWRITING_DIR = '.clwriting'
// （拆分波1）：books-repair.ts scanBookCandidates 扫长篇/短篇二级分组用；
...
export function readBooksStrict(workDir: string): BookEntry[] | null {
  // 缓存键含绝对路径（不同形态的 workDir 字符串指向同一文件时同键复用）
  const fp = resolve(workDir, BOOKS_FILE)
  // 原 existsSync 判存合并进指纹 stat——一次调用同时承担「缺文件 → 空表」
  ...
  // 剥 BOM 前缀——win 记事本「UTF-8 with BOM」保存后首行变
  ...
  // 仅成功解析入缓存（缺文件/读失败不缓存失败值）；FIFO 淘汰最旧键
...
export function tryBooksLock(workDir: string): (() => void) | null {
  // 获取段整段收编——首行 mkdirSync（.clwriting 建不出：EACCES/
...
export async function tryBooksLockAsync(workDir: string): Promise<(() => void) | null> {
  // 同 tryBooksLock 的获取段收编（异步孪生同步堵）——mkdirSync
