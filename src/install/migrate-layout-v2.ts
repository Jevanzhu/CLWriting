...
function migratePath(oldPath: string): string {
  ...
  if (oldPath.startsWith('大纲/伏笔/')) return '设定/伏笔/' + oldPath.slice('大纲/伏笔/'.length) // 伏笔迁移路径同步
  // 细纲.md 不再随迁（运行时永久写在 工作区/，见 moveDrafts）——
...
function migrateManifestPaths(bookRoot: string, errors: string[], moved: string[]): number {
  ...
  try {
    return withManifestLock(manifestPath, () => {
      const m = readManifestStrict(manifestPath) // RMW strict 读（读失败上抛走外层 errors 收口）
      let count = 0
      for (const entry of m.entries.values()) {
        const newPath = migratePath(entry.path)
        if (newPath === entry.path) continue
        // 伏笔前缀豁免——伏笔物理搬迁由 migrateLegacyForeshadows
        // （document/ 层）负责、不经本文件 moveTree，且搬迁前物理文件就在旧路径上
        // （若按下方「旧路径有残留文件则保持」判定会被误留旧值，断链序：
        // 先改写清单 → 伏笔按改写后路径搬/接定稿基线），维持无条件改写。
...
/** relPath 是否落在任一已成功搬移的子树内（精确命中或为其子路径）。 */
...
function moveTree(
  ...
  newRel: string,
  errors: string[],
  moved: string[], // 实际搬移成功子树根登记（清单改写过滤用）
): number {
  ...
  if (!existsSync(newPath)) {
    ...
    try {
      // 收编退避——win 瞬时锁不再让启动迁移报错横幅（幂等可重试）
      renameWithRetry(oldPath, newPath)
      // 整搬成功 → 旧目录整棵子树都已在新路径，按子树根登记（内部文件
  ...
  try {
    for (const name of readdirSync(oldPath)) {
      ...
      if (existsSync(dst)) {
        // ②：statSync 单独 try/catch——原先裸 stat 抛错直穿外层
      ...
      try {
        renameWithRetry(src, dst) // 退避收编
        // 单搬成功（文件或子目录整体 rename）→ 按旧相对路径登记子树根
...
function moveDrafts(bookRoot: string, errors: string[], moved: string[]): number {
  ...
  try {
    for (const name of readdirSync(workdir)) {
      // 仅草稿文件；其余运行时资产不动
      // 不再认领 细纲.md——运行时 outline 端点把章纲永久覆盖写在
