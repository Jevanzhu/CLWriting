...
export function migrateLayoutV3(bookRoot: string): { migrated: number; errors: string[] } {
  ...
  // 旧稿入回收站的旧 path 集合——主清单同路径旧条目待清
  const trashPaths = new Set<string>()

  // 裸 readdirSync 异常（草稿路径被普通文件占用 ENOTDIR、EACCES
  ...
  for (const name of draftNames) {
    ...
    if (!m) {
      // 非隐藏的非 .md 认识文件不再静默跳过——v3 布局已退役草稿
  ...
  // 更新文档清单（path 变更 → 重建 entries Map，key 不变只改 entry.path；
  ...
  if (pathRemap.size > 0 || trashPaths.size > 0) {
    try {
      ...
      withManifestLock(manifestPath, () => {
        const manifest = readManifestStrict(manifestPath) // RMW strict 读（读失败上抛走外层 errors 收口）
