 ...
 *
 * R0916-7-P3-3（2026-09-16 评审修复批）：登记读写 + 锁（BOOKS_FILE/CLWRITING_DIR/
...
// R0916-7-P3-3：存储层（常量 / 解析缓存 / 读写 / 跨进程登记锁）下沉中立模块——
...
// ── R46-11 指纹缓存的读 path / 写 path / books.lock 互斥 ────────────────────
// R0916-7-P3-3：解析缓存（R46-11）、读写（readBooksStrict / readBooks / writeBooks）、
// 登记锁（BOOKS_LOCK_TIMEOUT_MS / tryBooksLock / tryBooksLockAsync，R63-2/R34D-19/
// R44-18）与常量（BOOKS_FILE / CLWRITING_DIR / KIND_DIRS）整体下沉 books-store.ts
// （中立模块）——同域拆出的 books-repair / books-resolve 与本文件由此单向向下依赖。
// 实现与沿革注释随代码走（含 DA-3 拒写 / R0912-3 stat 分诊 / R40-25 BOM / R0912 浅拷贝
// 纪律），本文件不再持副本。
...
// R0916-7-P3-3：两模块已不再回引本文件（改引 books-store），桥自此单向。
...
// R0916-7-P3-3：同上，桥自此单向（repair 的存储原语改引 books-store）。
