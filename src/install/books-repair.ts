/**
 * books.jsonl 自愈（#32 第 6 节，文件即真相 + 不报错拒绝）—— 依据 #32。
 *
 * （⑤④产品拆分波1）：自 install/books.ts 缝 A 纯移动拆出——
...
export interface RepairResult {
   ...
   *  不带该键。 */
  missingHint?: string
  /** 显式清除（opts.purgeConfirmedMissing=true）时移除的幽灵登记条目——
 ...
 * missing 只报告不清除（幽灵登记处理提示见 missingHint）；要清除
...
/** 目录「确认不存在」（stat ENOENT）才可显式清除——EACCES/EIO 等瞬态不可读
...
function repairBooksLocked(workDir: string, purgeConfirmedMissing: boolean): RepairResult {
  ...
  for (const relPath of entries) {
    ...
    if (existingPathIndex >= 0) {
      ...
      // path 命中改名分支补重名检查——本分支以 book.yaml title
      // 直接覆写登记名，撞上 rebuilt 中另一条目同名时会落成同名双登记（resolveBook
      // 首匹配遮蔽其一、removeBookEntry 按名过滤连删两条）；同款跳过留痕：
  ...
  // 显式清除幽灵登记（默认关，见 repairBooks 头注）——仅清 ENOENT
  ...
  // missing 不再计入 changed——幽灵条目自愈不自动清除，
  ...
  // 幽灵条目的可操作提示（missing 非空才带）——自愈不自动清除（避免把「暂时
