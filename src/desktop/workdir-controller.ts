 ...
 *  （或本应用他路径）删除的书库目录会残留展示至重启，注释只声明了「外部手改
...
function readStore(): WorkDirStore {
  ...
  const fp = storePath()
  // （六十一轮评审）：文件级读失败容错——此前失败面不对称：parseStore 对内容
 ...
 *  （杀毒/同步盘瞬时锁，同族病因）恢复后的切库不再丢库指针与 recent；
 ...
 * 「重新选择」原为无上限递归——反复选非书库目录会无限弹窗；
 * 改循环 + 次数封顶（10 次），超限记 error 日志后返回 null 退出（由调用方按取消处理）。
 * @returns 校验通过的目录绝对路径；取消/超限返回 null
 */
const PICK_LIBRARY_MAX_ATTEMPTS = 10 // 目录选择循环封顶

/**
 * 平台规范化批 E：大小写敏感卷警告——探测目录所在卷敏感性，敏感
...
async function pickLibrary(): Promise<string | null> {
  ...
  for (let attempt = 1; attempt <= PICK_LIBRARY_MAX_ATTEMPTS; attempt++) {
    ...
    if (choice.response === 1) continue // 重新选择（回到循环顶，受封顶约束）
    return null // 取消
  }
  // 封顶退出——留痕报错后按取消收口，不无限弹窗困住用户
...
/**
 * （GLM-5.3 修复批）：bootstrap 的 recent 失效过滤
