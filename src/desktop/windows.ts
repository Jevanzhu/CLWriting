...
/**
 * F2d（复审-0914-优化修复批）：单例子窗打开骨架——openShelfWindow/openLibraryWindow
...
interface SingletonWindowSpec {
  ...
  /** Y-12/R44-16：appUrl 未就绪命中守卫的 info 留痕文案（书架/书库措辞各一） */
  appUrlGuardLog: string
  /** R0910-W：closed 清理体隔离标签（「书架窗口引用置空」/「书库窗口引用置空」） */
  cleanupLabel: string
  title: string
  /** 尺寸/下限计算（wa = 主屏工作区；R44-15 钳制口径在调用方闭包内原样保留） */
