/**
 * 退出前清理——中断在途编排 + 关 HTTP server。
 *
 * 只用现有 API：abortSelfHeal / abortChat（per book，self-heal 与 chat 编排的
 * 中断入口，与删书端点同款接线）；server.close。：DocumentService 保存
...
// 保存队列 drain + 在途外部工作（Worker 线程）有界等待
...
interface ShutdownOptions {
  ...
  /** #7/：被中断编排的收尾等待上限（session/end 落库）；缺省 1.5s（与 close 同量级） */
  settleTimeoutMs?: number
}

/** 轮询等某书全部 task-gate 释放（100ms 间隔；上限即 settle 预算，由外层 race 兜底）。 */
...
export async function shutdownStudio(
  ...
  const names: string[] = []
  // 书根清单——退出前 drain 每本书的 DocumentService 保存队列（在途 save
  ...
  if (workDir) {
    for (const b of readBooks(workDir)) {
      ...
      abortChat(b.name)
      // spawn 在途的中止——spawn ctrl 按 owner='spawn' 注册进 driver
  ...
  // #7/：等被中断的编排收尾（session/end 事件落库）——此前 abort 后不等
  ...
  // settle/close 兜底超时定时器 .unref——不 unref 时它们是进程
    ...
    names.map((n) =>
      Promise.race([
        Promise.all([
          ...
          // task-gate 长任务（review/outline/analyze/onboard/lead-updates）
          // 无 abort 通道（为 spawn 修的同型缺口）——给有界 settle 窗口等闸释放
  ...
  // 保存队列 drain + 在途 Worker（重建/导出/扫描）收尾。此前本层只等编排
  ...
  if (server) {
    await new Promise<void>((resolveP) => {
      ...
      const fin = (): void => {
        ...
        if (timer) clearTimeout(timer) // settle（close 回调）即清，不留挂满时长的空转定时器
        resolveP()
      }
      server.close(() => fin())
      // close 只停接新请求并等在途响应，keep-alive/SSE 空闲连接
      // 此前靠下方定时器硬等拖满排水尾——主动摘除空闲连接（Node ≥18.2）。
      server.closeIdleConnections?.()
      timer = setTimeout(fin, opts.closeTimeoutMs ?? 1_500)
      timer.unref() // 同上，不阻塞无其他句柄时的正常退出
    })
  }
