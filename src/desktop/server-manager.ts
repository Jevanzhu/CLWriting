...
/** R0916-7-P3-2（2026-09-25 评审 P3-2）：管理器只读配置——deps 缺省注入后的组装结果
...
/** R0916-7-P3-2：管理器运行上下文——只读配置 + 唯一可变状态源。各单元显式传参
...
/** R0916-7-P3-2：组装依赖（拆分单元之一）——deps 缺省注入单点，缺省真件只在此出现。 */
...
/**
 * R0916-7-P3-17：管理器状态容器（唯一可变真相源）——旧 12 个闭包变量收敛于此。旧注释里
...
/**
 * 子进程 argv 组装（launch 纯前置，独立可测）：E-9b（第五十三轮）token 不经 argv
...
async function launchPinned(
  ...
  try {
    ...
    transition(ctx, { ev: 'round-close' }) // R44-12：轮收口（含在途 fork 句柄）同点归还
  }
}
...
async function doRestart(ctx: ManagerCtx): Promise<void> {
  const { cfg, state } = ctx
  if (killMarked(ctx)) return // 等待窗口内被停机（S-5）
...
async function stopActiveChild(ctx: ManagerCtx): Promise<void> {
  ...
  cancelPendingRestart(ctx)
  if (!current) return
  transition(ctx, { ev: 'stop-mark' }) // 主动 kill：随后的 exit 是预期收口，不触发重启（S-5）
  current.proc.kill()
  // SIGTERM 被吞的兜底：R26-87 起超时升级 SIGKILL 强杀（退出事件迟到时当值位已由
  // exit 监听清空，语义不变）；无 pid / 升级失败回落「超时放行」
  // R27-90（二十七轮）：主体在模块级 killProcAwaitEscalating——握手超时路径（模块级
...
function killNow(ctx: ManagerCtx): void {
  cancelPendingRestart(ctx)
  transition(ctx, { ev: 'stop-mark' }) // S-5：被杀 child 的 exit 不触发自动重启
...
async function stopChildImpl(ctx: ManagerCtx): Promise<void> {
  ...
  // R0916-7-P3-2：同构块收拢为 killStartingProc（行为零变化）。
...
/** 启动（X-3 在途轮唯一入口 + E-9a 并发复用 + B-7 停机门 fail-closed） */
...
/** 当值 child 读数（P3：main 侧「关旧」判据之一） */
function isRunning(ctx: ManagerCtx): boolean {
  return ctx.state.child !== null
}

/** 挂起自动重启读数（P3：child 已崩但重启在途时 isRunning() 为 false，仅凭它会漏关） */
...
/**
 * R0916-7-P3-2（2026-09-25 评审 P3-2）：管理器工厂——只剩组装与句柄映射；单元见
