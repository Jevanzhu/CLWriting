...
/**
 * Rosetta 翻译态判定（v1.0.0-rc.0 发布）：x64 进程跑在 arm64 机型上。
 *
...
export function loadOrGenerateOsKek(userDataPath: string, deps: OsKekDeps = {}): Buffer | null {
  try {
    ...
    // （1.0 前质量）：本条原为 warn 且带「恢复 = os-kek.ts
    ...
    // v1.0.0-rc.0 发布：翻译态整面提前回落——safeStorage 任一调用
    ...
    if (!safeStorage.isEncryptionAvailable()) {
      // linux 无钥匙串等环境常态也留痕——「v2 vault 为何回落内置通道」可诊断
      log.warn(
        'desktop',
    ...
    if (existsSync(fp)) {
      ...
      // 损坏分诊——providers.json 持有（或疑似持有）v2 凭据 → 绝不重建
...
/** 读 + 解锁 sealed hex——失败不再静默，病因随返回体交调用方统一 warn。 */
...
/**
 * providers.json 是否可能持 v2 vault（os 通道凭据）——判定口径对齐
