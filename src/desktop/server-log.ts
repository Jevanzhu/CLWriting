 ...
 * server-manager.ts（state 拆分同款纪律）。服务器状态机
...
/** child 行 err 字段 {name,message,stack?} → Error 重建（透传；缺字段按无 err 处理） */
