...
// IPC 响应回程窗口——handle 返回值需先送达渲染进程再 relaunch
...
/** context-menu 取消补发延迟——macOS NSMenu 先关菜单再派发
...
export function registerIpc(): void {
  ...
  handleTrusted('desktop:open-library', async () => {
    ...
    // 落库失败转契约化失败，不再裸抛绕过 {ok,reason} 信封
  ...
  // -2：同 show-in-folder——readBooks 同步扫书库前的失联卷预探
  ...
  // TTL 缓存降半档——系统字体枚举是跨平台系统命令（mac 自带
  ...
  // ② kill 接线（台账待拍板项随作者「全部修复」指令落地，原「打包态路径不可解」
  ...
  handleTrusted('desktop:open-shelf', () => {
    // openShelfWindow 是 async（内部 await devProxyApplied）——此前
    // fire-and-forget 裸调，窗工厂早期抛错成主进程 unhandledRejection 丢诊断。对齐
    // 的 loadURL 口径：promise 接日志留痕（handler 同步返回，invoke 端不悬等待、
    // 错误不外抛到渲染层，窗口崩溃另有自愈兜底）
  ...
  handleTrusted('desktop:open-library-window', () => {
    // 同 open-shelf——async 工厂 promise 接日志，防 unhandledRejection
    openLibraryWindow().catch((e) => {
      log.error('desktop', '书库管理窗口打开失败', e)
    })
  })
  // 在系统文件管理器中打开当前书库根目录
  // -2：同 show-in-folder——realpathSync 同步解析前的失联卷预探
  ...
  onTrusted('desktop:context-menu', (event, specs: unknown) => {
    // 补 isDestroyed——fromWebContents 命中与 menu.popup 之间存在
    // 微窗口，窗口关闭后 popup 同步抛「Object has been destroyed」（对齐 663-664 行
    // set-fullscreen 守卫）
    const win = BrowserWindow.fromWebContents(event.sender)
    if (!win || win.isDestroyed()) return
    // 载荷形状校验前置——非数组/无合法项整体忽略（弹不了菜单但不崩主进程）
    ...
    function sendOnce(key: string | null): void {
      if (sent) return
      sent = true
      // 菜单滞留期间窗口可被关闭（click 晚于菜单关闭、popup 回调的
    ...
    // 0ms 单拍竞窗实测可被 NSMenu 迟派发穿透）。渲染侧是
