...
export interface ExportResult {
  ...
  /** 清偿-导出未过滤提示（残留清偿批）：定稿过滤是否实际生效。
   *  定稿清单缺失（finalizedPathSet → null）时导出兜底不过滤（宁多勿漏，/PL-2
  ...
  const strip = (respectFence: boolean): { text: string; unclosed: boolean } => {
    ...
    const text = body
      ...
      .map((line) => {
        ...
        // 大识别范围，口径不变）：单源只共享「围栏行识别」，开/闭栏语义两侧各自
        ...
        // IR-5 `const a = 1  #% 松散字面`）维持 登记口径一律保留；紧贴分支
        ...
        // MP2-4（专项二轮修复批）：截断行保留原行尾——replace(/\s+$/) 会把 \r 一并
...
function archiveOldExport(exportDir: string, oldName: string, warnings: string[]): boolean {
  try {
    ...
    // MP2-3（专项二轮修复批）：归档 rename 收编 renameWithRetry——win 杀软/索引器
 ...
 *  拍板快断批（作者指令「按建议顺序开工」）：displayNum 语义拍板维持
 *  ——已发布章号不可变优先于显示序单调（发布号是读者侧锚点）；已发布章不居
 *  sortKey 序前时显示序非单调（[6,7,8,9,5,10] 形态，-源码 ⑰）系该语义的
 *  自然结论，接受不改。
...
export function exportBook(options: ExportOptions): ExportResult {
  ...
  // ── 阶段六·投稿视图（十八轮：short 分支整体收编进错误信封—— 只包了
  ...
  return {
    ...
    unit: '章',
    // 清偿-导出未过滤提示（残留清偿批）：成功面核心消费点——清单缺失
...
export function prepareExportLayout(args: {
  ...
  // 上抛，worker 形态变 500 且丢 chapterCount/warnings，违背 /确立的
...
/** 阶段五·写出：全本（单遍流式，内存闸审计 ——不再物化 purified 全书
