 ...
 *
 * 写入纪律（§三）：每批一个事务；先落库后返回；启动修复补孤儿 session 的
...
export interface SessionStore {
  ...
  // appendEvent 单事件便捷封装随清理批删除（原注
  // 即预告）——生产链全走批接口，测试消费方已随本批迁 appendEvents(sid,[ev])[0]!。
  /** 可选 limit 限量通道（seq 升序取前 N）——现有调用方均为全量投影
  ...
  /** 每书一个 workspace 会话（ws- 前缀）承载非对话链路事件（step/llm/retry/check）；惰性创建复用 */
  workspaceSession(book: string): string
  /** 最新对话会话查询——生产零调用（对话恢复经内存 histories/restore
  ...
  lastSeq(): number
  /** SessionRecorder.close 写 compaction 前遮蔽区间自检的数据源
...
/** 开口标记续期周期——活句柄定期 utimes 刷标记 mtime，让「标记年龄」
...
export function repairOrphanSessions(db: DatabaseSync, skip: ReadonlySet<string>): void {
  ...
  // 事务内复核语句——外层 SELECT 与本事务之间，他进程可能已给同一
  ...
  for (;;) {
    ...
    for (const o of orphans) {
      ...
      if (o.starts > o.ends && !skip.has(o.session_id)) {
        ...
        attempted++
        // INSERT（补 end）与 UPDATE（touch updated_at）两步同事务——
        ...
        try {
          ...
          if (fresh && (fresh.starts ?? 0) > (fresh.ends ?? 0)) {
            ...
            touch.run(o.last_at, o.session_id) // 真实 last_at（上方头注）
          }
          db.exec('COMMIT')
        } catch (err) {
          // 同款加固——裸 ROLLBACK 在事务已自动回亡时抛
...
export function openSessionStore(userDataPath: string | null | undefined, bookRoot: string): SessionStore | null {
  ...
  // 首开段（建库 + DDL + 孤儿修复）进 session 跨进程锁——
...
/** store 方法族的公共上下文（连接句柄 + 库路径 + 引用计数条目）。 */
interface StoreCtx {
  db: DatabaseSync
  dbPath: string
  entry: StoreEntry
...
/** 语句准备：B2listEvents/iterateEvents 的 SQL 装配单源——
...
function* queryEventRows(
  ...
  if (type !== undefined) args.push(type)
  if (cap !== undefined) args.push(cap)
  // 同上——iterateEvents 新编译、listEvents 走缓存（SQL 单源本处一份）
...
function createWriteMethods(
  ...
  return {
    createSession(book: string, header?: Record<string, unknown>): string {
      ...
      const sid = ulid()
      const now = Date.now()
      // 固定 SQL 走连接级 prepared 缓存（每会话一条，编译一次复用）
      prepared(
        db,
    ...
    appendEventsResolveLineage(sessionId: string, evs: NewEvent[]): number[] {
      ...
      return withEventsTx(db, () => {
        ...
        evs.forEach((e, idx) => {
          // 拒收 sourceSeqs——本方法只认批内索引 sourceIdxs；
...
function createReadMethods(
  ...
  return {
    listEvents(book: string, sessionId?: string, limit?: number, type?: EventType): ChatEvent[] {
      // limit 可选限量（seq 升序前 N）；投影折叠调用方不传（全量语义不变）
      // 内存闸（审计）双降：①type 可选 SQL 下推——trace/cost 聚合只取
      ...
      // SQL 装配/坏行降级单源 queryEventRows，本方法只承担
      // cap 解析 + 物化数组（坏行降级见 safeRowToEvent）。
    ...
    listEventsTail(book: string, tail: number): ChatEvent[] {
      ...
      if (cap === 0) return []
      // 降序取尾 + 反转升序；同款：表达式内同步排干生成器，走 prepared 缓存
      const rows = prepared(
        db,
        `SELECT * FROM events
    ...
    *iterateEvents(book: string, sessionId?: string, type?: EventType): IterableIterator<ChatEvent> {
      ...
      // 聚合结果与全量读逐字段一致）；坏行降级沿用 safeRowToEvent。调用方
...
function createSessionQueryMethods(
  ...
  return {
    workspaceSession(book: string): string {
      // SELECT→INSERT 包 BEGIN IMMEDIATE——双进程并行首开同书时，原裸
      ...
      try {
        ...
        // 同款加固（见 cache/rebuild.ts）——SQLite 部分
    ...
    latestSession(book: string): SessionRow | null {
      // 排除 workspace 会话（ws- 前缀）——链路事件不干扰对话恢复选会话
      // ORDER BY 加 rowid tiebreaker——同一毫秒创建/更新的多个会话选择结果稳定
...
function createMaintenanceMethods(ctx: StoreCtx): Pick<SessionStore, 'clearBook' | 'clearBooks' | 'close'> {
  ...
  return {
    ...
    clearBooks(books: string[]): void {
      // 低级项：多 book 键单事务清理——audit DELETE / chat 清史都是
...
export async function migrateBookSession(
  ...
  // 迁移整段（在途断言→checkpoint→搬移→改钥匙）进 session 跨进程
  ...
  // 新旧路径两把 per-book 锁（bookHash 排序获取，见 acquireMigrateLockPair）——
  // 首开旧库对 lock(old)、首开新库对 lock(new)，rename 窗口两侧都不再漏。
  // 锁获取异常（EACCES/只读卷等非 EEXIST 故障会 throw）收口为
  // false——函数契约「false = 迁移失败可重试」，此前裸异常穿到 books.ts 改名端点。
  // 函数转 async（books.ts 改名端点/测试两处调用方随迁）——
  ...
  // 钥匙改写抽闭包——迁移第 4 步与半迁移态自愈共用。幂等：两条
  // UPDATE 的 WHERE 只匹配仍持旧钥匙的行（对话书名 / 工作区 hash），已改行不再命中，
  // 重复补跑零副作用。kk-busy_timeout 先于 BEGIN——同进程连接已全关，但另一
  ...
  try {
    if (!existsSync(oldDb)) {
      // 半迁移态自愈——「rename 成功（3）→ 钥匙 UPDATE 未及 COMMIT
    ...
    try {
      ...
      // 墓碑改原子写——裸 writeFileSync 写中途进程死会留半截 JSON，
    ...
    for (const suffix of ['', '-wal', '-shm'] as const) {
      ...
      if (existsSync(from)) {
        // 收编 renameWithRetry——win 杀软/索引器瞬时锁（EPERM/EBUSY）
        // 下裸 renameSync 直接失败会触发回滚链；同一瞬时锁未释放时回滚 rename 同样失败，
        // 叠加下方「回滚失败仍撤墓碑」即拆掉防线（旧位 .db 与 .migrated 双缺 →
    ...
    // 回滚同样收编 renameWithRetry，且**回滚存在失败项时保留墓碑**——
    // 原实现回滚失败仅记日志、随后无条件 rmSync 撤碑：瞬时锁同时打断搬移与回滚时，
    // 旧位 .db 与 .migrated 双缺，迟来首开按「正常缺库」重建空库（要防的事件流
