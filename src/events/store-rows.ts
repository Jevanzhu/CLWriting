 ...
 * safeRowToEvent（R65-20 坏行降级）原样随迁（零行为变化，历史注释原样随代码迁移）。
 * 实读偏差记档：侦察口径的 rows 缝（约 85 行）含 R46-42 prepared 语句缓存族——该族
 * closeEventsDb 被 test/events/r0911-g-p3-4-close-cache.test.ts 结构契约钉在 store.ts
