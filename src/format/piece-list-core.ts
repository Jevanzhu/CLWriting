 ...
 * 头注纠偏：本解析器**不**实现「未知段进 _raw」——只提取上述
 ...
 * 备案：该「上层文本级补丁路径」生产尚未接线（writePieceList
...
/** 解析面留痕——本模块零 Node 依赖（浏览器端 MetaFormPanel 共用），
...
function parseEmotionSection(lines: string[], startIdx: number): { curve: EmotionCurvePoint[]; endIdx: number } {
  ...
  while (i < lines.length) {
    const trimmed = lines[i]!.trim()
    // 同 parseReversalSection（同名重复段头同样终断，防后段条目静默覆盖）
    if (/^##\s/.test(trimmed)) break
