 ...
 * 扩展名剥除大小写不敏感——.MD 大写章文件名（mac 敏感卷/win 手工
 *  改名形态）此前标题派生残留 .MD 尾，parseChapterFileName 的 路径/标题 派生错位。
 * 与 filename.ts isMdFileName（单源）同口径；本文件须零 Node 依赖供 web-next
 ...
 * 守卫语义与 filename.ts chapterNoFromName 单源
...
export function parseChapterFileName(fileName: string): { 章号: number; 标题: string } | null {
  ...
  if (!m) return null
  // 16+ 位数字超 2^53 精度错位——isSafeInteger 守卫，非法按无章号
  const no = Number(m[1])
  return Number.isSafeInteger(no) ? { 章号: no, 标题: m[2]! } : null
}

/**
 * 章号补零宽度写侧单源——长篇 chapter 4 位 / 短篇 piece 3 位。
