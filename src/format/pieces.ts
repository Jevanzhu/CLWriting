 ...
 * 短篇正文元数据读写 —— 依据 M8 #27。
 *
 * 短篇落点：写作/正文/<章号>-<标题>.md，含 front matter（章号/标题/目标情绪/核心反转）+ 正文。
 * 章纲分离到 大纲/章纲/<章号>-<标题>.md（与正文同文件名、不同目录，互不混放）。
 * 与长篇 chapters.ts 分轨：短篇目标函数是单章情绪爆破，字段集不重合（无钩子类型/情绪定位）。
...
/** 从文件名提取章号 + 标题（001-标题.md → 章号 1，标题「标题」） */
export function parsePieceFileName(fileName: string): { 章号: number; 标题: string } | null {
  // 归一化：去 .md 后缀 + 按 / 切段，取末段（文件名 = 章号-标题）
...
/**
 * 扫描 写作/正文/ 目录，读所有已定稿短篇正文（容错）。
 * 每个 .md 文件 写作/正文/<章号>-<标题>.md 读正文。
 */
...
/**
 * 扫 写作/正文/ 目录下格式合法的短篇文件数（<章号>-<标题>.md）。
 * 与 state.ts countPieces 同口径：只计 `^\d+-*.md` 文件名，不计散文件/隐藏项。
 * state.ts 复用本函数作单源（避免两份计数逻辑漂移）。
 */
export function countPieces(pieceRoot: string, excludeNames?: Set<string>): number {
  try {
    return readdirSync(pieceRoot, { withFileTypes: true })
