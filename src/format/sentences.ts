...
/** 分句：按中文句末标点（。！？）+ 半角 !?（中英混排收尾）+ 省略号 …
 *  （三十一轮：省略号收句与句号同级，此前「她想说什么……」整句不切、句读
...
/**
 * 滑窗句级 n-gram 复读率——count.ts checkRepeat 与 metrics/style.ts
 ...
 * 返回值新增 repeatChars——绝对重复字符量（量纲：每个重复
 ...
 * 与旧字符串键实现逐位等价（等价性由 test/format/ngram-hash-equivalence.test.ts 内嵌
 ...
 * [0, 2^52) 结构性不相交，精确对齐语义；反之 astral 句里纯 BMP 码点组成
...
export function ngramRepeatRate(
  ...
  // BASE^(n-1) mod 2^32——出窗码元/码点在多项式最高次项的系数，滑窗减除用。
  ...
  // 滑窗默认按 UTF-16 码元（快路径——纯 BMP 文本码元=码点，行为
  ...
  for (const s of sentences) {
    if (ASTRAL_CHAR_RE.test(s)) {
      ...
      let h1 = 0
      let h2 = 0
      // 窗内 astral 码点计数——滑窗 O(1) 增减，判别位只随「窗真含 astral 码点」
