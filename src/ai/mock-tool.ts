...
export const MOCK_TOOL_INPUT: Readonly<Record<string, unknown>> = {
  ...
  submit_issues: {
    issues: [{ category: 'pacing', severity: 'S3', evidence: '正文原句', issue: 'mock 问题', fix: 'mock 修复' }],
  },
 ...
 * 非 mock 环境返回 null（调用方走真实 provider 路径）。
 */
export function tryMockTool(toolName: string): { input: unknown; text: string; usage: TokenUsage } | null {
