 ...
 *
 * 威胁模型声明（R73-7，二十一轮 A-7 登记·注释强化；0918三拍板批 KEK v2 更新）：
 * 本文件碎片异或合成是**混淆级**防护，不是密码学秘密——分发制品内两份碎片同包
 * 携带，持久攻击者（能对制品做差分分析/逆向）可复原 IKM。它防的是低成本泄漏面：
 * 磁盘/备份/同步盘上 providers.json 的随手翻阅与拷走（vault 密文无 IKM 解不开），
 * 以及 strings 级扫描的顺手提取。**不防**持源者（掌握本仓库/制品的攻击者）、
 * 运行时内存转储与调试器。
 *
 * KEK v2（0918三拍板批）：桌面形态已接 OS 凭据通道（Electron safeStorage——mac
 * Keychain / win DPAPI）承载 KEK，v2 vault 仅 byOs 通道、本材料不再可解（持制品
 * 攻击面对已迁移用户收口）；v1 vault（纯 node 环境创建 / 未迁移存量）仍由本文件
 * 解锁并自动迁移。**改动此文件 = 摧毁全部 v1 存量用户凭据**（v2 用户不受影响）——
 * 发布检查清单仍必检此文件；迁移清零（监控：盘上 v1 vault 归零）前不可降级此约束。
 */
