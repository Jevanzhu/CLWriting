 ...
 *
 * 写入健壮性（原子写/备份/损坏不静默）属于；凭据文件权限统一 0600 且随创建即生效。
 */
...
export interface ProviderStore {
  ...
  /** 并发修订号：save 写前 +1，读侧无该键视为 0；写端点带 expectedRevision 校验 */
  revision: number
  vault: Vault | null
  dek: Buffer | null
}
 ...
 * ee- 同口径）。
 * （win 平台专项）：原 copyFileSync 覆盖写在 win 有两条失效边——目标只读
...
export function loadProviders(userDataPath: string, opts: { restoreFs?: ProvidersRestoreFsDeps } = {}): ProviderStore {
  ...
  try {
    ...
    // 损坏不静默，且不再直接放弃——主文件解析失败时尝试从 bak 恢复
  ...
  // JSON 合法但形状坏（providers 非数组）——不再静默 emptySettings
  ...
  // RAG（嵌入）服务商——同款解密/明文迁移规则（同源）。形状坏容错为 []：
...
/**
 * 写 providers.json——加密 apiKey 进 vault + 剥离明文 + mode 0600 创建即生效。
 *
 * 每次 save 以 providers 列表为准重建 vault.keys（删除的 provider 自动清除密文）。
 ...
 * 设置页写端点的 expectedRevision 校验（陈旧快照 409 重读）管「请求发起时刻」，
...
/** 跨进程锁等待超时（毫秒）——写段为本地文件 IO 级毫秒，5s 已极保守（同 calls.ts） */
const PROVIDERS_WRITE_LOCK_TIMEOUT_MS = 5_000

/**
 * 迁移后收敛 bak 明文残留——saveProviders 的写前备份会把迁移前的明文
...
function saveProvidersLocked(userDataPath: string, store: ProviderStore): void {
  ...
  // 队列序落盘后到者按旧快照整态覆盖先者（丢更新）——快路同刻 race 由串行链与
  ...
  const sealKeyOf = (id: string, apiKey: string): void => {
    // providerId 绑 AAD——密文换位到其他条目认证失败
 ...
 *
 * 收编：注册槽随 ProviderRuntime 端口实例化——模块级单槽改
...
export function persistDegraded(key: string, userDataPath?: string): void {
  ...
  try {
    ...
    // 降级记忆是优化通道——写失败（load/save 抛错）不向调用方传播，不得中断
    // 已成功的建流；失败由 runner 侧「不标记」承载，下次 persistDegraded 自然重试。
  }
}

/**
 * 降级记忆新鲜读（批次）——与 persistDegraded 对称的查通道。
 *
 ...
 * 显式 path 维度同 persistDegraded（见上注）。
 */
 ...
 *
 * 收编（ForTest 缝随端口实例化）：原模块级测试缝 __clearProvidersCacheForTest/
...
export function tierFromStore(s: ProviderStore, kind: 'creative' | 'assistant' | 'chat'): TierSlot {
  ...
  // tiers 缺 creative 键（providers.json 直灌/半迁移）防御性访问——
...
/**
 * 当前供应商 + 当前模型的模型行覆盖（§7.2 显式 resolve 链第 1 层）。
