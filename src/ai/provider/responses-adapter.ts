...
// 流事件的语义单元（增量/工具累积/终态判决/流尾）——本文件只留骨架
...
/**
 * 本线工具项——SDK 的 FunctionTool 把 strict 声明为必填，本线沿用既有
...
/**
 * 白名单转换点（本文件唯一一处窄断言）——SDK 的 tools 元素要求 strict
...
/**
 * GenRequest → /v1/responses 请求体（全量翻译走 responsesQuirksFor 视图）。
 *
...
export function toParams(conf: ProviderConf, req: GenRequest): ResponsesParams {
  ...
  for (const m of req.messages) {
    ...
    if (m.role === 'assistant') {
      ...
      // function_call_output 先出、text 后出，与 openai 线
      // 块序对齐——Responses 语义 function_call_output 须紧跟其 function_call
      //（跨轮历史里 assistant 的 function_call 在前，中间插 user text 会拆散关联对，
      // 严格网关 400）。当前链路 tool_result 恒独占 user 消息不触发（同注），
      // 本序修正是防御性口径对齐，两线不再相反。
  ...
  // tool-choice.ts（三适配器同构 if 树收敛；rw.toolChoiceMode 视图无
  ...
  if (req.toolChoice) {
    ...
    // 契约「一轮最多一个工具调用」（对齐 Chat/Anthropic 线）
...
export function createOpenAIResponsesProvider(
  ...
  // SDK 内建重试关闭（runner 重试层是唯一重试决策方，见 openai-adapter 同注）
  ...
  return {
    ...
    async *stream(req: GenRequest, signal: AbortSignal): AsyncIterable<GenEvent> {
      ...
      // 事件判定的外部依赖——本线骨架只做「建流 → 逐事件交判定 → 逐条 yield」，
      ...
      try {
        ...
        for (const attempt of plan.attempts) {
          ...
          try {
            ...
            // 本 attempt 的异常用量估计器绑定——流中 SDK 直接 throw 时异常
