...
/** 默认策略：429/5xx/超时/网络错误重试 3 次（族属由 failure.ts 决策表判定），1s 起步指数退避，30s 封顶，±20% 抖动 */
...
/** 错误是否按策略重试（有 code 走决策表单口径；无 code 按 mode 兜底布尔 retryable） */
export function shouldRetryError(policy: RetryPolicy, e: GenError): boolean {
  if (e.code !== undefined) return failureAction(e) === 'retry'
