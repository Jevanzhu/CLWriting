...
// 引号字符集/句读集取 check 域单源（quotes.ts 全部检查器同源宣言）；
...
/** 删书/改名失效挂点（同 forgetSearchCache 口径——书键清理；本缓存键即
...
/** 带缓存的设定数据读取（替代规则层直调 loadSettingData）。 */
...
/** 引号内 2-4 字片段正则（字符集对齐 check/quotes.ts 双体系 LENIENT
...
function loadSettingData(bookRoot: string): SettingData {
  ...
  if (existsSync(roleDir)) {
    for (const f of readdirSync(roleDir)) {
      // .md 判定收敛 isMdFileName（大小写不敏感）——.MD 角色卡
      // 此前被静默跳过（姓名不进离散名称集合）
  ...
  if (existsSync(itemDir)) {
    for (const f of readdirSync(itemDir)) {
      // .md 判定收敛 isMdFileName（大小写不敏感）——.MD 物品卡
      // 此前被静默跳过（名称不进离散名称集合）
      if (!isMdFileName(f)) continue
...
export const settingConsistencyRule: WritingRule = {
  ...
  toPrompt(ctx): string | null {
    // 经 TTL+探针缓存读取（AI 热路径不再每章全量读设定目录）
  ...
  check(body, ctx): RuleViolation[] {
    // 同上——缓存读取，规则变更（目录结构/名册内容）后探针失效不缓存陈旧
    const data = loadSettingDataCached(ctx.bookRoot)
    if (isEmpty(data)) return []

    // 已登记名精确比对集合——离散名称（角色卡/物品卡）+ 名册解析
    // 名。原名册面 rosterText.includes 全文子串粗匹配，长名吞短名（「林晚晴」⊃「林晚」
    // 误判已登记、独立新角色漏报），对齐 check/count.ts checkNewNames 的精确
    ...
    for (const rawLine of ruleStripFm(body).split(/\n+/)) {
      ...
      for (const span of spans) {
        ...
        if (registered.has(name)) continue // 精确全等判重（原 includes 粗匹配吞短名）
        seen.add(name)
        violations.push({
