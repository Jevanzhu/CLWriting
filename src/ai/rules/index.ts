...
/**
 * rules 注入文本与登记清单的同源单次派生——此前 rulesToPrompt 与
...
export function rulesPromptParts(task: string, bookRoot?: string): { prompt: string; files: string[] } {
  ...
  // 高频违规前置注入（读该书 Top-N 命中生成预防指令；无统计零注入，行为同现状）
  const top = bookRoot ? topRuleHits(bookRoot, 3) : []
  const preventionText = buildPreventionText(top)

  // 注入段源文件清单——与上方注入同一次读盘派生（单源）：
  // AI味标签禁词（经 loadAiFlavorRule 注入，空词表不入 prompt 不登记——「空段不登记」）
...
/**
 * 跑全部适用规则 check → 汇总违规项。
 * 违规项 message 可直接作为重写 prompt 条目（多维反馈回流）。
 * chapter 仅供情节一致等按章定位的规则，不传则跳过此类规则。
 */
