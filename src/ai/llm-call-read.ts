/**
 * llm/call 事件读侧单源（2-， GLM-5.3，AI 域 -③）。
 *
...
interface ReadLlmCallRowsOptions {
  /**
   * true = usage 缺失的行跳过（cost 侧口径：判跳看 usage 而非 ok——失败调用
...
function projectRow(e: ChatEvent, skipMissingUsage: boolean): LlmCallReadRow | null {
  ...
  // （cost 侧口径）：skipMissingUsage 见 options 注
...
export async function streamLlmCallRows(
  ...
  if (!store) return
  try {
    // （内存闸）：type SQL 下推——只取 llm/call 行；核查为全量语义必需
  ...
  // 迭代期错误（游标/SQLite 故障）不在此吞——上抛给调用方，使其能丢弃已
