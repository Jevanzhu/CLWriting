...
export async function runAgentTurns(deps: TurnDeps): Promise<boolean> {
  ...
  /** 回合 commit 点 flush 异常收编 finishTurn——磁盘满/血缘校验越界时
  ...
  // 与实际发送用同一预切预算的不变量（「预算先于 runTask 定型」）不受影响。
