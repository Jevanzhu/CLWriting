...
/** 批内序号 → 全局 seq 换算收口——pendingMsgSeqs 累积本回合各消息事件的批内
 ...
 * 依赖它兜底 dispose（buildChatContext 等后续步骤抛异常时防孤儿活跃登记）。
 */
export function prepareChatRun(
  ...
  // 事件库（userData 为空 → null，退化内存模式）+ 跨重启恢复。
  // 内存无历史且库有投影 → 恢复（LRU 逐出/重启后都走这条）。
  let msgSeqs = msgSeqMap.get(opts.bookName) ?? []
  if (opts.regenerate && store) {
    // 重新生成——总是从事件重建到触发 user（parentSeq）为止（不依赖内存历史，
    // 内存可能含旧分支或被截断的历史），沿分支路径
    // 核查：selectBranchTo 需全量事件定祖先链可达性（parentSeq
    ...
    // 恢复走默认分支投影（最新变体组 + 线性兜底），与 GET /chat/history 视图同口径——
    // 全量投影会把兄弟变体顺序堆进模型上下文（regenerate 过的书重启后答非所问）
    // 核查：默认分支判定（最新变体组）与线性兜底都是全量结构
    ...
    // L-：事件库降级（store=null，场景）时 regenerate 的「从事件重建到
  ...
  // （写侧谱系）：本回合分支归属——regenerate = parentSeq + 新 branchId；
  // 无活跃分支（线性书/清空后）→ undefined，行为与旧版完全一致。
  // 续聊回合除 branchId 外补 parentSeq（前驱消息首事件 seq，与前端
  ...
  // 血缘：注入快照指纹（settings/正文预览/技巧包索引）——turn 内登记 settings/snapshot
  ...
  const seqs: ChatSeqLedger = {
    ...
    commitPendingMsgSeqs(range) {
      if (!range) {
        // flush 失败/未落库（无 store 的内存模式或空批）——原样 return 不清
  ...
  if (!opts.regenerate) {
    // regenerate 复用已有 user 消息（历史恢复已含），不再 push/写新 user 事件；
    // 普通回合带活跃分支归属（写侧谱系——regenerate 后的续聊 user 进组）
