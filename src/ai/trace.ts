 ...
 * 旧的文件写入层（appendTrace/readTraceLines + 5MB 轮转）已被事件库
...
interface PromptMeta {
  ...
  hash: string
  /** 清偿批模型可见工具面摘要——挂 tools 的调用登记工具名清单
...
export function promptMeta(systemPrompt: string, userPrompt: string, files: string[] = [], tools: string[] = []): PromptMeta {
  ...
  // chars 记码位数（for..of 迭代）而非 UTF-16 length——usage 估算
  ...
  // 清偿ls 摘要独立成键（不并入 hash/chars——token-calibration
...
/** trace 落盘用的 token 用量提取（从 TaskOk.usage 或 GenResult.usage；含 cache 字段）。
