 ...
 * 同步版 searchBook 切异步孪生——chat 工具经 TOOL_EXECUTORS 在
...
// 命中行截断改用码位口径助手——rewrite.ts 预览切片同款（第 4 处
// 消费方先例），本处为第 5 处；直引 shared/text.js（原经 process/summary
...
export async function bookSearch(ctx: ToolContext, input: Record<string, unknown>): Promise<ToolResult> {
  ...
  return {
    ok: true,
    summary: '找到 ' + out.results.length + ' 处包含「' + q + '」的命中：\n' + lines.join('\n') + more,
  }
}
