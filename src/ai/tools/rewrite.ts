 ...
 * 全文 spill 暂存——确认保存时按 spill 路径取回全文（apply_spill，
 * 兑现「确认后落盘」承诺：write 级确认闸 → 按 locator 取回全文存为章草稿）。
...
// 预览/字数码点口径单源；直引 shared/text.js（原经 process/summary
...
/** 改写 spill 的溯源元数据（章号 + 基线正文 sha256），apply 侧凭此校验归属与新鲜度 */
...
/** 把已确认的改写 spill 落盘为章草稿（保留原章 front matter，只替换正文）。
