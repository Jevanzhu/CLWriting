...
// 全量 rebuild 的 worker 卸载层（摘要自愈路径消费；同步 rebuild 内核在
...
export function chapterSummaryRelPath(chapter: number): string {
  // 登记/留痕路径不得产平台分隔符（win 反斜杠与全库 posix 口径分裂）——
...
export function chapterSummaryState(bookRoot: string, chapter: number, bodyAbsPath: string): SummaryState {
  ...
  // existsSync 后裸 read 直穿自愈链（权限/TOCTOU 读失败抛出）——
  ...
  // 正文 TOCTOU 消失（定稿章在 manifest 扫描与状态判定之间被移删）
  // → computeRevision 裸抛 ENOENT 穿出自愈循环，被 materials 备料的静默 catch 吞掉——
  // 补漏断链且零痕迹。读失败按 missing 降级 + warn：自愈对 missing 照常尝试重生成
...
export function readChapterSummaryBody(bookRoot: string, chapter: number): string | null {
  ...
  // 读失败（权限/TOCTOU）按 null 降级 + warn，不再直穿调用链
  let raw: string
  try {
    raw = readFileSync(fp, 'utf8')
  } catch (e) {
  ...
  // 同上走 frontmatter-core（剥 fm 口径与全库一致，BOM/CRLF 不再漏进注入正文）
  const split = splitFrontMatter(raw)
...
// 约束，崩溃残留不永锁的语义保留。
/** 锁等待档（毫秒；起生产固定 0 = 纯 try-acquire）。三件套换装
...
/**
 * 锁续期周期（毫秒）——锁盖「状态判定 + AI 调用 + 落盘」全临界段
 * 而锁原语的活 pid 超龄门槛与 AI 任务默认超时同为 10 分钟：两处锁
 * 调用不传 renewIntervalMs 时锁文件 mtime 恒为创建时刻，AI 调用一超 10min（慢派发/限流
 * 重试），第二进程按「活 pid 超龄且无续期」接管成双持锁 → 同一章/卷双生成双计费，
 * 跨进程互斥被静默击穿。接线锁原语既有续期能力（task-gate 同款）：30s 刷一次
 * mtime 远低于超龄门槛，活锁不再被误接管。保测试可缩周期。
 * 三件套换装 testableConst 工厂（惯例）：getter 消费点显式调用 + setter
 * 元组第二位原名原签名（测试面零感知）。
 */
 ...
 * （确定性上限，不信任模型自觉）。
 * 进程内 inFlight 快速去重 + 按章跨进程锁盖「状态判定 + AI 调用 + 落盘」全临界段；
...
export async function generateChapterSummary(opts: GenerateChapterSummaryOpts): Promise<GenerateSummaryResult> {
  ...
  // 跨进程锁（锁内才判状态——锁外判会重开「判完他进程开写」的 TOCTOU 窗口）
  // 接线续期——持锁面覆盖全长 AI 调用，不续期会被第二进程按超龄接管成双持锁
  const releaseLock = acquireCrossProcessLockWithTimeout(
    // 锁名补 .lock 后缀——fs/atomic sweepAbandonedTmpFiles 的陈锁
    // 清扫分支只认 `*.lock`，此前 `.摘要锁-章N` 永不命中：崩溃残留锁只能靠
  ...
  try {
    ...
    // 非 UTF-8 正文拒绝生成摘要——GBK 等文件以 utf-8 解码出 U+FFFD
    // 后喂 AI 生成摘要再回写，摘要静默失真且被指纹绑定（过期判定认它为 fresh，永不再生）。
    // 判据与保存链闸（document/service.isUtf8Bytes）同款 fatal 解码探测，补齐防线
    ...
    const out = await runSpec(SUMMARY_CHAPTER_SPEC, {
      ...
      // 登记真实注入源——模型实际读的是 draft.body（正文全文），
    ...
    const fm = [
      ...
      // 低级项：占位符 'summary-chapter' 换成实际模型 id（TaskOk.model 透出；
...
async function runFinalizeSummaryOnce(
  ...
  // manifest 是可篡改数据面——路径过 safeManifestPath 防只读
...
export function afterFinalizeGenerateSummary(
  ...
  docId: string,
  bookName?: string,
  /** driver/session 可选接线——传入时后台任务持独立登记 ctrl（可被 /interrupt
  ...
  // 整段 try-catch 自留痕（p 不 reject）——登记进 per-book 后台表供 settle 追赶
  if (bookName) registerBackgroundTask(bookName, p)
}

/**
 * 批量定稿的串行摘要链：逐章 fire-and-forget 会让一键定稿 N 章 = N 路摘要
 * AI 并发发出（provider 限流整批失败 + 成本尖峰）；同书摘要互不依赖，串行即可。
 * 整条链作为**一条**后台任务登记：删书/改名的 settle 在链首即能追上全部在途
...
export function afterFinalizeGenerateSummaryBatch(
  ...
  docIds: string[],
  bookName?: string,
  /** driver/session 可选接线（同 afterFinalizeGenerateSummary）——整条串行链
...
/**
 * 挂点二（按需自愈，-①）：备料前发现近章（/）摘要缺失或过期 → 现场补生成。
...
export function volumeSummaryRelPath(volume: number): string {
  // 登记/留痕路径 posix 归一（同 chapterSummaryRelPath）
...
/**
 * 生成第 volume 卷摘要：输入 = 该卷完整章摘要链（N × summary_chapter_max 字）。
...
export async function generateVolumeSummary(opts: {
  ...
  // 卷摘要对齐章摘要同款两级去重——进程内 inFlight 快速去重 +
  ...
  // 接线续期（同章摘要锁）——链完整性判定 + AI 调用 + 落盘全临界段在持锁面内
  const releaseLock = acquireCrossProcessLockWithTimeout(
    // 锁名补 .lock 后缀（同章摘要锁——sweep 陈锁清扫分支只认 *.lock）
  ...
  try {
    ...
    // 已有且链未变 → skipped（sourceHash 重读包 try/catch——读失败（权限/TOCTOU）
    ...
    if (existsSync(fp)) {
      ...
      // 指纹只认 fm 段——正文行首 sourceHash 不再参与 skipped 判定
    ...
    const out = await runSpec(SUMMARY_VOLUME_SPEC, {
      ...
      userPrompt,
      bookRoot,
      ...(opts.signal ? { signal: opts.signal } : {}), // 中断透传到在途 LLM 调用
      // 登记真实注入源——模型实际读的是章摘要链（chainText），
    ...
    const fm = [
      ...
      `volume: ${volume}`,
      `generatedAt: ${new Date().toISOString()}`,
      // 低级项：占位符 'summary-volume' 换成实际模型 id（mock 快路 → 'unknown'）
      `model: ${out.model ?? 'unknown'}`,
...
/**
 * 挂点：备料 rank-3 段需要 `卷摘要/<当前卷-1>.md` 而缺失时按需生成。
...
export async function selfHealVolumeSummary(
  ...
  if (existsSync(fp)) {
    ...
    // 指纹只认 fm 段——正文行首 sourceHash 不再把无 fm 手写产物
    // 伪装成程序生成（作者优先甄别回归 fm 事实）
    const m = volRaw !== null ? volumeSourceHash(volRaw) : null
    if (!m) return null
    const { chain } = volumeChainState(bookRoot, targetVolume, volumeSize)
    // 链不全时链指纹无从计算（既不能证新也不能证旧），此前与
...
/**
 * 备料陈旧闸判据（prepare 弹性#3 注入前查询）——「可证明过期」
 * 仅指：程序生成（fm 带 sourceHash）+ 当前链完整非空 + 链指纹不匹配。手写产物（
 * 作者优先）、链不全（指纹无从计算）、空链（退化指纹无比较意义，legacy/无清单书一律
 * 放行）、读失败（宁不动）一律 false——宁窄勿误杀，闸只拦「拿得出指纹证明
...
export function volumeSummaryProvablyStale(bookRoot: string, volume: number, volumeSize: number): boolean {
  ...
  try {
    ...
    return false // 读失败无法证明过期（同款保守：宁注入路径自行降级，不在这里下判）
  }
  // 指纹只认 fm 段——正文行首 sourceHash 不再触发误判「可证明过期」
