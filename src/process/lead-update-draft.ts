...
import { snapshotBeforeOverwrite } from './draft-pipeline.js' // 覆盖留底单源复用
...
// ff- 常量归一：路径唯一出处 check/lead-updates.ts（闸/回写/草拟三方共用），此处再导出兼容既有导入方
export { LEAD_UPDATES_FILE, LEAD_UPDATES_ARCHIVE_DIR }

// kk-同书生成串行化队列——端点直调与 self-heal 写稿完成后两路并发时，
...
// 进程内队列不防**跨进程**双写（GUI 与 CLI 同书各跑各的队列），
...
async function generateLeadUpdateDraftInner(
  ...
  // promptFiles 登记真实注入源——prune 后本章正文 + 细纲（若在）。
  // 「进行中账本」段来自 布线/ 等目录聚合（多文件、按状态过滤），无逐文件实路径，
  // 不虚报登记。铁律：模型可见 ⟺ 已记录。
  // 细纲登记与水源同门禁——prompt 注入的是 readOutlineLeads 的
  ...
  // signal 桥接进 runSpec——调用方（self-heal/chat）中断时本生成同步中止
  // chapter 透传进预算闸——原 LEAD_UPDATE_SPEC 路径只进 task 块
  ...
  try {
    ...
    // 归档 + 覆写两步在按书跨进程锁内原子执行（跨进程双写收口，见上注）。
    // withLeadUpdateLock 异步化 → await（锁语义不变）
    await withLeadUpdateLock(bookRoot, () => {
      archivePendingLeadUpdates(bookRoot, chapter)
      // 覆盖前快照留底（对齐 onboard.ts 先例）——lead-updates
      ...
      // 产物不因留底 IO 抖动丢弃，同取舍）。调用方（端点/chat 工具/self-heal
      // 收尾）均为生成路径，无不该留底的反例面。
...
/**
 * 主文件若载有**其他章**的待确认条目 → 归档到 工作区/.账本推进暂存/第N章.md。
...
export function archivePendingLeadUpdates(bookRoot: string, forChapter: number): void {
  ...
  // （win 平台专项）：剥首行 BOM——记事本「UTF-8 with BOM」保存后 ^# 不中
  // → 误判「无标签旧格式」跳过归档 → 他章待确认推进草稿被下次生成静默覆盖丢失
  // （读侧孪生 check/lead-updates.ts readLeadUpdateChapterTag 的同族写侧补齐）。
  ...
  try {
    ...
    // 旧档读失败：宁保两代不损——回落时间戳变体（L- 保底语义，见下）
  ...
  // L-语义延续：同章旧「未确认推进草稿」仍不静默丢失——可解析条目全数
 ...
 * 关键约束：证据必须是正文原句（readChapterLeadUpdates 用 evidenceNeedles 多候选命中正文才算兑现），
...
export function buildLeadUpdatePrompt(bookRoot: string, chapter: number, body: string): string {
  ...
  const parts: string[] = [
    ...
    // 超长正文无通知硬切 slice(0,6000) → 修剪器头尾保留（账本证据常在章尾）。
