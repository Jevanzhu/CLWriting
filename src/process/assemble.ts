 ...
 * - 当前卷信息
 *
 * 全程读精准读取（format/read.ts），零 token 脚本组装。
 */
...
interface StatusSnapshot {
  ...
  /** 因 cap 未列入 openLeads 的进行中线数（未超限缺省；formatStatus 据此追加提示行） */
 ...
 * 组装近况快照。
 * @param db 书仓库的缓存
 * @param config book.yaml 配置（读 thresholds + leads.enabled；起第三参缺省时还读 book.volume_size）
...
export function assembleStatus(
  ...
  // 第三参缺省原先硬编码 50——chapter_status 工具按「只传 config」调用导致
  ...
  // 进行中的账本
  // 行按 opened_at 升序（现有排序）——超 cap 取尾部（最近开启的线），省略数
...
export function formatStatus(snapshot: StatusSnapshot): string {
  ...
  if (snapshot.openLeads.length > 0) {
    ...
    // cap 生效时追加提示行——AI/作者可知晓尚有未列入的进行中线
