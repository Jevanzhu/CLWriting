 ...
 * 补单弯引号 ‘’——QUOTE_OPEN/QUOTE_CLOSE 常量自含 ‘’，
...
/** 剥除行内全部引号片段，返回引号外文本（对话标签判定只看提示语）。
 *  ：本函数按行高频调用、原每次 new RegExp 提升为模块级常量
 *  （String.replace 对 g 正则每调用重置扫描位，共享常量安全，语义逐字不变）。
 *  （修复批）：2-slot 引用 memo——runAllChecks 每
