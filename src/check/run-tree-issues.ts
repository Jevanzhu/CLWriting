 ...
 *
 * R0916-7-P3-2（2026-09-24 评审 P3-2/P3-3）两处结构变更（行为逐位不变）：
 ...
 *   run.ts 退为兼容桥（只 re-export）。两文件互 import 的运行时环（评审 P3-3 的
...
// ── 聚合管线：四段接力（R0916-7-P3-2）─────────────────────────────────────────
// 段序是三处既有不变量所定，不可调换：
// ① 纪元基线（epochFp0）必须在 rebuild **之前**算得（R52-E-1）——头窗内源变使首尾 fp
//    不等，整批落盘按既有失效路径丢弃，陈旧红值不会以新纪元固化；基线缺席（前算失败）
//    一律按 miss 处理（宁重算勿混纪元）。
// ② 账本全书性红项先于章循环定型（H-1）——章级条目的展示值要合并它，而章级缓存行只存
//    章作用域 hasRed。
// ③ 待落盘章缓存行必须在循环后经一次终核纪元再写盘（R32-14）——故「逐章收集」段一次
 ...
 * verdict」产出红点条目与待落盘行，循环后经终核纪元一次批量落盘（R32-14/R33D-17）。
...
/** R37-3：树红点聚合的实现体（生成器，单源供同步/async 双驱动）——四段接力驱动。
 *  阶段 52 批 1：yield 面含效应请求（TreeEffect），由驱动解释执行（见上「效应让出」注）。
 *  R0916-7-P3-2：段序不变量见上方「聚合管线」块注。 */
function* collectTreeIssuesCore(
  ...
  try {
    ...
    if (pre.db) closeTreeIssuesDb(pre.db) // R0916-6：裸 close 改道（prepared ephemeron 断链，closeTreeIssuesDb 单源）
  }
}
