 ...
 *
 * R0916-5e（2026-09-16，⑤④产品巨件拆分波1）：count.ts（1144 行）纯移动拆分——
...
// R0916-5e（2026-09-16 拆分批）：HANZI 区间/SPEECH_VERBS 动词集是跨缝共享单源——
// 「留 count.ts 残核供两缝 import」经 ESM 环形依赖推演不可行（count.ts 的 re-export
// import 提升先于残核常量初始化，两新文件顶层 RegExp 构造即踩 TDZ ReferenceError），
// 故随缝 A 落 count-dialogue.ts export、本文件直接 import，单源不重复（R28-8 口径不破）。
 ...
 * R0916-5e（2026-09-16 拆分批）：自 count.ts 缝 A 区段随消费方 computeStyleMetrics
