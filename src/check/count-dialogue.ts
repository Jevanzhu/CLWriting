...
// R0916-5e（2026-09-16 拆分批）：本常量跨缝共享（本文件 HANZI_CHAR_RE/
 ...
 *  R0916-5e（2026-09-16 拆分批）：导出——DIALOGUE_TAG_RE 随其唯一消费方
 *  computeStyleMetrics 落 count-style.ts（缝 B），本动词集跨缝仍收单源（R28-8
...
export function parseRosterNames(roster: string): string[] {
  ...
  for (const rawLine of roster.split(/\r?\n/)) {
    const cleaned = rawLine
      // R37-8：`[ \t]*` 收窄（原 `\s*`）——`\s` 含换行/全角空格，本处虽逐行处理
