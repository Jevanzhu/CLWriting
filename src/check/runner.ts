 ...
 *
 * 聚合 ⑩ 第 2 节全部 11 项检查（红 4 项 + 黄 7 项）→ CheckReport。
 * 红 > 0 → 自愈打回（⑩ 第 6 节）。
 * 顺带产出账本变动清单 / 信息差候选 / 新专名候选（⑩ 第 2 节末，供阶段 6 三审）。
 */
...
export interface CheckInput {
  ...
  declaredLeadIds?: string[] // 本章细纲声明推进的账本编号（两端闭合，⑩ 项 1）
  actualLeadIds?: string[] // 本章实际写入履历的账本编号（两端闭合对照侧）
  imageryWords?: string[] // 高频意象表（⑩ 项 7，默认空，数据待 M4 知识层平移）
  leakKeywords?: string[] // 信息差关键词（⑩ 项 11，默认空，数据源待定）
}

/** 跑全套机检（⑩ 第 2 节 11 项）→ CheckReport */
export function runAllChecks(input: CheckInput): CheckReport {
  ...
  // ⑩ 项 1 账本形式三检（红）—— 章号一致 / 引文命中 / 状态闭合 / 两端闭合
  sections.push(
  ...
  // ⑩ 项 9 文风可量化（黄）—— 读 文风铁律.md 的可量化硬约束阈值（⑤ 第 8 节）
