/**
 * 单文档机检执行（架构下沉：从 studio/server/api/check 下沉内核）。
 *
 ...
 * 直接引本模块，环边消失；既有 'check/run.js' import 面零改动。
 *
 * 依赖方向不变量：本模块**不得**引 run.ts / run-tree-issues.ts（引回即重建环）——
 * 需要复用的公共件（如 enabledLeadTypes → leads-config.ts）一律落在中立模块。
 */
...
/**
 * 「rebuild→开库→PRAGMA」前奏参数化——单章端点与树聚合
...
export function openCheckDb(bookRoot: string, hasWiring: boolean, opts: OpenCheckDbOpts): OpenCheckDbResult {
  ...
  try {
    // 单章机检链的增量探测 opt-in 3s TTL 节流——四棵源树逐文件
    ...
    try {
      ...
      closeTreeIssuesDb(db) // 审计：PRAGMA 抛错（库损坏/锁超时）不留已开句柄；改道 closeTreeIssuesDb（prepared 调用面连接断 ephemeron 链再关，同 rag closeRagDb 口径）
 ...
 *
 * 与同步版的**已知差异**（刀 2 承接，见下）：的 3s 探测节流状态在 worker 内随
...
export async function openCheckDbAsync(
  ...
  try {
    // 承接（备选 (a)）：窗内跳重建——库在盘才跳（被删/首次进门的自愈面不受节流
...
/**
 * 降级黄项「后置 push + strict 短篇升红」三连同构收编
 * ——原在 runCheckForDocument（book.yaml 降级）与 checkWithDb（账本兑现侧
 * / 声明侧）逐字复制。section 名 / checkId / level / 升红语义逐位不变，
 * 纯机械去重。
 */
function pushDegradedYellow(
  ...
  // -（全量代码）：strict 生效判定走 effectiveShort
...
export function runCheckForDocument(
  ...
  const db = opened.db
  try {
    // book.yaml 降级黄项透出——config 回落默认仍能跑，但阈值/词表/
    // 账本类配置本轮未生效，作者只看 warn 日志无从知晓；在机检报告里透出黄项（面板可见，
    // 不驱动红闸）让「降级事实」与「机检结果」同屏。（升红语义见 pushDegradedYellow 注）
    ...
    if (db) closeTreeIssuesDb(db) // 裸 close 改道（prepared ephemeron 断链）
  }
}
...
export function* maxWrittenChapterOfCore(
  ...
  // 树聚合侧接受已读 entries（collectTreeIssuesCore 聚合头
  ...
  for (const e of entries.values()) {
    // join 键折叠（platformCaseFold 单源：win32/darwin 折叠 + NFC；
    // 起 darwin 也折叠）——外部 case-only 改名 / NFD 文件名后精确串失配，
  ...
  // （阶段 24）：并入感知——合并最高定稿章后源章摘除（正文文件 + manifest 定稿集
  ...
  if (max > 0) return max
  // 零定稿书回退全书最高现存章号——与树聚合（collectTreeIssues
...
export function* scanChapterUpdatesByChapterCore(
  ...
  if (existsSync(archiveDir)) {
    // existsSync→readdirSync 间隙目录被瞬删/异常迁移（TOCTOU，同
    // 口径）时 ENOENT 直穿炸整条批量机检链路（端点 500）——降级空列表 + warn
...
export function* checkWithDbCore(
  ...
  try {
    ...
    // 细纲声明按被检章过滤（细纲单文件覆盖写，旧草稿复检不得对上新章声明）
    // 声明侧三态——细纲自带章号 ≠ 被检章 = 声明未知（批量连写常态：
    ...
    // 实际侧同口径按被检章过滤（声明侧同向）——
    // 他章证据不作本章「已兑现」参照。
    // actual 侧改走 readChapterUpdatesForChapter 单源（主文件（属于
    // 本章时）+ 工作区/.账本推进暂存/第N章.md，内含 in-scope 判定）——此前只读主文件：
    // 批量连写书的归档章推进被无视，误报 lead-declared-not-done 红（定稿闸与履历回写
    // 自 ff- 已统一本函数，机检是最后缺口）。batch 预扫闭包同口径（消每章重读）
    // 兑现侧读失败（权限/瞬态占用）≠「无推进」——unreadable 时
    // actualLeadIds 传 undefined 跳过两端闭合（对齐声明侧 known:false 口径，
    ...
    // 批量聚合经 batch 传预扫列表；单章端点现扫（只消除批量时的每章重扫）
    ...
    const report: CheckReport = yield* runAllChecksCore({
      ...
      body: draft.body,
      // 必须用真实文件名（从章号自身合成则 fm-chapter-mismatch 恒不触发，
    ...
    // push + strict 升红收编 pushDegradedYellow（语义见其注）。
    ...
    // 声明侧读失败的黄项降级（对齐兑现侧 fail-noisy 口径）
    ...
    // （§四.11）：异常转 CHECK_ERROR 信封时补 warn
