...
export interface RagConfig {
  ...
  endpoint?: string
  model?: string
  /** 召回惰性指纹校验候选章上限（book.yaml rag.candidate_depth，缺省 20） */
  candidate_depth?: number
  /** embedding 单请求超时毫秒（book.yaml rag.embed_timeout_ms，缺省 embed.ts
 ...
 * 「配置损坏」与「未设段」分岔——book.yaml 读不出/解析失败时书级
...
export function readRagConfig(bookRoot: string, userDataPath?: string | null): RagConfig {
  ...
  if (!cfg.ok) {
    // 损坏不托底——禁用并留痕（book.yaml 是书库必需件，读不出
