...
// R47-1（四十七轮）：标题派生防抖——下方 watch 的 parseFmFields 此前每击键全文行
// 数组分配；切文档（docId 变）即刻取新值（watch 的 titleEditing 脱离编辑态分支当拍
// 判定语义不变，沿 R43-17 口径）。
const debTitleContent = useDebouncedSource(() => entry.value?.content ?? '', { key: () => props.docId })
