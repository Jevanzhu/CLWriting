/**
 * E5（复审-0914-优化修复批）：dirty 正文节流镜像（渲染进程硬崩溃兜底）——自
 * stores/doc.ts 抽出的独立模块。纯 localStorage 逻辑：键格式/节流分档/指纹台账/
 * 书级清扫/复活判读的语义与实现逐位不变（R55-F-3 首版 / R-P2-1 清理面补全 /
 * R-P2-2 节流分档+指纹跳写 / R57-E-1 baseRev 复活时效门 / R57-E-3 属主精确判定，
 * 沿革注记随实现移位于此），文档缓存态经 deps 参数化注入，doc store 侧改薄委托。
 *
 * 三面（R55-F-3）：①写侧——dirty/conflict entry 内容节流镜像进 localStorage
