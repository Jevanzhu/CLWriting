...
// UI 全局状态：命令面板 / 设置 / 导出弹窗可见性 + Toast 队列 + AI 可达性（降级）。
export interface ToastItem {
  id: number
  msg: string
  // 补 'warning' 级——「已落盘但未完全生效」类半失败提示（如恢复后
...
export const MASK_ALPHA = {
  ...
  // 章节属性/拆分对话框原只写自己的 CSS、未登记——⌘P 命令面板（z 150）盖在
...
/** 探测失败重试间隔——指数退避（5s 起步 ×2 封顶 60s）。原 5s 固定
...
export const useUiStore = defineStore('ui', () => {
  ...
  // 窗控遮罩联动（win）：有效遮罩浓度变化 → prefs 按浓度合成压暗色并单拍瞬切
  ...
  function ask(opts: {
    ...
    return new Promise((resolve) => {
      // 并发 ask 直接顶掉旧弹窗时，旧 Promise 以「取消」结清——
  ...
  /** toast 上限——循环失败（AI 轮询报错等）此前逐条无界堆叠遮屏 */
  const TOAST_MAX = 5
  /** 消失计时器登记表（同文案合并重置计时用；dismiss 时同步清理） */
  const toastTimers = new Map<number, ReturnType<typeof setTimeout>>()
  function armToastTimer(id: number, kind: ToastItem['kind']): void {
    toastTimers.set(
      id,
      setTimeout(() => dismissToast(id), kind === 'error' || kind === 'warning' ? 5000 : 1800), // warning 需作者行动（如手动重载），时长对齐 error 档
    )
  }
  /** 关闭单条 toast（点击关闭 + 计时到期共用出口） */
  ...
  /** 探测 AI 可达性（启动调一次；失败按指数退避自动重试，available:true 成功即停）。 */
  let probeTimer: ReturnType<typeof setTimeout> | null = null
  let probeStep = 0 // 退避阶数（成功复位；每次失败 +1 → 5s→10s→…→60s 封顶）
  ...
  async function probeAiStatus(): Promise<void> {
    try {
      ...
      // available:false 也重试（与网络异常同口径）——后端可达但
      // AI 供应商未配置/未就绪是暂时态，只有 available:true 才停止探测
      if (s.available) {
        probeStep = 0 // 成功复位退避阶数（下次失败重新从 5s 起步）
        if (probeTimer) {
