...
export const useDocStore = defineStore('doc', () => {
  ...
  function setBook(name: string): void {
    ...
    loading.clear()
    // R33-12（三十三轮）：inflightSaves 一并清——旧书在途保存的 finally 无条件按
  ...
  async function open(node: TreeNode): Promise<void> {
    ...
    try {
      ...
      if (gen === bookGen) loading.delete(node.docId)
    }
  }
  ...
  return { docs, bookName, setBook, get, open, patch, save, reloadFromRemote, overwriteRemote, refresh, finalize, conflictedDirtyDocs, flushDirty, flushSyncOnUnload, autosaveTick, discard }
})
