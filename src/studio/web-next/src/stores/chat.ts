 ...
 * 刷新/切书后经 seedHistory 从事件库投影恢复历史（仅 messages 为空时种子化）。
...
// RC：消息/工具卡片模型迁入 ./chat-dispatch——对外类型面原样转发。
...
export const useChatStore = defineStore('chat', () => {
  ...
  /** 当前激活分支（history 返回的实际采用分支；无分支语义/未拉取时 null） */
  ...
  /** 配套：章号显式记忆只读访问器（迁移链回归测试面；无记忆 = undefined）。 */
  ...
  /** RC：事件分发状态机随批迁入 ./chat-dispatch（dispatch 的 11 个
  ...
  async function seedHistory(bookName: string, opts?: { replace?: boolean }): Promise<void> {
    ...
    if (!bookName) return
    // running 中种子化会吞掉在途回合的增量（clear 后回合引用已复位）——改为
  ...
  /** best-effort 刷新分支列表（失败静默；gen 不符丢弃防旧书数据污染新书） */
  ...
  /** 生成新分支 id（b + 时间戳 36 进制 + 随机尾，防同毫秒碰撞） */
  ...
  async function regenerate(bookName: string, chapter?: number): Promise<void> {
    ...
    try {
      let data: ChatHistoryResult
      try {
        // 带 activeBranchId 拉当前显示分支的历史（对齐 switchBranch
      ...
      // regenBook 前置到 POST 之前——原实现「POST 成功返回后才赋值」的
      ...
      // 未交接（POST 失败/前置拒绝/期间清空）时连带清前置登记的 regenBook，
  ...
  function clear(): void {
    ...
    running.value = false
    // 重置分支态 + 复位重新生成进行中标志（清空后旧分支/在途操作不得残留）
    ...
    turn.regenPending = false
    turn.regenBook = null
    // 切书在此收口（Book.vue 切书链统一调 clear）——章号语境换到目标书的
  ...
  // 在途回合收尾（running 翻 false）自动补种登记中的书——切书窗口内被 clear
  // 掉的回合届时从服务端历史回填，不再失明。store 常驻（App 级），watch 不卸载。
  // 补种走 replace:true——pendingReseed 登记时 messages 已非空（历史先于
