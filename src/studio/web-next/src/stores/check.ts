 ...
 *  ：分隔符用 \u0000（learn.ts sampleKey 同款手法）——冒号拼接在
 *  书名自身含冒号时前缀歧义：clearFalsePositiveMarks('A') 的 `clw-fp:A:` 前缀会连带
 *  命中书 'A:B' 的键。存量冒号旧键不迁移（展示态 best-effort，自然失配即弃），
 *  删书清理处顺手清旧键。 */
/** 书名前缀单源导出——useShelf.migrateBookKeyedState 的改名迁移
 *  分支此前仍拼旧冒号前缀（改 \u0000 时的连带漏改），现行键永不匹配、改名迁移
...
export const useCheckStore = defineStore('check', () => {
  ...
  // （修复批）：原 lastDocId ref 删除——赋值后全仓零消费
  ...
  /** 操作代（与 review store 同款）：run/clear 共用——切文档后旧请求结果不落。
  ...
  async function run(name: string, docId: string): Promise<void> {
    ...
    try {
      ...
      // 新报告落位即在途标记态复位——flag 在途时 run 推代（flag 只
      // 快照不推进），迟到的 finally 查代不过会让 flagging 停留旧报告的 checkId，误报
      // 按钮永久禁用直到切文档；此处直接复位（对齐修 loading 的思路，双保险）
      flagging.value = null
      // 灰显态从 localStorage 按书+文档回填（刷新后已标误报仍灰显）
  ...
  function clear(): void {
    ...
    error.value = null
    hasRed.value = false
    // checkId 是检查器级 id（跨文档/跨书同名）——不清会让旧文档的标记灰显到
  ...
  /** 标误报（幂等——已标过不重复请求）；错误置 flagError 供面板提示。
   *  ：入口捕获 opGen、落态前查代——与同文件 run/clear 的既有纪律
  ...
  async function flagFalsePositive(name: string, docId: string, checkId: string): Promise<void> {
    ...
    try {
      ...
      saveFlagged(name, docId) // 刷新后灰显态可回填
    } catch (e) {
      if (opGen.stale(gen)) return
      flagError.value = friendlyError(e)
    } finally {
      // 归属制清除（不依赖代数）——查代制在「flag 在途 + run 推代」
...
export function clearFalsePositiveMarks(bookName: string): void {
  // 前缀改取 fpBookPrefix 单源（口径）——原处内联重拼
  // `clw-fp:<书>\u0000`，与 fpKey/fpBookPrefix 构成双源，键格式再演化（如分隔符调整）时
  // 此处必成漏改点（修的正是 useShelf 侧同款漏改）。行为不变：同串前缀。
...
export function clearFalsePositiveMarksForDoc(bookName: string, docId: string): void {
  // 键形 `clw-fp:<书>\u0000<docId>`——书前缀带 \u0000 分隔（更长书名不误吞，
  // 口径），其余段即 docId 段，精确等值才删（前缀兄弟键如 `a.md2` 存活）
