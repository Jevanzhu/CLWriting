...
export const useWordsStore = defineStore('words', () => {
  ...
  /** 今日精确增量（当日所有 save settled 的 delta 累加；null = 无记录回退 baseline）。 */
  ...
  // 并发 save settle 各调一次 ensureBaseline(同书)，原样发 N 次 GET /words-diary 而次
  ...
  async function doEnsureBaseline(name: string): Promise<void> {
    // 请求代守卫——切书后旧书慢响应不污染今日字数基线（后调者胜）
    const gen = reqGen.begin()
    // 切书入口清态——tree.totalWords 已是新书而 baseline/delta 还是旧书时，
    ...
    try {
      ...
      if (r.date !== localToday()) {
        // 属主不匹配（bookTotalWords=null）时无基线源可重记——跳过重记/重取，
        ...
        if (r.baseline === null && bookTotalWords !== null) {
          baseline.value = bookTotalWords
          // postBaseline 后查代——await 期间切书（旧书 ensureBaseline
      ...
      if (reqGen.stale(gen)) return
      // 失败降级须一并清 delta——否则旧书/上次成功的 delta 残留且
      // 优先级高于 baseline 回退，今日字数显示的是别人（旧书）的增量
      todayDelta.value = null
      date.value = null
      // 降级基线同过属主校验——树滞留旧书时不取旧总值（今日 0 展示）
      baseline.value = bookTotalWords
    } finally {
  ...
  /** 清今日字数展示态（脏路由 name='' 离开书时由 ChapterTreePanel 调）——
   *  前书的 date/baseline/delta 不再参与展示；loadedFor 复位 + reqGen 推代，在途旧书
   *  响应落定不回填，下次进书按新书重取。 */
  function reset(): void {
    reqGen.invalidate()
    loadedFor = null
    // 在途台账一并清——reset 已推代，在途共享 promise 落定时被 gen 守卫
