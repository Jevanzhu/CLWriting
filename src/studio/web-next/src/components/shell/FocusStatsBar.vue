...
// 隐藏判据 = 页宽 ≥ 视口（侧位 ≤0，条必然压正文）；侧位紧张但 >0 时靠的
...
function resetSession(): void {
  // 防抖下切章/重进先冲刷——基线必须锁「当拍」字数，防 150ms 窗内基线滞留
...
// 会话重开改盯 entry 对象身份——换章必伴随 entry 换对象
...
watch(words, (w, old) => {
  // 文档切换/迟到加载的置位跳变恰好落在 reset 快照上——非动笔，不起钟
  ...
  wordsAtReset = null
  // 空章首笔（基线 null 且文档在场，words 从 0 起）——基线按旧值 0 锁：
...
const chapterTarget = computed(() => {
  if (entry.value) {
    ...
    // isFinite 守卫同 WritingInfoPanel——脏 fm 手填不产 NaN 目标
...
watch(
  ...
  (v) => {
    ...
    // 退出汇报前冲刷在途字数防抖——退出落在 150ms 防抖窗内时本次增量不被低估
    flushWords()
 ...
 * 窄窗侧位不足（边距 < 条宽 + 间距，如 1280 屏配默认页宽 1020 边距仅
