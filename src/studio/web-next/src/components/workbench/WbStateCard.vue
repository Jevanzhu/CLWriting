...
// 工作台状态卡（巨石 拆分）：导航灯——当前在哪（stateName）+ 该做什么（humanMsg +
// kk- 连写暂停提示）+ 一键操作（建议动作按钮）。动作执行（写稿上下文拼装）在父层。
import { computed } from 'vue'
import { useWorkbenchStore } from '../../stores/workbench'
// #17：建议按钮补 AI 可用性闸（主「生成」按钮有闸，此旁路此前无）
...
const ui = useUiStore() // #17：AI 可用性闸读取
...
// state 5 的 action='volume-review' 原样映射「卷复盘」按钮——点下去
...
// kk-连写暂停提示（#34）——上次批量连写中途停且未再开批时，状态卡提示从哪章续起
...
// humanMsg 呈现侧兜底对齐——服务端态 5 humanMsg 仍写「建议做卷复盘」
...
// 态 1 崩溃 pending（crashedWrite）的「忽略此提醒」按钮渲染条件。
// 服务端 /state 现未透出逐条 opId（服务端批只落了端点，payload 透出待后续批
...
/* kk-连写暂停提示——弱于 humanMsg 的次级信息行 */
.pause-msg {
  font-size: var(--font-size-s);
  color: var(--text-muted);
  line-height: 1.6;
  /* --border-color 全库无定义（computed-value 无效回落 none），
     连写暂停提示的左强调条此前静默消失；改用实际存在的边框 token */
  border-left: 2px solid var(--background-modifier-border);
  padding-left: 8px;
}
/* 崩溃 pending 忽略行（小按钮 + 次级说明） */
.ack-row {
  display: flex;
  align-items: center;
  gap: var(--size-4-2);
