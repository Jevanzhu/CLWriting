...
// chat 域快端点 15s 档单源（原 5 处裸值 15_000 收敛；
...
/** GET /chat/history 响应 —— 事件库投影的对话历史（扩展 seqs/branchId） */
export interface ChatHistoryResult {
  ...
  /** L-S2limit 尾窗生效时 true（更早消息在事件库/审计视图可查） */
  truncated?: boolean
  /** 投影前该分支消息总数 */
  total?: number
}
...
export async function fetchChatHistory(bookName: string, branchId?: string): Promise<ChatHistoryResult> {
  ...
  // 500→200 对齐 chat store 的 MAX_MESSAGES——展示种子只留
  ...
  // messages 运行时归一单源——类型虽必选，后端异常形态（字段缺省的
...
/** GET /chat/branches —— 分支列表 + 当前激活分支（变体切换器数据源） */
...
/** POST /chat/regenerate —— 从指定 user 消息（parentSeq）重新生成回复。
