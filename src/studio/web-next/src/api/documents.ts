...
// 回包类型单源 = 根 src/shared/contract/documents.ts（两端共用同一份
...
// expectedRevision 可选——传入时服务端做乐观锁，基线不符抛 ApiError{code:'REVISION_CONFLICT'}；
...
// 显式声明可重放——body 带 newOperationId 幂等键（服务端按 operationId
...
export async function batchFinalizeDocs(name: string, docIds: string[]): Promise<BatchFinalizeOk> {
  return apiJson<BatchFinalizeOk>(
    ...
    120_000, // 慢档对齐 clearAudit——批量定稿逐章 git 提交可达数秒/章，30s 默认档必假超时（服务端继续成功、前端报超时不刷树）
  )
}
