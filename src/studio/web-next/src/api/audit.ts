// 审计客户端：GET /api/books/:name/audit → 事件重放 + 遮蔽差异 + 工作流链路。
...
export interface AuditConversationFE {
  events: AuditEventFE[]
  /** 对话事件总条数（与 events.length 区分——前者是当前页条数，后者是能否翻页的总数） */
  eventsTotal: number
...
/** 分页参数（limit 服务端夹取 1..500；offset 为已载条数起点） */
interface AuditPagingFE {
  limit?: number
  offset?: number
}

/** goal 当前态快照（foldGoals 重放） */
