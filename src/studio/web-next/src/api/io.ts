...
/** 域形状负载（与后端 /export 契约同步收敛——旧 CLI 信封 code/stdout/stderr 已废）。
...
interface ExportResponse {
  ...
  /** 被滤草稿章计数（服务端信封透传）——>0 时 ExportDialog 补提示，
   *  作者可知本次导出实际未含全部在写章 */
...
export async function exportBook(
  ...
  return apiJson<ExportResponse>(
    ...
    // 大书同步遍历可能数秒，无超时则 ExportDialog loading 永真
    60_000,
  )
}
