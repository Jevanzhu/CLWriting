...
export function useBookSwitchGuard(deps: BookSwitchGuardDeps): void {
  ...
  function enterBook(n: string, gen: number): void {
    ...
    clearEventStores()
    // 切书/刷新后从事件库恢复对话历史（store 内自带空判/竞态守卫，失败静默）
    if (n) void chat.seedHistory(n)
    // 切书后刷新对话档位（防短暂显示旧书模型列表）
    void useChatTier().refresh()
    // 进书 = 建书会话（并 abort 上一个会话）。调用前提见 useBookSession
    // 头注的生命周期纪律：此刻旧书的冲刷/决断已全部落定，abort 不会打断冲刷途中的保存。
    beginBookSession(n)
    // 切书链收尾强制重取 SSE sync 快照——上方 await 链（确认弹窗/
  ...
  /** 三段守卫共用决断弹窗（自原 askDropOrRevert 收编：弹窗文案、「丢弃并切换」
   ...
   * onApproved：预检通过、flushDirty 之前的状态转移钩子（兜底路径把 beginSwitch 放这里
   * ——「clear 早于 flushDirty」口径逐位不变；提交前守卫不传：取消要零变更）。
   */
  async function flushAndAdjudicate(opts: {
    ...
    // 守卫拓宽——非冲突保存失败（网络断/5xx）的 dirty 文档同样从未
    // 落盘，setBook 清缓存即不可恢复丢失，与冲突形态同类灾难；统一走确认弹窗
  ...
  onBeforeRouteUpdate(async (to, from) => {
    ...
    // 「切书」决断（无回退目标），既有口径的 flush + 留痕仍在 watch 内（提交后）
