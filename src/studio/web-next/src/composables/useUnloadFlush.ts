...
export function useUnloadFlush(): void {
  ...
  // 告知后由作者处理（冲突守卫并入本监听；：Electron 不渲染浏览器
  ...
  onMounted(() => {
    ...
    // （GLM-5.3 修复批）：钩子首位并入书级 prefs 冲刷
    // ——workspace store 的 500ms 防抖窗内末次布局态此前随关窗静默丢失（备案的
  ...
  // 路由离开 /book（组件卸载，watch(bookName) 不再触发）也 flush 脏文档——
