...
/** 改名迁移失败草稿——与 clearFailedDrafts 删除出口同族，改名路径
...
/** 配套：失败草稿读写访问器——与 clear/migrate 同族的模块级四件套
...
export function useChatComposer(
  ...
  /** enabled=false = 哑实例——不注册草稿回填/章节跟随 watch、
  ...
  const input = ref('')
  /** 取出本书的失败草稿回填输入框（仅输入框为空时回填，不打断已开始的新输入） */
  function restoreFailedDraft(book: string): void {
    const stash = failedDrafts.get(book)
    if (stash === undefined) return
    // 先删后查——stash 存在即无条件销毁、回填却以输入框空为前提，
    // 「切书后回来」时输入框已有新输入反而永久删稿（找回语义自毁）。
  ...
  // 章号语境单一事实源在 chat store（dock/工作台双 composer 实例 +
  ...
  async function handleSend(): Promise<void> {
    ...
    try {
      ...
      if (result.queued) {
        // 成功分支书名复检——await 窗口切书后 chat 上下文已换代为
        // B 书，A 书的入队提示/清错写入 B 对话区（对齐失败分支守卫族口径）
      ...
      if (bookName() === book) {
        // 末条仍是本次幽灵气泡才回滚——直接 popUser 弹「当前末条」在上下文
  ...
  function handleKeydown(e: KeyboardEvent): void {
    if (!enabled) return
    // IME 组合期确认候选的 Enter 让渡——此时 v-model 尚未同步
  ...
  function selectChapter(ch: number | undefined): void {
    if (!enabled) return
    // 写经 chat store（落本书记忆），双实例与 regenerate 同源可见
  ...
  // 菜单开时 Esc 关闭自身且不外溢——对齐 ModelPicker 手法
  ...
  async function handleClear(): Promise<void> {
    if (!enabled) return
    // 书名入口捕获——确认弹窗 await 期间书可能已切换（弹窗可跨书滞留）。
    // 弹窗按发起时的书提问，确认时已不在该书 → 中止：不删错书的服务端历史、不清错书的前端对话区
    const book = bookName()
    // 清空连服务端历史一起删（不可恢复）——danger 二次确认后才执行
    ...
    // dd-失败即中止本地清空并提示——否则前端显示已清、事件库保留，刷新后旧对话复活
    try {
