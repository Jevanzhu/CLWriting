/**
 * 书库选择/切换 IPC 交互单点化（修复批）。
 *
 * Welcome.vue 与 Library.vue 此前各自近逐行维护 chooseLibrary / switchTo（-前端
 * 捕获、 reason toast、 返回值吞没、 #21 交互失败不顶列表——四轮
...
export function useLibraryIpc(options: UseLibraryIpcOptions): {
  ...
  // -前端：交互路径 IPC 捕获（原先 unhandled rejection 零反馈）。
  async function chooseLibrary(): Promise<void> {
    try {
      // （全量代码）：openLibrary 落库失败返回 {ok:false,reason}
      // （此前类型面缺失该变体、返回值被静默吞）——reason 失败就地 toast 交代（用户
      // 取消 canceled 维持静默），对齐 switchLibrary 的处理写法
      const r = await window.clwritingDesktop?.openLibrary()
      if (r && !r.ok && 'reason' in r) ui.toast(r.reason, 'error')
    } catch (e) {
      // #21：交互失败改 toast——原先写 loadError 顶掉已加载信息（loadError 双职）
  ...
  async function switchTo(path: string): Promise<void> {
    ...
    try {
      ...
      // （评审补修）：switchLibrary 返回 {ok:false, reason}（大小写敏感卷警告选「换个
      // 目录」等）此前只 catch 抛错、返回值被静默吞掉——取消原因就地 toast 交代
      if (r && !r.ok) ui.toast(r.reason, 'error')
    } catch (e) {
      // #21：同 chooseLibrary——交互失败不再顶掉已加载信息
      ui.toast(options.formatError(e), 'error')
    }
  }
