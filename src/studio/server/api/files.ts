...
interface FileCtx {
  workDir: string | null
  /** 留底保留策略读 global.json 需要（缺省 → 硬编码默认，与快照端点同口径） */
...
const EDIT_DIRS: { dir: string; mode: 'text' | 'md' }[] = [
  ...
  // 作者确认位：细纲.md（推进声明）与 账本推进.md（实际履历行）需在编辑器可见可改，
  // 但 工作区/ 其余内部资产（.版本/.confirm.json 等）不进编辑白名单——只放行这两个确认文件。
]

/** 作者确认位专用白名单（工作区下仅这两个文件可进编辑器） */
const WORKDIR_EDITABLE = new Set(['工作区/细纲.md', '工作区/账本推进.md'])

/**
 * 异步读全文 + 字节指纹单次读取共源——原 readFileSync + hashFile
 ...
 * （GET 侧编码探测）：读入原始字节后做 isUtf8Bytes 探测——存量
...
export function registerFileRoutes(ctx: FileCtx): void {
  ...
  defineRoute('books.file.get', {
    ...
    handler: async ({ params }, req: IncomingMessage, res: ServerResponse) => {
      ...
      // 异步读取（原 existsSync + readFileSync + hashFile 三份同步 IO）
      // encodingSuspect 随响应透出（非 UTF-8 存量的打开时告警，doOpen 消费）
      const read = await readFileHashed(safe)
      if (!read) return replyError(res, 404, 'NOT_FOUND', '文件不存在')
      // 附带字节指纹——与 /documents 协议的 revision 同源（hashFile），
  ...
  // 写 .md 全文
  // 本端点原在 handler 里内联 readJson + as 断言——现读体与 content/
  ...
  defineRoute('books.file.put', {
    ...
    gate: ({ params, req, res }) => {
      ...
      const file = q.get('file') ?? ''
      // 路径寻址 PUT 不放行 写作/正文——正文保存必须走 /documents/:docId/content
      // （乐观锁 + journal + 快照协议），否则编辑器并发保存被静默覆写、树状态失真
      // writablePath 同次解析带回 rel（快照留底按 bookRoot 相对路径寻址）
    ...
    // RC：全文写走内容档上限（设定/大纲等大 md 同属正文类写入面）
    ...
    handler: async ({ params, input, gate }, _req: IncomingMessage, res: ServerResponse) => {
      ...
      // 可选乐观锁——expectedRevision 与盘上字节指纹不符 → 409。缺省时保持
      // 旧「后写为准」语义（存量调用方零改动）；成功响应回新指纹供客户端滚动基线。
      // 乐观锁挡的是「基线不符」，挡不住两标签页同 expectedRevision
...
/** 等待某书根前缀下全部在途 PUT /file 串行链排空——改名/删书前
...
/** 测试观测钩子（对齐 stream.ts __getSseConnections 风格）——当前在途 PUT 链
...
/** 写后回指纹——对写入内容直接哈希（盘上即该内容，语义与 hashFile 相同且免二次读盘）。 */
...
function editablePath(bookRoot: string, file: string): string | null {
  ...
  // 作者确认位：细纲.md（推进声明）与 账本推进.md（实际履历行）放行编辑器读写
...
function writablePath(bookRoot: string, file: string): { rel: string; abs: string } | null {
  const safe = resolveWithinRoot(bookRoot, file)
  if (!safe) return null
  // .md 判定收敛 isMdFileName（大小写不敏感，与 editablePath 同批）
  if (!isMdFileName(file) || basename(file).startsWith('._')) return null
...
/** PUT /file 的布线锁键——与 DocumentService.wiringFileLockKey /
