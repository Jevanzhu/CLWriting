...
/** 收尾：__setLearnTtlForTest / __setLearnCommitYieldForTest 删除——
 * TTL 覆盖档与让出原语桩改组装根 RouteOverrides 注入（handler 逐调用传入，随实例
 * 隔离；undefined = 生产口径 defaultCommitYield / 5s 逐位不变）。 */

/** 缓存壳收编 ttl-cache.ts 通用件（原本地 Map + FIFO +
 * 过期逐出本地壳删除；命中/失效时序/逐出序逐位不变——纯 TTL + FIFO 32；
 * 「只缓存成功结果」收编 storeIf（原口径：失败多为输入问题，重试应现算），
 * 见 ttl-cache.ts 头部收敛映射表）。
 * 收尾：导出即观测面（生产对象，先例同 analysisOverviewCache）。 */
export const learnCache = createTtlProbeCache<string, LearnResult>({
  name: 'learn',
  keyOf: (k) => k,
  max: LEARN_CACHE_MAX,
  ttl: () => LEARN_CACHE_TTL,
