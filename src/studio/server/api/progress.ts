 ...
 *
 * 从 overview.ts 提取（书架充实：books 端点补摘要需复用，避免 DRY）。
 * 删除三个零引用导出（computeLastEdited / computeLatestChapter /
 * finalizedFiles——独立扫描版早被 computeBookSummary 取代，已 grep 复核零引用）。
 * api 层全书扫描的异步让出原语与 async 孪生落此共享（overview.ts
...
// 让出原语精简批单源化至 src/async.ts，此处 re-export 保既有 import 面。
export { yieldToEventLoop }
/** 逐块让出粒度——每处理 25 章/条让出一次（与 check/run.ts TREE_ISSUES_YIELD_EVERY 同口径）。 */
...
/**
 * computeProgress 的 async 孪生（overview 端点改走此路径）。
...
export async function computeProgressAsync(
  bookRoot: string,
  /** 可选复用调用方已扫出的章列表（overview 单趟三投影共享）——
 ...
 * 原让出点在 mtime 扫描循环（每 25 章），单轮化后该循环消失，单次让出
 * 维持「端点 handler 不是无让出的整段同步链」性质（心跳
...
export async function computeBookSummaryAsync(bookRoot: string): Promise<BookSummary> {
  // 壳体收编 ttl-cache.ts 通用件（缓存命中口径与同步版
  // 一致：共壳共 Map；过期逐出移入通用件，时序逐位不变）
  return summaryCache.get(bookRoot)
}

/** 保存成功后失效该书摘要（书架卡即时反映新字数，不等 TTL）。 */
...
/**
 * computeBookSummaryUncached 的 async 孪生。win 单轮化（readChapterDirSummary）
