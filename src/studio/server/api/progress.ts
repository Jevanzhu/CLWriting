 ...
 *
 * - computeProgress：长篇=正文章数+字数；短篇=写作/正文/ 目录数。
...
/** 定稿文件列表（长篇 写作/正文，短篇 写作/正文/）。读取失败返回空（调用方降级）。 */
...
export function computeLatestChapter(bookRoot: string, kind: 'long' | 'short'): string | null {
  const items =
    kind === 'short'
      ? readPieceDir(join(bookRoot, '写作', '正文')).pieces
      : readChapterDir(join(bookRoot, '写作', '正文')).chapters
