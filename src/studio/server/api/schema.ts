 ...
 *    R0916-7-P3-13（源码质量评审）起另有 gate（parse 前置闸）与 bodyLimit 两个声明位，
 *    迁移清单与「留而不迁」的理由见 providers/style/files/books/snapshots/settings/stream
 *    各端点处的 R0916-7-P3-13 注；
...
interface RouteSchema<I = unknown, G = undefined> {
  ...
  path: string
  /**
   * R0916-7-P3-13：parse 的前置闸——**在 parse 读体之前**执行，语义与 parse 一致
   ...
   */
  parse?: (raw: unknown) => I
  /** R0916-7-P3-13：parse 读体的字节上限（缺省 readJson 的 1MB 档）——正文类大 body
 ...
 *  R0916-7-P3-13：存量测试面（test/state、test/studio 多处直调 handler）按
 ...
 * 执行序（R0916-7-P3-13 起四段）：gate（前置闸，闸内可自行回错误并中止）→ parse
...
export function defineRoute<I, G = undefined>(name: string, schema: RouteSchema<I, G>): RouteSchema<I, G> {
  ...
  route(schema.method, schema.path, async (req, res, params) => {
    ...
    try {
      ...
      // R0916-7-P3-13：闸内登记资源的唯一释放点——parse 失败 / handler 早退 / handler
