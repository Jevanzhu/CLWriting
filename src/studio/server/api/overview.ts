 ...
 * 起异步，调用方 await；短时缓存 5s，且起只有成功结果落缓存）。失败不崩
 *（返 state:0 + 错误，不落缓存）。
 *
...
// state 判定结果短时缓存。detectState 内部全量 rebuild index.db（clearAllTables 清空重建），
// overview 每次请求都触发会慢（大书几百 ms~秒级）。概览页 stale 5s 可接受；精确态走 /state 或 enter。
// 原单条目缓存多书交替访问永 miss（观察项）——改多书 Map（key=bookRoot）+ FIFO 上限。
// 只有成功路径落缓存——此前 catch 降级态（state:0 + error）同样
...
/** 收尾：__setOverviewCacheTtlForTest / __overviewScanCountForTest（含
 * reset）删除——TTL 覆盖档改组装根 RouteOverrides 注入（handler 逐调用传入，随实例
 ...
 * 整包缓存同挂本函数（books.ts forgetBookKeyedCaches 家族零新挂点）。 */
 ...
 * 过期逐出本地壳删除；命中/失效时序/逐出序逐位不变——单级探针 + FIFO 32，
 * 见 ttl-cache.ts 头部收敛映射表；计算体闭包 ctx/entry，经 get(key, compute) 逐调用
 * 传入；state 段降级/book.yaml 缺失不落缓存由 storeIf 承担）。
 * 收尾：导出即观测面（stats.misses 取代 __overviewScanCountForTest）。 */
export const overviewCache = createTtlProbeCache<string, OverviewCompute>({
  name: 'overview',
  keyOf: (k) => k,
  max: OVERVIEW_CACHE_MAX,
  ttl: () => OVERVIEW_CACHE_TTL_MS,
...
function listVolumes(bookRoot: string): { name: string; path: string }[] {
  ...
  try {
    for (const f of readdirSync(dir)) {
      // .md 判定收敛 isMdFileName（大小写不敏感），剥尾正则同步加 /i
 ...
 * 低级项：只统计已定稿章——写作中的草稿保存也会刷 mtime，原先被计入
...
async function computeTimeline(bookRoot: string, chapters: ChapterMeta[]): Promise<{ date: string; count: number }[]> {
  ...
  // 精确匹配失配）；posix 恒等
  const finalizedKeys = finalized === null ? new Set<string>() : new Set([...finalized].map(docJoinKey)) // 升 docJoinKey（+NFC 归一）
