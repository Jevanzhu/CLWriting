...
export function registerOverviewRoutes(ctx: OverviewCtx): void {
  defineRoute('books.overview', {
    ...
    // 全书遍历（statSync 逐章），大书单请求冻结事件循环。R47-7（四十七轮）：缓存壳
    // 就位（见上）+ recentDoc 并入 timeline 同一次 readChapterDir（原 getRecentDoc
    // 为取章号最大章独立再扫一遍正文目录——三扫收敛为二扫，readChapterDir 本身还有
    // stat 级目录缓存）。
    const { timeline, recentDoc } = await computeTimelineAndRecent(bookRoot)
 ...
 */
/** R47-7（四十七轮）：timeline + recentDoc 单扫合并——同一份 readChapterDir 章列表
 *  既走定稿 mtime 聚合又取章号最大章（原 getRecentDoc 独立再扫正文目录）。语义与
 *  两函数各自原实现逐位一致（recentDoc 排序取最大有 _path 章）。 */
async function computeTimelineAndRecent(
  bookRoot: string,
): Promise<{
  ...
  // R47-7：recentDoc 同列表取章号最大章（原 getRecentDoc 逻辑原样收编——含防御口径：
  // 最大章号章无 _path → null，不回落次大章）
  let maxCh: (typeof chapters)[number] | undefined
  for (const c of chapters) {
    if (c._path) files.push(c._path)
    if (maxCh === undefined || (c.章号 ?? 0) > (maxCh.章号 ?? 0)) maxCh = c
  }
  const recent: { no: number; 标题: string; path: string } | null =
    maxCh && maxCh._path
      ? { no: maxCh.章号, 标题: maxCh.标题, path: relative(bookRoot, maxCh._path).replace(/\\/g, '/') }
      : null
