 ...
 * R0916-5h（2026-09-16，⑤④产品巨件拆分波4）：路由段按域拆出三个兄弟文件——
 * documents-save.ts（缝 1：W1 保存/文件树/定稿确认/批量定稿）、documents-crud.ts
 ...
 * ——R0916-5a 收敛的公共底座）内容逐字节零触碰，单源迁 documents-core.ts。
 * 装配实读记档：路由经 defineRoute 副作用注册进当前活动路由表（schema.ts
 * WeakMap 按表隔离），无路由表数据导出面；registerDocumentRoutes 是 server/
 * index.ts 与 r1010b-srv-documents-bookmoved.test.ts 的既有消费名（消费面零
...
// R0916-5h：外部消费名桥——基建段单源迁 documents-core.ts 后逐名再导出（消费方
...
export function registerDocumentRoutes(ctx: DocumentCtx): void {
  // R0916-5h：路由段已按域拆出（见头注拆分记档）；本函数保留为聚合入口（外部
