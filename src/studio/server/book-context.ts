/**
 * 书级上下文读取（架构下沉：readKind 已下沉 src/format/kind.ts）。
...
/** resolveBook 双行样板单源（SRV-·专项精简优化 §五，机械批收编路由入口
...
/** TOCTOU 读稿失败人话文案单源（原 analysis.ts:470/551/633、review.ts:145/324 逐字同文）。 */
export const DRAFT_UNREADABLE_TEXT = '读不到正文文件（可能已被移动或删除），请刷新后再试'

/** 解析失败结构（status/code/message 直连 replyError 三参；message 已是人话）。 */
...
/**
 * docId → 正文解析链全链单源（清单 → 安全路径 → 存在性 → 单次读取（TOCTOU
 * 500 守卫）→ 章稿解析）。（analyze）/ （autotag/infer-meta）当年各自
