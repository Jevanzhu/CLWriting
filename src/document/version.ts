 ...
 *  PM-6（性能与内存专项·2026-09-05）：版本面板打开一次要对每个版本 readVersion
 *  全量整读（2MB 章 × 数十版本 = 数十 MB IO + 等量字符串物化），而 entry 只需要
 *  id/time/origin/reason/words/pinned 六个头部字段——words 在 meta 时（写侧 PM-6
 *  补齐后全覆盖；存量旧版本 fm 缺 字数 行）走 readVersionMeta 的 64KB bounded 头读，
 *  全量读只留给无字数的存量版本兜底。头部读失败（损坏/截断）与旧实现同款回落
 *  readVersion 抢救一次，仍失败跳过（口径不变）。 */
export function listVersionEntries(
  ...
  for (const s of listVersions(versionsDir, docId)) {
    const head = readVersionMeta(versionsDir, docId, s.id)
    if (head !== null && head.meta.words !== undefined) {
      out.push({
        id: s.id,
        time: head.meta.time,
        origin: head.meta.origin,
        reason: head.meta.reason ?? '',
        words: head.meta.words,
        pinned: head.meta.pinned ?? false,
      })
      continue
    }
