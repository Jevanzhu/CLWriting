/**
 * DocContext —— 文档层共享设施的显式上下文（R0916-7-P3-8，源码质量评审 P3-8）。
 *
 ...
 * 都能改类内部状态，封装形同虚设，且读代码要在两个文件间来回跳（评审 P3-8 原文）。
 ...
 *   的文件名编码口径 R68-3 在此单源）——各操作不得再自行 join，编码漂移会让同一 docId
...
export class DocContext {
   ...
   *  `<文件绝对路径>.lock`，与保存链按 journal 命名的 save 锁互不感知——R26-6 注释宣称
   ...
   *  R30-6（等待异步化）的机制本体自该批起单源此处，动机沿革见各调用面注释。
   *  R0916-7-P3-8：编排随共享设施自 DocumentService 迁入本对象——本方法不再依赖类实例，
  ...
  /** PM-4：字数缓存读取（命中返回该 revision 下的正文字数，未命中/换版返回 undefined）。 */
  ...
  /** R31-20（三十一轮）：同 docId meta 操作串行链——锁等待让出事件循环后，同文档第二
