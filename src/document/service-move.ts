/**
 * 结构性操作 move/rename —— R0916-7-P3-8（2026-09-25，源码质量评审 P3-8）自 service.ts 拆出。
 *
 ...
 * 编译期擦除，运行时零回边）；共享设施一律经 `ctx: DocContext` 显式取得（R0916-7-P3-8
 * 起不再有类实例字段/方法可摸）。
 *
 ...
 *  R0916-7-P3-8：类方法迁模块函数，共享设施经 ctx 显式取得（本体原样，类内部成员访问改走 ctx）。 */
...
export async function doMoveOrRename(
  ...
  try {
    ...
    // R46-39（四十六轮）：留底补传 policy（ctx.snapshotPolicy()）——此前缺省走
    ...
    writeVersion(ctx.snapshotsDir, docId, oldContent, {
      origin: 'manual',
