 ...
 * 上层 document 的环风险边）；守卫与路径定位强耦合（契约：写路径
 * resolveDraftPath 抛 = 拒绝覆盖定稿，migrate-layout-v3 与 4 处钉测试依赖），
...
// 正文目录卷扫描 readdirSync 容错降级留痕（同 run.ts/runner.ts 口径）
 ...
 * - opts.forRead：只读定位口径——跳过定稿覆盖守卫。对话上下文注入 /
...
export function resolveDraftPath(
  ...
  // 1. 已有同章号 → 覆盖（已定稿章除外——覆盖定稿 = 静默摧毁作者已确认内容）
  ...
  // 补零走 chapterFilePrefix 单源（长篇 4 位）——原 3 位与 service 改名
  // （4 位）写侧分裂，靠读侧 chapterNamePrefixes 三口径兜底；统一后新章与改名同口径，
  // 存量 3 位文件读侧仍全口径兼容
  // 标题净化收口到 sanitizeChapterTitle（对齐导出侧口径：
...
/** 目标章已定稿（manifest finalizedRevision 基线在位）→ 拒绝覆盖写。
...
function ensureChapterNotFinalized(bookRoot: string, relPath: string, chapter?: number): void {
  ...
  for (const e of manifest.entries.values()) {
    ...
    if (chapter !== undefined) {
      // 章号数值匹配限正文路径——finalizedRevision 是全文档通用
...
export function inferVolumeDir(bookRoot: string, chapter: number): string {
  ...
  if (existsSync(bodyDir)) {
    ...
    // 口径）或路径被文件占用（ENOTDIR——existsSync 对文件同为 true）时 ENOENT/
...
/** 卷号中文数字/阿拉伯 → 数值（卷目录排序用；一~九十九覆盖现实卷数，
