...
/** 清单指纹缓存测试钩子（生产零调用；口径同 rebuild.ts __testHooks）。 */
...
/** 读清单 + 降级显式化——readManifest 的「读失败→空清单」静默降级
...
export function writeManifest(filePath: string, manifest: Manifest): void {
  try {
    if (existsSync(filePath)) {
      ...
      // .bak 影子写删 `{ fsync: false }`（回 atomicWriteFile 默认
      // fsync:true）——.bak 是主清单损坏后的恢复源，掉电撕裂恰在兜底场景失守（
      // 落地时按「高频低价值写」口径关了 fsync，但 .bak 低频〔随主写一次〕高价值〔唯一
      // 恢复源〕，该口径在此不成立）。
      atomicWriteFile(`${filePath}.bak`, previous)
    }
  } catch {
    /* .bak 影子失败不阻断主写（best-effort） */
  }
  ...
  // 写后清指纹缓存（双保险——写必 bump mtime，指纹本会自然失配）
...
/**
 * 清单锁等待超时（毫秒）。
 * 2s → 5s，对齐 ai-calls 口径（AI_CALLS_LOCK_TIMEOUT_MS=5s）——
 * 超时降级裸写后，双进程同时降级时后写者会吞掉先写者的清单更新（正是要防的
...
// 异步重入自等死锁防御——withManifestLockAsync 的持锁执行体（fresh
 ...
 *
 * 锁超时**不再降级裸写**，改 fail-closed——原「超时降级 + warn
 * 留痕」在双进程同时降级时后写者整文件覆盖先写者，finalizedRevision/清单条目丢行
 * （正是要防的事故在超时窗口复现；清单整文件重写不是 append-only，journal 那套
...
/**
 * 清单 RMW 互斥段的异步孪生——等待期改 setTimeout 轮询（事件循环
...
export async function withManifestLockAsync<T>(manifestPath: string, fn: () => T | Promise<T>): Promise<T> {
  ...
  if (held) {
    ...
    if (isAsyncFunction(fn)) {
      // async 同 key 重入 = 排队自等死锁（本调用会挂到含自身调用方
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    if (release) {
      ...
      // 2-：临界段首节——fn 的执行 promise 即重入排队链（tail）的头节；
      ...
      try {
        ...
        // 2-：释放前循环排空重入排队链——排队中的 async fn 仍是本进程临界
