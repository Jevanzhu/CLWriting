...
export async function updateChapterMetaLocked(
  ctx: DocContext,
  docId: string,
  ...
  return ctx.withSaveLocks<MoveResult>({
    ...
    onSaveLockThrown: (e) => ({
      ok: false,
      code: 'WRITE_ERROR',
      reason: `元数据保存锁获取失败（未执行保存，可重试）：${errMsg(e)}`,
    }),
    onSaveLockTimeout: () => ({
      ok: false,
      code: 'WRITE_ERROR',
      reason: '元数据保存等待超时：另一进程正在保存此文档（5 秒未让出），请重试',
    }),
    wiring: {
      relPath: path,
      ...
      onTimeout: () => ({
        ok: false,
        code: 'WRITE_ERROR',
        reason: '元数据保存等待超时：另一进程正在回写此布线文件（5 秒未让出），请重试',
      }),
    },
    body: async () => {
      ...
      try {
        ...
        writeVersion(
          ...
          fileBytes,
          {
            origin: 'meta-overwrite',
            reason: '章节元数据修改前留底（R26-51）',
            words: countWords(bodyOf(fileBytes.toString('utf-8'))),
          },
      ...
      const no = normalizeChapterNo(map.get('章号'))
      const safeTitle = sanitizeChapterTitle(标题) || '未命名'
      const newName = no !== null ? `${chapterFilePrefix(no, 'chapter')}${safeTitle}.md` : basename(path)
      if (basename(path) !== newName) {
...
async function syncRenamePieceList(ctx: DocContext, oldBodyRel: string, newName: string): Promise<void> {
  ...
  if (existsSync(ctx.manifestPath)) {
    ...
    try {
      ...
      log.warn(
        'document',
        `章纲清单读失败（strict），章纲同步重命名跳过（${oldListRel} 滞留旧名，登记与盘上文件保持一致）：${errMsg(e)}`,
      )
      return
    }
  ...
  try {
    ...
    if (placed === 'exists') {
      ...
      log.warn(
        'document',
        `章纲同步重命名未落位（目标 ${newListRel} 被持续占用，正文已改名，章纲滞留旧名 ${oldListRel}）`,
      )
      return
    }
    try {
...
export async function updateDocMetaLocked(
  ctx: DocContext,
  docId: string,
