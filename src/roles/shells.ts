...
function renderClaudeSkill(roles: RoleDefinition[]): string {
  return [
    ...
    '2. Draft the outline with the outline role. The outline must start with front matter such as `章号`, `标题`, `钩子类型`, `钩子强弱`, `情绪定位`, `场景`, and for long-form books `推进` (e.g. `推进: [成长线-001, 设定线-002]`) so check can verify both-ends closure. Then run `clwriting confirm <chapter>` after author approval, and record the outline call with `clwriting record-call <chapter> --step outline`.',
    '3. Run `clwriting prepare` to generate `工作区/本章写作材料.md`; use that bounded context and its style samples before drafting.',
    '4. Draft prose with the writer role; write long-form drafts to `工作区/草稿-1.md` with front matter (`章号`, `标题`, `钩子类型`, `钩子强弱`, `情绪定位`) before the prose. After drafting, record the draft call(s) with `clwriting record-call <chapter> --step draft` (pass `--calls N` for best-of-N drafts, `--tokens N` if the host can report usage). For long-form books, when a chapter opens a new ledger line, first create its file under `大纲/<类>/<编号>-<标题>.md` (front matter + empty 履历); then declare the actual ledger advances of this chapter in `工作区/账本推进.md` (one line each: `- <编号> <动词>：<evidence sentence from the prose>`), which finalize reads to write 履历 into the ledger. Review calls are accounted automatically by `review collect`; do not record them here.',
