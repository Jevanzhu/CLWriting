...
function renderClaudeSkill(roles: RoleDefinition[]): string {
  return [
    ...
    '4. Draft prose with the writer role; after drafting, record the draft call(s) with `clwriting record-call <chapter> --step draft` (pass `--calls N` for best-of-N drafts, `--tokens N` if the host can report usage). Review calls are accounted automatically by `review collect`; do not record them here.',
