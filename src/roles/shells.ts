...
function renderClaudeSkill(roles: RoleDefinition[]): string {
  return [
    ...
    '# CLWriting Write',
    '',
    'Use this skill to run the CLWriting chapter/piece workflow.',
    '',
    '## SessionStart',
    '',
    ...
    '2. Draft the outline in the host session. For long-form books, the outline front matter uses `章号`, `标题`, `钩子类型`, `钩子强弱`, `情绪定位`, `场景`, plus `推进` when ledgers advance (e.g. `推进: [成长线-001, 设定线-002]`). For short collections, use `篇号`, `标题`, `目标情绪`, `核心反转`, `场景`, and maintain `工作区/清单.md` with reversal clues and payoff items. Then run `clwriting confirm <chapter|piece>` after author approval, and record the outline call with `clwriting record-call <chapter|piece> --step outline`.',
    '3. Run `clwriting prepare` to generate `工作区/本章写作材料.md`; use that bounded context and its style samples before drafting. Short collections may still use the same materials file name; interpret it as current-piece materials.',
    '4. Draft prose with the writer role; write drafts to `工作区/草稿-1.md` (the `1` is the candidate index, not the chapter/piece number). Long-form drafts use front matter (`章号`, `标题`, `钩子类型`, `钩子强弱`, `情绪定位`); short drafts use (`篇号`, `标题`, `目标情绪`, `核心反转`) and must keep `清单.md` in sync. After drafting, record the draft call(s) with `clwriting record-call <chapter|piece> --step draft` (pass `--calls N` for best-of-N drafts, `--tokens N` if the host can report usage). For long-form books, when a chapter opens a new ledger line, first create its file under `大纲/<类>/<编号>-<标题>.md` (front matter + empty 履历); then declare the actual ledger advances of this chapter in `工作区/账本推进.md` (one line each: `- <编号> <动词>：<evidence sentence from the prose>`), which finalize reads to write 履历 into the ledger. Review calls are accounted automatically by `review collect`; do not record them here.',
