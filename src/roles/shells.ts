...
function renderClaudeSkill(roles: RoleDefinition[]): string {
  return [
    ...
    '2. Draft the outline with the outline role. Declare the chapter scene(s) in the outline front matter (e.g. `场景: 对话`, or multiple `场景: [战斗, 对话]`, matching the `文风/样章库/` scene names) so prepared style samples fit the chapter. For long-form books, also declare the ledger lines this chapter advances in the outline front matter (e.g. `推进: [成长线-001, 设定线-002]`) so check can verify both-ends closure. Then run `clwriting confirm <chapter>` after author approval, and record the outline call with `clwriting record-call <chapter> --step outline`.',
