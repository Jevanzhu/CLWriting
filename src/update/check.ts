/**
 * 阶段 53 S1：更新检查客户端（静默、进程内一次、失败不抛）。
 *
 * 口径六面（设计 §三）：版本源见 `resolveAppVersion`；只提示**正式版**（D1，筛选在
 * pickLatestStable）；启动后延迟一次、进程内一次、不落盘（D2）；失败/离线/被墙/限速
...
async function fetchOutcome(fetchImpl: typeof fetch, timeoutMs: number): Promise<FetchOutcome> {
  ...
  try {
    ...
    const tags = data
      ...
      // draft 匿名 list 本不含，防御性保留；prerelease 标记不参与筛选（D5：rc 不标
