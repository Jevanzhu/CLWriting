...
/** 终止 session 下全部在途 ctrl（interrupt/dispose 共用）——owner 分槽后
...
function push(id: string, ev: DriverEvent): void {
  ...
  if (ch.consumers.size === 0) {
    ...
    if (!ch.preTaken) {
      ch.pre.push(ev)
      // pre cap——超过只留最近 N 个（首个消费者只接管最近 N 个）
  ...
  for (const c of ch.consumers) {
    // 内存核查：消费者队列 cap——广播腿是 pre/execRing 之外的
    // 一支（原先无上限），超限丢最旧腾位；每轮积压首次超限时补发 notice。
    // notice 走「容量 +1 内部槽」——修复前首次溢出先 shift 腾位、
...
export const ccDriver: StudioDriver = {
  ...
  // stream 改工厂形态（生成器主体不变）——创建时在 WeakMap 登记取消句柄，
  ...
  stream(session: Session): AsyncGenerator<DriverEvent> {
    ...
    const gen = (async function* (): AsyncGenerator<DriverEvent> {
      // 低级项：已 dispose 的会话不再建 channel（原先懒建复活 Map 条目无人清）
      ...
      try {
        while (!session.closed) {
          ...
          if (session.closed) return
          // 断开唤醒后的检查点——不再续 park，自行 return（finally 摘除 consumer）
  ...
  interrupt(session: Session): void {
    ...
    sessionCtrls.delete(session.id)
    // 低级项：不再 channel(id) 懒建——push 已对已删 channel 短路，防复活
  ...
  registerCtrl(session: Session, ctrl: AbortController, owner?: string): void {
    // 已 dispose 会话拒收登记——此前懒建 byOwner 会把 dispose 刚删的
    ...
    // 同编排换新 ctrl 时先 abort 旧的（防并发时前者变不可中断僵尸）
    if (old && !old.ctrl.signal.aborted) old.ctrl.abort()
    byOwner.set(own, { ctrl, owner: own })
  },

  // 生成终态注销——isRunning 立即归 false（此前 done 后仍登记，SSE 快照假报「生成中」，
  ...
  isRunning(session: Session): boolean {
    ...
    // aborted 的 ctrl 不算在途（编排层直接 abort 自身 ctrl 而非走 interrupt 的路径兜底）
    for (const slot of byOwner.values()) {
