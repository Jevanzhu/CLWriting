/**
 * 未入账手改对账 —— 依据 ⑱ 手改对账 spec 第 3-4 节（原则 1「文件即真相」核心）。
 *
 * 检测定稿区/账本/大纲的未 commit 手改（作者绕过 finalize 直接编辑），分类、提议补登。
 * 与 finalize（⑬ 写时入账）殊途同归——都让「md 真相」与「缓存/账本」一致。
 *
 * 原则（⑱ 第 1 节）：
...
/** 按文件路径选手改类型（⑱ 第 3 节脚本分类） */
function classifyByPath(path: string): HandEditKind {
  // 已发布正文：定稿/正文/*.md（⑱ 第 4 节，已 commit 的正文只读）
 ...
 * - 账本/大纲 → 提议同步进缓存（重建相关项）
 * - 设定 → 提议同步 + 若被引用触发影响分析（⑰）
 * - 已发布正文 → 警示 + 提议（撤销/回滚/坚持），不强制拒（⑱ 第 4 节）
 */
  ...
  switch (edit.kind) {
    ...
    case 'setting':
      return {
        ...
        needsImpactAnalysis: true, // ⑰ 触发
      }
    case 'published-text':
      // ⑱ 第 4 节：已发布正文手改特殊处理（对作者是提醒不是禁止）
      return {
        edit,
        proposal: '⚠ 这章已发布，按铁律是只读的——读者已经看过，改它会和已读内容不一致。建议：①撤销手改（git 恢复）；②或走「回到第 N 章」正式重发；③或坚持保留（你是作者，你说了算，系统会记一笔供后续对账）。',
        needsImpactAnalysis: false,
      }
    case 'other':
      return {
