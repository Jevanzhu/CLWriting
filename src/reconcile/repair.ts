/**
 * 源文件修复确认 —— 依据 #18 手改对账 spec 第 2 节（补 #1 M1 占位）。
 *
...
/** 修复建议（#18 第 2 节，M3 出结构化建议 + 作者手动修通道） */
...
/**
 * 全量扫描源文件解析错误（#18 第 2 节）。
 * 复用 rebuild 收错（幂等，#4 重建器）；坏文件计入 errors 不中断。
 */
...
function analyze(error: ParseError): { kind: RepairSuggestionKind; proposal: string } {
  ...
  // 枚举值笔误：状态值不是 进行中/已收尾/已放弃（#3 第 5 节）
