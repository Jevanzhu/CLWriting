...
// R0912-3（2026-09-12 全量重评 #49）：本判式为两脚本 argValue 的严格口径——值缺失/
// 空串/以 `--` 开头（误吞下一个 flag 名）一律按缺参返回 null。（六轮重评 D101 勘误：
// 原注「calibrate-tokens.ts 宽松直取、待另行对齐」已失实——该处早已同口径对齐，
// 现两脚本判式等价，勿据旧注再去「修」对侧。）
