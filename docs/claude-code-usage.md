...
这是你日常写作的主入口。当前插件定义 13 个 Claude Code commands：
...
|---|---|
| `/story-init` | 创建或刷新项目 |
| `/story-preflight` | 写作前检查，并给出下一步建议 |
| `/story-review` | 审查章节，支持 full/lean/solo |
| `/story-repair` | 按审查结果修复，修完强制复审 |
| `/story-deslop` | 6-Gate 去 AI 味检查 |
| `/story-query` | 只读查询状态、记忆、上下文、经验 |
| `/story-learn` | 沉淀可复用写作经验 |
| `/story-import` | 导入外部作品或素材 |

**长篇专用**：`/story-long-plan`（规划卷章与大纲）、`/story-long-write`（写章）、`/story-long-analyze`（结构分析）、`/story-long-scan`（一致性扫描）

**短篇专用**：`/story-short-write`（写章）

短篇没有独立的分析/扫描命令；对应能力由 Skill `story-short-analyze`、`story-short-scan` 承担（它们没有 command 文件，不能直接斜杠调用）。短篇做质量或一致性复盘，请用 `/story-review` 和 `/story-deslop`。

Python CLI 另有 20 个 CLI 顶层子命令，是这些命令背后的底层工具层，日常写作用不到。CLI、commands 和 Skills 是三套清单，不要求一一映射。
...
- `/story-long-analyze`：长篇只读分析，不生成新剧情事实；短篇用 `/story-review` 做质量复盘。
- `/story-long-scan`：长篇扫描占位符、一致性或 AI 味，不直接修复正文；短篇用 `/story-deslop` 查 AI 味。
