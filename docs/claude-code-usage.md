...
- `/story-write N`：写第 N 章，调用 Agent 并在通过后验收章节。
...
CLI 不自动调用 Agent，只负责确定性校验、报告、修复计划、兜底抽取和验收记录。
...
- **字数闸门**：低于规划字数 60% 阻断验收，低于 80% 或超出 135% 发出警告。使用 `--strict-warnings` 可把警告也视为阻断。
- **审查闸门**：reviewer 发现 blocking issue 时章节记录为 rejected，不写入最终 `正文/`，不更新记忆。
