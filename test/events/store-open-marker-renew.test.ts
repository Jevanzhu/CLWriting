...
describe('R71-24 开口标记 pid 复用防护', () => {
  it('活 pid + 标记超龄（>10min 无续期）→ 视同死残留 GC，迁移放行', async () => {
    const ud = freshUd()
    ...
    expect(await migrateBookSession(ud, oldRoot, newRoot, '复用书', '复用书新')).toBe(true)
    expect(existsSync(marker)).toBe(false) // 超龄残留被 GC
    expect(existsSync(oldDb)).toBe(false)
  })

  it('活 pid + 标记新鲜 → 仍是活标记拦迁（R67-2 语义不回退）', async () => {
    const ud = freshUd()
    ...
    expect(await migrateBookSession(ud, oldRoot, newRoot, '活标书', '活标书新')).toBe(false)
...
describe('R71-25 墓碑前置与回滚撤碑', () => {
  it('迁移成功：墓碑在位指向新库（前置落位，成功路径不落文件操作）', async () => {
    const ud = freshUd()
    ...
    expect(await migrateBookSession(ud, oldRoot, newRoot, '墓甲', '墓乙')).toBe(true)
    expect(existsSync(oldDb + '.migrated')).toBe(true)
    expect(existsSync(newDb)).toBe(true)
    expect(existsSync(oldDb)).toBe(false)
  })

  it('改钥匙阶段失败 → 回滚：源库回旧位完整，预写墓碑被撤', async () => {
    const ud = freshUd()
    ...
    expect(await migrateBookSession(ud, oldRoot, newRoot, '败甲', '败乙')).toBe(false)
