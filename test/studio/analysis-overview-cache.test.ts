...
describe('R36-7 analysis-overview 缓存', () => {
  it('连续两次请求：第二次缓存命中不重算（scan 计数不变），结果一致', async () => {
    const { root } = makeBook()
    const r1 = await getAnalysisOverviewCached(root, 60_000) // 长档：慢机下不自然过期
  ...
  it('新增信封（新章分析落盘）：TTL 窗内节流命中；TTL 到期重探失效，趋势增多；forget 显式失效同效', async () => {
    ...
    await getAnalysisOverviewCached(root, 60_000)
  ...
  it('TTL 到期重算：探针无变化也按超期重算（注入 TTL=0）', async () => {
    ...
    expect(analysisOverviewCache.stats().misses).toBe(1)
    await getAnalysisOverviewCached(root, 0) // 即刻过期
    expect(analysisOverviewCache.stats().misses).toBe(2)
    // 长档覆盖后再调命中缓存（TTL 重新计）
    await getAnalysisOverviewCached(root, 60_000)
    expect(analysisOverviewCache.stats().misses).toBe(2)
