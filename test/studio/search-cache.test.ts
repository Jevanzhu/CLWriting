...
function makeTree(): string {
  root = mkdtempTracked(join(tmpdir(), 'search-cache-'))
...
describe('R35-7 searchBookCached 缓存与去重', () => {
  it('并发同参数只跑一次底层扫描，全部调用拿到同结果', async () => {
    ...
    expect(searchCache.stats().misses).toBe(1)
  ...
  it('TTL 内重复查询命中；目录结构变化（新增文件）即时失效重扫；forgetSearchCache 同效', async () => {
    makeTree()
    const first = await searchBookCached(root, '烛火', undefined, 60_000) // 长档：慢机下也不会自然过期
    expect(searchCache.stats().misses).toBe(1)
    const second = await searchBookCached(root, '烛火', undefined, 60_000)
    expect(searchCache.stats().misses).toBe(1) // 无写入：TTL 内命中，未重扫
    ...
    const third = await searchBookCached(root, '烛火', undefined, 60_000)
    expect(searchCache.stats().misses).toBe(2)
    expect(third.results.map((h) => h.path)).toContain('写作/正文/0002-晨光.md')
    forgetSearchCache(root) // 删书/改名挂点同款失效
    await searchBookCached(root, '烛火', undefined, 60_000)
    expect(searchCache.stats().misses).toBe(3)
  })

  it('TTL 到期重扫：盘上变更可见', async () => {
    makeTree()
    await searchBookCached(root, '烛火', undefined, 0) // 即刻过期
    writeFileSync(join(root, '写作', '正文', '0002-晨光.md'), '烛火熄了。\n', 'utf-8')
    const r = await searchBookCached(root, '烛火', undefined, 0)
    expect(searchCache.stats().misses).toBe(2)
...
beforeAll(async () => {
  httpWorkDir = mkdtempSync(join(tmpdir(), 'search-cache-http-'))
