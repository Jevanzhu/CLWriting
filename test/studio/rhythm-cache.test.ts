 ...
 *
 * 断言用「全量重算计数」观测口（rhythmCache.stats().misses），确定性不依赖墙钟 5s
 *（先例 search-cache / version-stats-cache）。
 */
...
function makeLongBook(): string {
  const root = mkdtempTracked(join(tmpdir(), 'rhythm-cache-'))
  roots.push(root)
...
describe('R44-8 rhythm 缓存壳', () => {
  it('TTL 内二次调用命中不重扫（scan 计数不变），结果一致', () => {
    ...
    const r1 = getRhythmCached(root, 60_000) as { kind: string; written: { count: number }; planned: { count: number } }
    ...
    const r2 = getRhythmCached(root, 60_000)
    expect(rhythmCache.stats().misses).toBe(1) // 命中：未重扫
    expect(r2).toEqual(r1)
  })

  it('目录变化（指纹变）后重算：新增正文章节 written.count 反映新章', async () => {
    const root = makeLongBook()
    getRhythmCached(root, 60_000)
    expect(rhythmCache.stats().misses).toBe(1)
    ...
    const r2 = getRhythmCached(root, 60_000) as { written: { count: number }; chapterDiff: unknown[] }
    expect(rhythmCache.stats().misses).toBe(2) // 写作/正文 mtime 变 → 重扫
  ...
  it('book.yaml 内容变化（kind 翻转）→ 文件 stat 指纹失配 → 重算见新形状', async () => {
    const root = makeLongBook()
    const before = getRhythmCached(root, 60_000) as { kind: string }
  ...
  it('TTL 到期重扫；forget 显式失效同效', () => {
    ...
    getRhythmCached(root, 60_000)
    expect(rhythmCache.stats().misses).toBe(2)
    getRhythmCached(root, 0) // 即刻过期
    expect(rhythmCache.stats().misses).toBe(3)
  })
})
...
describe('P3-2 rhythm async 孪生（双轨）', () => {
  it('async 主路 MISS 计数入同壳，结果与同步版逐位一致；同步版命中 async 已算结果', async () => {
    ...
    const s = getRhythmCached(root, 60_000) as unknown
    expect(rhythmCache.stats().misses).toBe(1) // 同壳共 Map：同步版直接命中，不重扫
    expect(s).toEqual(a)
  })

  it('并发 MISS in-flight 去重：同键并发只算一次', async () => {
    const root = makeLongBook()
    const [x, y] = await Promise.all([getRhythmCachedAsync(root, 60_000), getRhythmCachedAsync(root, 60_000)])
    expect(rhythmCache.stats().misses).toBe(1)
    expect(x).toEqual(y)
  })
})
