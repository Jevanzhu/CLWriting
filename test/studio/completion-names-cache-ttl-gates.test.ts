 ...
 * 文件 R46-16 settings 壳）。断言用 MISS 计数观测口（completionNamesCache.stats()
 * .misses），时钟注入走 vi.useFakeTimers({toFake:['Date']})（先例 r0912-ttl-write
 * -clock：TTL 到期即时推走，不付真实睡眠）；目录 mtime 变化臂写真实文件，跨同毫秒
 * 档的 5ms sleep 为 rhythm-cache 先例手法。无平台门，三平台同跑。
 */
...
import { completionNamesCache } from '../../src/studio/server/api/settings.js'
...
beforeAll(async () => {
  // 只接管 Date（TTL 判定读 Date.now()）；HTTP 服务器/真实 I/O 照常（先例 ttl-window-from-write-time）
  vi.useFakeTimers({ toFake: ['Date'] })
  studio = await bootStudio({
    book: BOOK,
    prefix: 'clw-r0912-ds41-cnames-',
    env: { CLWRITING_DRIVER: 'mock' },
    // completion 壳 TTL 短档经组装根 overrides 注入（原模块级 setter 已删）
    overrides: { completionNamesTtlMs: 1000 },
    files: [
...
describe('R0912-ds41：completion-names 缓存壳', () => {
  ...
  it('MISS → TTL 内 HIT：第二次请求不再扫盘（计数不变），响应一致', async () => {
