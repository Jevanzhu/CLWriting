...
describe('readCharacterCards(P2 结构化读)', () => {
  it('有 front matter:解析 姓名/身份/目标/境界/正文', () => {
    ...
    expect(cards[0]!.file).toBe('设定/角色/林远.md')
  })

  it('无 front matter:降级(姓名=文件名,正文=全文)', () => {
    writeFileSync(join(root, '设定', '角色', '赵衡.md'), '# 赵衡\n自由描述正文。')
    const cards = readCharacterCards(join(root, '设定', '角色'), root)
  ...
  it('缺字段容错(只填姓名,其余空串)', () => {
    writeFileSync(join(root, '设定', '角色', '清虚.md'), '---\n姓名: 清虚\n---\n正文')
    const cards = readCharacterCards(join(root, '设定', '角色'), root)
  ...
  it('目录不存在 → 空数组', () => {
    expect(readCharacterCards(join(root, '设定', '不存在'), root)).toEqual([])
  })
...
describe('buildSettingsContext(RAG 注入)', () => {
  ...
  it('只有角色 → 只角色段(无境界段)', () => {
    writeFileSync(join(root, '设定', '角色', '张三.md'), '---\n姓名: 张三\n---\n正文')
    const ctx = buildSettingsContext(root)
