 ...
 * R0916-7-P3-6 适配：探测函数经组装根 overrides 注入（原 __setProbeCapabilitiesForTest
 * 已删）——委托壳按用例置位，未置位回落真探测（ai/provider/probe.js 同源）。
 *
...
import { probeCapabilities as realProbeCapabilities } from '../../src/ai/provider/probe.js'

let studio: StudioHarness
let baseUrl = ''
let token = ''
let userDataPath = ''
// 探测委托壳的用例内置位：null = 走真探测（等效原 setter 置 null 的还原语义）
let probeFn: ((conf: ProviderConf, userDataPath?: string | null) => Promise<ProbeResult>) | null = null
...
beforeAll(async () => {
  ...
  studio = await bootStudio({
    prefix: 'clw-r75-probe-',
    userDataPath,
    overrides: {
      probeCapabilities: (conf, udp) => (probeFn ?? realProbeCapabilities)(conf, udp),
    },
  })
...
afterAll(async () => {
  probeFn = null // 恢复真探测，防注入泄漏到同进程其它用例
  await studio.close()
