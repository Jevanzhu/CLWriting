 ...
 *
 * 断言用「全量重扫计数」观测口（foreshadowCache.stats().misses），确定性不依赖墙钟 5s
...
describe('R44-8 foreshadows 缓存壳', () => {
  it('TTL 内二次调用命中不重扫（scan 计数不变），快照结果一致', () => {
    const root = makeTree()
    const s1 = getForeshadowsCached(root, 60_000)
    ...
    expect(foreshadowCache.stats().misses).toBe(1) // 命中：未重扫
  ...
  it('目录变化（指纹变）后重算：新增正文章节足迹可见；新增伏笔文件入列', async () => {
    ...
    const s3 = getForeshadowsCached(root, 60_000)
    expect(foreshadowCache.stats().misses).toBe(3) // 设定/伏笔 mtime 变 → 重扫
    expect(s3.entries.map((e) => e.标题)).toContain('旧信')
  })

  it('?q= 过滤走缓存：TTL 内不同检索词均不重扫，结果按过滤维度命中', () => {
    const root = makeTree()
    const snapshot = getForeshadowsCached(root, 60_000)
    expect(foreshadowCache.stats().misses).toBe(1)
    ...
    getForeshadowsCached(root, 60_000) // 再取一次快照（应命中）
    expect(foreshadowCache.stats().misses).toBe(1)
  ...
  it('TTL 到期重扫；forget 显式失效同效', () => {
    ...
    expect(foreshadowCache.stats().misses).toBe(2)
    getForeshadowsCached(root, 0) // 即刻过期
    expect(foreshadowCache.stats().misses).toBe(3)
  })
})
