 ...
 * GET/HEAD 与带 operationId 的 PUT（文档保存幂等键，shared/revision.ts 注入）之外
 * 不自动重放——re-boot 照常执行（新 token 已就位），401/403 响应原样透传调用方。
 *
...
describe('E106: 401 重放仅限幂等面', () => {
  ...
  it('PUT 无 operationId 401 → 不重放（无幂等键盲发即双写）', async () => {
    const c = await freshClient()
    const { bizCalls } = stubBootThen([401, 200])
    await c.boot()
  ...
  it('PUT 带 operationId 401 → 重放恰一次（文档保存幂等键面）', async () => {
    const c = await freshClient()
    const { bizCalls } = stubBootThen([401, 200])
    await c.boot()
    ...
    expect(bizCalls()).toBe(2) // 修复点：重放恰一次
  })
