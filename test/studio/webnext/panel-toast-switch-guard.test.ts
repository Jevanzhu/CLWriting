 ...
 * - R66-30/31/32 失败 toast 书名守卫（History/MetaForm/Review 三面板 catch 漏配；同型守卫表驱动）
...
// ── R66-30/31/32：三面板失败 toast 书名守卫（表驱动，同型守卫三面板） ───────

/** HistoryPanel：挂载 + 点恢复 + 确认弹窗 → 走到 await restoreSnapshot（挂起） */
...
/** MetaFormPanel：挂载 + 点保存 → 走到 await updateDocMeta（挂起） */
...
/** ReviewPanel：种子正文树 + 挂载 + 点「通过」裁决 → 走到 await runVerdictDoc（挂起） */
...
const panelGuardCases = [
  {
    label: 'HistoryPanel 恢复',
    mountAndAct: mountAndConfirm,
    api: mocks.restoreSnapshot,
    err: '恢复炸了',
  },
  {
    label: 'MetaFormPanel 保存',
    mountAndAct: mountAndSave,
    api: mocks.updateDocMeta,
    err: '保存炸了',
  },
  {
    label: 'ReviewPanel 裁决',
    mountAndAct: mountAndVerdict,
    api: mocks.runVerdictDoc,
    err: '裁决炸了',
  },
]

describe('R66-30/31/32: 三面板失败 toast 书名守卫（失败面表驱动）', () => {
  it.each(panelGuardCases)('$label 失败且已切书 → 不在 B 书界面 toast', async ({ mountAndAct, api, err }) => {
