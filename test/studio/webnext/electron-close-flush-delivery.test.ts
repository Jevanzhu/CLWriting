...
app.whenReady().then(() => {
  ...
  // 时长注记（测试资产行为化批评估结论：保留真实时长）——本测试跑真实 Electron 引擎 +
  // 真网络（不 stub 任何 XHR/fetch），300ms 是「页面 loadURL 完成 → 异步 flush 链挂好 →
  // 拦截 close」的真实引擎沉淀窗、4000ms 是 flush 兜底的竞速上限守卫、30s 是挂死杀进程
  // 兜底：三处都在真进程编排骨架内，非假时钟域，改造为假时钟/轮询反而改换被验证语义。
