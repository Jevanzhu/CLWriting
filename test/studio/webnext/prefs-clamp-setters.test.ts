...
const CLAMP_ROWS: Array<{
  ...
  { name: 'snapDays 1-365', call: (p, v) => p.set('snapDays', v), read: (p) => p.get('snapDays'), min: 1, mid: 30, max: 365 },
  { name: 'snapCount 1-200', call: (p, v) => p.set('snapCount', v), read: (p) => p.get('snapCount'), min: 1, mid: 30, max: 200 },
  { name: 'defaultVolumeSize 5-500', call: (p, v) => p.set('defaultVolumeSize', v), read: (p) => p.get('defaultVolumeSize'), min: 5, mid: 50, max: 500 },
  { name: 'aiBatchSize 1-20', call: (p, v) => p.set('aiBatchSize', v), read: (p) => p.get('aiBatchSize'), min: 1, mid: 8, max: 20 },
  { name: 'callsPerChapter 1-50', call: (p, v) => p.set('callsPerChapter', v), read: (p) => p.get('callsPerChapter'), min: 1, mid: 8, max: 50 },
  { name: 'relationMineThreshold 1-20', call: (p, v) => p.set('relationMineThreshold', v), read: (p) => p.get('relationMineThreshold'), min: 1, mid: 3, max: 20 },
  { name: 'checkRepeatCharsThreshold 2-1000', call: (p, v) => p.set('checkRepeatCharsThreshold', v), read: (p) => p.get('checkRepeatCharsThreshold'), min: 2, mid: 200, max: 1000 },
  { name: 'checkMaxSentenceLen 10-500', call: (p, v) => p.set('checkMaxSentenceLen', v), read: (p) => p.get('checkMaxSentenceLen'), min: 10, mid: 60, max: 500 },
  { name: 'checkImageryThreshold 1-100', call: (p, v) => p.set('checkImageryThreshold', v), read: (p) => p.get('checkImageryThreshold'), min: 1, mid: 3, max: 100 },
  { name: 'checkWordCountTolerance 1-500', call: (p, v) => p.set('checkWordCountTolerance', v), read: (p) => p.get('checkWordCountTolerance'), min: 1, mid: 30, max: 500 },
  // clamp+apply 双职项一并钉界（uiFontSizeStep：行 set clamp + side apply）
  { name: 'uiFontSizeStep -1..2', call: (p, v) => p.set('uiFontSizeStep', v), read: (p) => p.get('uiFontSizeStep'), min: -1, mid: 1, max: 2 },
]
