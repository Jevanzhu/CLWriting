...
describe('R46-6: migrateBookKeyedState 改名迁移五族按书键控状态', () => {
  it('五族状态旧名清理 + 新名值搬家（不丢作者既有状态）', () => {
    ...
    // ③ 误报灰显键族（两文档）。R50-D2-1（五十轮）：键随 R49-27 现行格式 \u0000 分隔
    // ——原测试钉死旧冒号格式，恰掩盖了迁移分支漏改（旧前缀对现行键永不匹配）
    localStorage.setItem('clw-fp:旧书\0doc-a', '1')
    localStorage.setItem('clw-fp:旧书\0doc-b', '1')
    localStorage.setItem('clw-fp:别书\0doc-a', '1') // 他书同文档名键不受牵连
    // ④⑤ 梗概 + 首开标记
    ...
    // 旧名全清
    expect(localStorage.getItem('clw-fp:旧书\0doc-a')).toBeNull()
    expect(localStorage.getItem('clw-fp:旧书\0doc-b')).toBeNull()
    expect(localStorage.getItem(onboardPremiseKey('旧书'))).toBeNull()
    expect(localStorage.getItem(treeFirstOpenKey('旧书'))).toBeNull()
    // 新名值在位
    expect(localStorage.getItem('clw-fp:新书\0doc-a')).toBe('1')
    expect(localStorage.getItem('clw-fp:新书\0doc-b')).toBe('1')
    expect(localStorage.getItem(onboardPremiseKey('新书'))).toBe('梗概内容')
    expect(localStorage.getItem(treeFirstOpenKey('新书'))).toBe('1')
    // 他书键不动
    expect(localStorage.getItem('clw-fp:别书\0doc-a')).toBe('1')
  ...
  it('同名 no-op + 空状态幂等', () => {
    localStorage.setItem('clw-fp:书\0doc', '1')
    migrateBookKeyedState('书', '书')
    expect(localStorage.getItem('clw-fp:书\0doc')).toBe('1')
