 ...
 * 接管；④指数退避 2s→4s→…→60s 封顶；⑤onopen 清零计数（重连成功后从 2s 重新起阶）；
...
describe('X-6 · SSE 退避接管协议', () => {
  ...
  it('第 6 次抖动错误 → 接管：close 原连 + 2s 后手动重连', async () => {
    useSse(ref('书A'))
    await settle()
    const es0 = MockES.instances[0]!
    es0.readyState = 0
    ...
    const es1 = await reconnectAfter(2_000)
    expect(es1.url).toContain('/api/books/%E4%B9%A6A/stream')
  })

  it('fail-closed（CLOSED）首次错误立即接管（不经 5 次快重试）', async () => {
    useSse(ref('书A'))
    await settle()
    const es0 = MockES.instances[0]!
    failClose(es0) // 首次错误即接管
    expect(es0.closed).toBe(true)
    await reconnectAfter(2_000)
  })

  it('指数退避 2s→4s 阶梯，6 次接管后封顶 60s（不再翻倍）', async () => {
    useSse(ref('书A'))
    await settle()
    let es = MockES.instances[0]!
    failClose(es)
    es = await reconnectAfter(2_000) // 第 1 阶：2s
    failClose(es)
  ...
  it('onopen 清零计数：重连成功后再抖 5 次不接管，下次接管从 2s 重新起阶', async () => {
    ...
    failClose(es)
    es = await reconnectAfter(2_000) // backoffStep 已清零：从 2s 重新起阶（非 4s）
    failClose(es)
    vi.advanceTimersByTime(3_999)
    await settle()
    expect(MockES.instances).toHaveLength(3) // 仍按 2s 阶梯（4s 未到不连）
