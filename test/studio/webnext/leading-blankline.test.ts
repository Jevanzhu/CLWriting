 ...
 *  时点从当拍移到窗末，故 emit 后须等窗再断言——改为按 store 落位轮询（vi.waitFor），
 *  不再固定 sleep 真实时长。 */
async function settleWriteback(expected: string, doc: ReturnType<typeof useDocStore>): Promise<void> {
  await vi.waitFor(() => expect(doc.get('d1')!.content).toBe(expected))
  await flushPromises()
}
