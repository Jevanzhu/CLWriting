/**
 * apiFetch 401/403 恢复通道（re-boot）与 token 轮换重放语义。
 *
 * E-2（第五十三轮）：boot 一次性取 token 失败（API 慢就绪超 ~21s）后 token 永久
 * null，写请求持续 401/403 只能刷新页面——修复后 apiFetch 在 401/403 且 token null
 * 时触发一次防抖去重的 re-boot，成功则重放原请求（单请求最多一次），失败原样透传。
 * Y-30（第五十七轮）：token 非空但失效（dev 重启 dev:api 换 randomUUID）同走该
 * 通道——401/403 一律防抖 re-boot，**token 变化才重放**（拿回同一枚说明 401/403
 * 另有原因，透传不空转）。生产 Electron 靠持久化 token 规避（低危定级依据），
 * 本回归锁 dev 形态。
 *
 * 0918二轮修复批（E106）：重放收敛幂等面（GET/HEAD/带 operationId 的 PUT）——
 * 重放面用例的请求方法统一改 GET（POST/PUT 无幂等键现属不重放面，另见
...
describe('E-2 · apiFetch 401/403 恢复通道', () => {
  ...
  it('并发多个 401 请求 → re-boot 防抖去重只触发一次', async () => {
    ...
    // 让三个请求都打到 401 分支并挂起等 re-boot——按 mock 命中数轮询（状态到达），
    // 不再固定 sleep 真实时长
    await vi.waitFor(() => expect(pathCalls).toBe(3))
    releaseBoot!()
