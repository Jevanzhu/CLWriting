...
describe('R34D-21: 七动作 catch 补切书守卫（A 书报错不落 B 书界面）', () => {
  ...
  // 七动作同型守卫：触发表驱动（动作名 → 触发器 → 挂起的 api mock）
  const guardCases: Array<
    [string, (a: ReturnType<typeof useChapterTreeActions>) => Promise<unknown>, ReturnType<typeof vi.fn>]
  > = [
    ['doCopy', (a) => a.doCopy(bodyFile('写作/正文/0001-雪.md')), copyMock],
    ['doDelete', (a) => a.doDelete(bodyFile('写作/正文/0001-雪.md')), deleteMock],
    ['doMove', (a) => a.doMove('d1', '写作/正文/卷一'), moveMock],
    [
      'onRenameCommit',
      (a) => {
    ...
    ['createSingleton', (a) => a.createSingleton('大纲/总纲.md', '总纲'), createMock],
    [
      'onSaveMeta',
      (a) => {
  ...
  it.each(guardCases)('%s：失败 + 切书 → openError 不落 B 书', async (_name, trigger, mock) => {
    await runCase(trigger, mock)
  })
