...
/**
 * CmHost 外部全量替换的 IME 组合态守卫（真实 CM6 行为级）——按行为合并两散落文件
 * （原 f5-cm-composition-guard + r43-frontend-batch 的 R43-15 节，装置同构）。
 *
 * - F5（五十九轮）：中文输入组合中（compositionstart → compositionend 之间）外部同步
 *   （refresh/SSE）到达：不立即整段替换（会吞组合中文本），挂起到 compositionend 后应用。
 * - B-1（重审批次）：挂起后继续组字 → compositionend 应用最新值，续打组合文本不被抹掉
 *   （修复前：登记时旧快照把续打整段抹掉且不可 undo）；B-25：compositionend 已排定后
 *   卸载 → 延迟回调不对 destroyed view 派发、无回写无异常。
 * - R43-15（四十三轮）：双组合间隙 <1 帧的丢弃分支保留挂起（自愈链：下一次
 *   compositionend 的既有消费路径再触发应用，修复前挂起被清后外部替换永久丢失）。
 */
