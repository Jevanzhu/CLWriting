...
describe('PM-1 伏笔扫描异步孪生', () => {
  it('async 与 sync 快照逐字段等价（entries + trails 全量 deep-equal）', async () => {
    const root = makeTree()
    const viaAsync = await getForeshadowsCachedAsync(root, 60_000)
    forgetForeshadowCache(root)
    foreshadowCache.resetStats()
    const viaSync = getForeshadowsCached(root, 60_000)
  ...
  it('并发 MISS in-flight 去重：两请求只扫一次，拿到同一快照实例', async () => {
    ...
    forgetForeshadowCache(root)
    foreshadowCache.resetStats()
    const [a, b] = await Promise.all([getForeshadowsCachedAsync(root, 60_000), getForeshadowsCachedAsync(root, 60_000)])
    expect(foreshadowCache.stats().misses).toBe(1)
    expect(a).toBe(b) // 同一 job → 同一实例（非各自重算的两份）
    // 第三次调用（job 已收尾）走缓存命中，不重扫
    await getForeshadowsCachedAsync(root, 60_000)
    expect(foreshadowCache.stats().misses).toBe(1)
  })
  ...
  it('async 填充后同步路径命中同一缓存（scan 计数不变）', async () => {
    ...
    foreshadowCache.resetStats()
    const viaSync = getForeshadowsCached(root, 60_000)
    expect(foreshadowCache.stats().misses).toBe(0)
