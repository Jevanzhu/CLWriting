 ...
 * （R0916-7 收尾批：文件由批号命名 r0916-p3-6-assembly-root.test.ts 改行为命名；判据不变。）
 *
 ...
 * ③ **必需能力缺失在编译期不可表达**：@ts-expect-error 探针（StudioDriver 必需契约——
 *    P3-16 收尾起含中断通道族——缺任一法即不可作为 deps.driver 注入；`npm run typecheck`
 *    是判据——指令未命中会报未使用错）。
 *
...
/** 替身驱动能力面（StudioDriver 必需契约全成员——缺能力的形态由用例③的编译期探针表达）。 */
...
describe('R0916-7-P3-6：组装根注入（deps 契约 + 双实例隔离）', () => {
  ...
  it('③ 必需能力缺失在编译期不可表达（@ts-expect-error 探针：缺 stream / 缺 interrupt 均不可作为 deps.driver）', async () => {
    ...
    // @ts-expect-error 缺必需能力 stream：StudioDriver 必需契约在编译期拒绝该形态（tsc 未命中会报未使用指令）
    const missingStream: ServiceDriver = partial
    // P3-16 收尾探针：中断通道族提为必需后，恰缺 interrupt 的替身同样不可赋值
    //（变异判据：契约回退成可选 → 两条指令均未命中 → typecheck 红）
    // @ts-expect-error 缺必需能力 interrupt：中断通道是 driver 契约必需成员
    const missingInterrupt: ServiceDriver = {
      ...partial,
      stream: host.driver.stream,
      isRunning: host.driver.isRunning,
      isWriterRunning: host.driver.isWriterRunning,
      registerCtrl: host.driver.registerCtrl,
      unregisterCtrl: host.driver.unregisterCtrl,
    }
    // 对照：补齐全员后可赋值（无指令、编译期放行）
    const complete: ServiceDriver = { ...missingInterrupt, interrupt: host.driver.interrupt }
    ...
    expect(typeof missingInterrupt.registerCtrl).toBe('function')
    expect(typeof complete.interrupt).toBe('function')
