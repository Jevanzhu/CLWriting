/**
 * R71-1 / R71-2（总七十一轮）服务端互斥矩阵补角回归（参照 server-mutex-redaction-cache-guards 写法）：
...
describe('R71-2: /auto-write 二次复查任务闸复检', () => {
  it('首检过后、body 发完前的窗口内 acquire 任务闸 → 409 BUSY（不再漏拦）', async () => {
    ...
    const resultP = new Promise<{ status: number; json: any }>((resolve, reject) => {
      ...
      void (async () => {
        // 保留固定垫（2026-09-26 批注记）：真实 socket 上无「readJson 已悬持」的服务侧
        // 探针可挂——此 80ms 是「首检臂未提前返回」的有界观察窗，且 earlyResponse 守卫
        // 把窗不够宽的形态显式转红（不静默通过），非盲睡
        await sleep(80)
        if (earlyResponse) {
          req_.destroy()
