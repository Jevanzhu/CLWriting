 ...
 * 的真实竞态 timer 复现——可绿但时序脆。现改用组装根 overrides 注入的 restore 让出档
 *（原 __setSnapshotsRestoreYieldForTest 已删，R0916-7-P3-6 收编进 RouteDeps）在
 * readJson 前确定性停走：档 parked 信号到手后直接改写 books.jsonl + renameSync 搬书
...
// restore 让出档（组装根 overrides 注入固定委托，用例内换臂）：null = 不停走（对照组）
let restoreYield: (() => Promise<void>) | null = null
...
beforeAll(async () => {
  studio = await bootStudio({
    ...
    // readJson 前确定性停走档：restoreYield 非空时悬持（用例置臂），空则透传
    overrides: {
      snapshotsRestoreYield: async () => {
        const y = restoreYield
        if (y) await y()
      },
    },
    dirs: ['项目'],
...
describe('重评二轮-P3-1: snapshots restore readJson 窗口书注册重验', () => {
  ...
  it('readJson 窗口内书被改名 → 409 BOOK_MOVED，旧路径无幽灵目录、新路径内容保旧', async () => {
    ...
    // 确定性停走：档进入即发 parked 信号，unpark 前 handler 悬在 readJson 前
    let unpark!: () => void
