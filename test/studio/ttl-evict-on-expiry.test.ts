 ...
 * 必败重算：修复前过期死条目仍驻留（has()=true），修复后随 miss 检查顺手逐出
 * （has()=false）。命中观测读导出缓存实例 stateCache/styleScanCache 的 has()（原模块级
 * 缝已删）；失败注入走透传式 vi.mock（先例 static.test.ts 的 node:fs/promises spy），
...
import { stateCache } from '../../src/studio/server/api/state.js'
import { styleScanCache } from '../../src/studio/server/api/health.js'
...
beforeAll(async () => {
  ...
  // 墙钟（先例 state-tree-issues-ttl），CI 慢机测试段被睡眠拖长（R0911-G-P1-1c
  ...
  // TTL 短档经组装根 overrides 注入（原模块级 setter 已删）
  server = await startServerSafe({
    port: 0,
    workDir,
    overrides: { stateTtlMs: 1000, styleScanTtlMs: 1000 },
  })
...
describe('R47-18：过期条目随 miss 检查顺手逐出（行为间接断言）', () => {
  it('state：过期 → 重算 500（不落缓存）→ 条目已被逐出；恢复后可重建', async () => {
    ...
    expect(stateCache.has(bookRoot)).toBe(true)
    ...
    expect(stateCache.has(bookRoot)).toBe(false)
  ...
  it('health/style：过期 → 扫描 rejection（dispatch 500、不落缓存）→ 条目已被逐出；恢复后可重建', async () => {
    ...
    expect(first.status).toBe(200)
    expect(styleScanCache.has(bookRoot)).toBe(true)
    ...
    expect(failed.status).toBe(500)
    expect(styleScanCache.has(bookRoot)).toBe(false) // 逐出（修复前驻留为 true）
