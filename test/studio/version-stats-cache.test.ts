 ...
 *
 * 断言用「全量重算计数」观测口（versionStatsCache.stats().misses），确定性不依赖墙钟 5s。
 *
...
describe('R36-7 version-stats 缓存', () => {
  it('连续两次请求：第二次缓存命中不重算（scan 计数不变），结果一致', async () => {
    ...
    expect(versionStatsCache.stats().misses).toBe(1) // 命中：未重算
  ...
  it('目录结构变化（新 doc 目录）：TTL 窗内节流命中；TTL 到期重探失效；forget 显式失效同效', async () => {
    const root = makeBook()
    await getVersionStatsCached(root, 60_000)
    expect(versionStatsCache.stats().misses).toBe(1)
    await sleep(5)
    ...
    await getVersionStatsCached(root, 60_000)
    expect(versionStatsCache.stats().misses).toBe(3)
  })
  ...
  it('TTL 到期重算：探针无变化也按超期重算（注入 TTL=0）', async () => {
    ...
    expect(versionStatsCache.stats().misses).toBe(1)
    await getVersionStatsCached(root, 0) // 即刻过期
    expect(versionStatsCache.stats().misses).toBe(2)
    await getVersionStatsCached(root, 60_000)
    expect(versionStatsCache.stats().misses).toBe(2) // 恢复长档后命中缓存
  })
})
