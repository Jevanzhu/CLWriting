 ...
 * R0916-7-P3-6 适配：墓地清理桩经组装根 overrides 注入（原模块级 setter/wait 钩子已删），
 * ULID 用例走专用实例（独立 workDir + no-op 桩），跨用例等待不再需要。
 */
...
function registerBooksIn(dir: string, names: string[]): void {
  writeFileSync(
...
async function req(
  ...
  path: string,
  body?: unknown,
  h?: { baseUrl: string; token: string },
): Promise<{ status: number; json: unknown }> {
  const r = await fetch(`${(h ?? { baseUrl, token }).baseUrl}${path}`, {
    method,
    headers: {
      'x-studio-token': (h ?? { baseUrl, token }).token,
...
describe('R42-14：删书墓地名唯一 + 并发双删收口', () => {
  ...
  it('墓地名带 26 字符 Crockford ULID 后缀：两次删除副本名互异（同毫秒亦不撞）', async () => {
    // 专用实例：组装根 overrides 注入 no-op 清理桩留副本供断言（独立 workDir，主实例
    // 的在途清理与本用例的 readdir 断言天然隔离，跨用例等待不再需要）
    const solo = await bootStudio({
      prefix: 'clw-r42-books-solo-',
      dirs: ['.clwriting'],
      overrides: { graveyardCleanup: async () => {} },
    })
    try {
      registerBooksIn(solo.workDir, ['墓地甲书', '墓地乙书'])
      const d1 = await req('DELETE', `/api/books/${encodeURIComponent('墓地甲书')}`, undefined, solo)
      const d2 = await req('DELETE', `/api/books/${encodeURIComponent('墓地乙书')}`, undefined, solo)
      expect(d1.status).toBe(200)
      expect(d2.status).toBe(200)
      const graveDir = join(solo.workDir, GRAVEYARD)
      ...
      // 手工清场后关专用实例（no-op 桩不消费墓地副本，不能留给 close 的递归清理赌运气）
      rmSync(join(solo.workDir, GRAVEYARD), { recursive: true, force: true })
      await solo.close()
    }
  })
})
