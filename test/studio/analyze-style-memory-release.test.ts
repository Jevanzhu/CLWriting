 ...
 * TTL 内二查命中缓存（R62-21 口径短档，组装根 overrides 注入）sourceHash 不变。造法沿用 style-scan-ttl-cache。
 *
...
beforeAll(async () => {
  ...
  studio = await bootStudio({
    ...
    // TTL 短档经组装根 overrides 注入（原模块级 setter 已删，R62-21 口径不变）
    overrides: { styleCorpusTtlMs: 1000 },
    dirs: ['写作/正文'],
