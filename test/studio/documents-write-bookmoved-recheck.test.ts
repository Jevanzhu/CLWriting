...
describe('R1010b-SRV-P2-1 面 B：drainForeshadowSaveChains', () => {
  it('链上有在途单元（svc.save 被保存锁挡住）时 drain 等到其收尾才 resolve', async () => {
    ...
    try {
      ...
      // 轮询观测钩子等链键出现（waitFor 单源；原手写 sleep(1) 轮询循环改收编）
      await waitFor(() => __foreshadowSaveChainKeysForTest().includes(rig.bookRoot), 2000, 1, '伏笔链键出现')
      let drained = false
      const drainP = drainForeshadowSaveChains(rig.bookRoot).then(() => {
        drained = true
      })
      // 确定性改造（2026-09-26 批，原固定实睡 60ms）：让出数拍——drain 若无视在途链
      // （回归形态）必在微任务/立即拍内错误 resolve，让出足矣显形
