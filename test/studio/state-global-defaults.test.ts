 ...
 *
 * 测试精简批（2026-09-12）：启动样板收编 bootStudio（get 走裸 http.request 定制形态，保留本地）。
 */
...
beforeAll(async () => {
  ...
  writeFileSync(join(userDataPath, 'global.json'), JSON.stringify({ defaultVolumeSize: GLOBAL_VOLUME_SIZE }), 'utf8')
  studio = await bootStudio({
    book: BOOK,
    prefix: 'clwriting-gg25-',
    userDataPath,
    dirs: ['布线/悬念', '工作区', '写作/正文'],
