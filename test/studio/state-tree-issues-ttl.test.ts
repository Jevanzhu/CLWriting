 ...
 * TTL 注入经组装根 overrides、命中观测读缓存实例 has()（原模块级缝已删）。
 */
...
import { treeIssuesCache } from '../../src/studio/server/api/check.js'
...
beforeAll(async () => {
  ...
  // TTL 短档经组装根 overrides 注入（原模块级 setter 已删，R76-37 口径 1000ms 不变）
  server = await startServerSafe({
    port: 0,
    workDir,
    overrides: { stateTtlMs: 1000, treeIssuesTtlMs: 1000 },
  })
...
describe('R75-D-P3b：删书生命周期清理（forgetBookKeyedCaches 接线）', () => {
  it('GET 填充两缓存 → DELETE /api/books/:name 后条目随书失效', async () => {
    ...
    expect(stateCache.has(join(workDir, STATE_BOOK))).toBe(true) // 端点填充
    expect(treeIssuesCache.has(treeBookRoot)).toBe(true)

    const del = await req('DELETE', `/api/books/${encodeURIComponent(STATE_BOOK)}`)
    expect(del.status).toBe(200)
    expect(stateCache.has(join(workDir, STATE_BOOK))).toBe(false) // 删书清理
    // 另一本书的 tree-issues 条目不受影响（书键隔离）
    expect(treeIssuesCache.has(treeBookRoot)).toBe(true)
    const del2 = await req('DELETE', `/api/books/${encodeURIComponent(TREE_BOOK)}`)
    expect(del2.status).toBe(200)
    expect(treeIssuesCache.has(treeBookRoot)).toBe(false)
  })
})
