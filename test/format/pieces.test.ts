...
test('writePiece + readPiece: 最小字段往返', () => {
  const piece: PieceMeta = { 章号: 1, 标题: '雪夜来客', 目标情绪: '压抑到释然', 核心反转: '来客即凶手' }
...
test('readPiece: 缺必填章号 → 容错错误', () => {
  const fp = join(tmp, '正文.md')
  writeFileSync(fp, '---\n标题: 无章号\n---\n正文', 'utf-8')
  const r = readPiece(fp)
  expect(r.ok).toBe(false)
  if (r.ok) return
  expect(r.error.message).toContain('章号')
})
...
test('parsePieceFileName: 从文件名提取章号+标题', () => {
  expect(parsePieceFileName('篇/001-雪夜来客.md')).toEqual({ 章号: 1, 标题: '雪夜来客' })
  expect(parsePieceFileName('001-雪夜来客')).toEqual({ 章号: 1, 标题: '雪夜来客' })
  expect(parsePieceFileName('无章号.md')).toBeNull()
})
...
test('readPieceDir: 扫 篇/*.md 读正文', () => {
  ...
  writePiece(join(篇Root, '001-雪夜来客.md'), { 章号: 1, 标题: '雪夜来客' }, '正文一')
  writePiece(join(篇Root, '002-暗河.md'), { 章号: 2, 标题: '暗河' }, '正文二')

  const r = readPieceDir(篇Root)
  expect(r.pieces).toHaveLength(2)
  expect(r.pieces.map((p) => p.章号).sort((a, b) => a - b)).toEqual([1, 2])
})

test('countPieces: 只计 章号-标题.md 格式文件', () => {
  const 篇Root = join(tmp, '篇')
  mkdirSync(篇Root, { recursive: true })
  writeFileSync(join(篇Root, '001-标题.md'), '', 'utf-8')
  writeFileSync(join(篇Root, '002-标题.md'), '', 'utf-8')
  writeFileSync(join(篇Root, '散文件.md'), '', 'utf-8') // 不计：无章号前缀
...
test('migratePieceLayout: 旧目录结构 → 新文件结构', () => {
  ...
  writePiece(join(tmp, '篇', '001-旧案', '正文.md'), { 章号: 1, 标题: '旧案' }, '正文')
...
test('migratePieceLayout: 无旧结构则 no-op', () => {
  ...
  writePiece(join(tmp, '篇', '001-已有.md'), { 章号: 1, 标题: '已有' }, '正文')
