 ...
 * 被睡住、定时器不可能先行触发；等待用 vi.waitFor 轮询，无定长实睡）；随后移除
 * 探针锁模拟对方释放，保存须照常完成。
 */
...
test('R30-6: executeSave 布线锁等待不阻塞事件循环（定时器先行触发）；释放后保存照常完成', async () => {
  ...
  // 同步 Atomics.wait 形态下事件循环冻结，探针定时器与本次轮询一同饿死 → 超时红
  await vi.waitFor(() => expect(timerFired).toBe(true), { timeout: 90 })
  // save 仍等待布线锁（未以超时/错误提前返回——注入档为默认 5s）
  const settledEarly = await Promise.race([saveP.then(() => true), Promise.resolve().then(() => false)])
