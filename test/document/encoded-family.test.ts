...
test.skipIf(process.platform === 'win32')(
  // R70-8：字面冒号文件/目录仅 mac/Linux 可造（NTFS EINVAL），编码路径由其余用例覆盖
...
async function makeLegacyMovePendingBook(
  pos: 'new' | 'old',
): Promise<{ root: string; docId: string; journalName: string }> {
  const b = makeGitBook()
...
test('孤儿 journal（清单无登记 + move 两端都不在）→ 归档 .orphaned 不报 crashedWrite', async () => {
  ...
  expect(readdirSync(join(root, '工作区', '.journal')).some((f) => f.startsWith('legacy_ghost.jsonl.orphaned-'))).toBe(
    true,
  )
})
