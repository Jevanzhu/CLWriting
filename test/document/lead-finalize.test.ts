...
test('X-P2-8: resolve 动词（揭晓）→ 状态派生 已收尾', () => {
  const { root } = makeBook()
  try {
...
test('X-P2-8: drop 动词（放弃）→ 状态派生 已放弃', () => {
  const { root } = makeBook()
  try {
...
test('X-P2-8: 成长线 resolve（突破）→ 常态化升级保持 进行中', () => {
  const { root } = makeBook()
  try {
    makeGrowthLead(root)
...
test('X-P2-8: 作者显式终态不被 advance/其他动词覆盖（仅 进行中 才派生）', () => {
  const { root } = makeBook()
  try {
...
test('X-P2-6: 主文件载其他章待确认 → 只回收本章归档，主文件不动', () => {
  const { root } = makeBook()
  try {
...
test('X-P2-6: 主文件标签=本章 + 本章归档并存 → 双源合并回收 + 双清', () => {
  const { root } = makeBook()
  try {
    const main = join(root, '工作区', '账本推进.md')
...
test('X-P2-6: 主文件无标签（旧格式）→ 视为本章回收清空（单章旧行为兼容）', () => {
  const { root } = makeBook()
  try {
    const main = join(root, '工作区', '账本推进.md')
...
test('X-P2-6: 其他章归档留存 + 无本章条目 → 0 且互不影响', () => {
  const { root } = makeBook()
  try {
    const archDir = join(root, '工作区', '.账本推进暂存')
    mkdirSync(archDir, { recursive: true })
