...
test('revision→final：脏文件被 commit → git 干净 + 状态回 final', () => {
  ...
  expect(deriveStatus('写作/正文/0001-开篇.md', collectDirtyFiles(root))).toBe('revision')
  ...
  expect(deriveStatus('写作/正文/0001-开篇.md', collectDirtyFiles(root))).toBe('final')
...
test('非定稿区文档（写作/草稿 draft）→ NOT_DRAFT_REGION', () => {
  const root = mkdtempSync(join(tmpdir(), 'finalize-nodraft-'))
  execSync('git init', { cwd: root, stdio: 'pipe' })
  execSync('git config user.email t@t.com', { cwd: root, stdio: 'pipe' })
  execSync('git config user.name t', { cwd: root, stdio: 'pipe' })
  mkdirSync(join(root, '写作', '草稿'), { recursive: true })
  writeFileSync(join(root, '写作', '草稿', '草稿-1.md'), '草稿', 'utf-8')
  const manifestPath = join(root, '项目', '文档清单.jsonl')
  mkdirSync(join(root, '项目'), { recursive: true })
  const m = readManifest(manifestPath)
  const docId = generateDocId()
  upsertEntry(m, { id: docId, nodeType: 'document', path: '写作/草稿/草稿-1.md', parentId: null })
  writeManifest(manifestPath, m)
