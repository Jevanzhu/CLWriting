...
test('R36-1: CRLF 账本经定稿回写——既有履历保留 + 新条目追加（不物理清空）', async () => {
  ...
  try {
    ...
    // 行尾契约演进：R38-11（三十八轮）曾为「主导行尾保真」（源 CRLF 回写保持 CRLF）；
    // 平台规范化批一（2026-09-03）推翻为规范形 LF——定稿回写后全文归一 LF（跨机
    // 差异归零）；履历完整性语义与 R36-1 不变。LF 账本字节不变的锚见
    // test/format/md-extension-family.test.ts（R38-11 段；R0916-7-P3-5 行为命名前 r38-batch-e）。
