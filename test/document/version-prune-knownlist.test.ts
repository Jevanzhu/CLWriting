 ...
 *
 * 注：ulid() 随机后缀非进程内单调，同毫秒写入的 id 序不稳定——逐版 +5ms 假时钟
 * 推进保证 id 时间序 = 创建序（断言锚定的前提）。P3-5 确定性化：原 sleep(5) 实睡
 * 改 vi 假时钟 advanceTimersByTime（writeVersion 全链同步 fs、无定时器消费，假时钟
 * 只改 Date.now() 供 ULID 取毫秒头），间隔不再依赖真实调度。
 */
...
/** 逐版间隔 ≥5ms 写 n 版（假时钟推进，保证 ULID 时间序 = 创建序），返回创建序 id 数组 */
