 ...
 *
 * 工单施工序 3-4 验证点：
 * - 态 4 续跑判定（⑬ 第 5 节中断点：pre-commit 续写 / post-commit-residue 幂等清理）
...
test('findChapterCommit: 按 ch:NNNN 前缀反查章 commit（⑯ 第 5 节回滚定位）', () => {
...
test('commit msg 合规（⑯ 第 4 节前缀）: ch:0001 标题 + Confirmed trailer', () => {
  const root = makeGitBook({ withCache: true })
  const workDir = join(root, '工作区')
  const outline = join(workDir, '细纲.md')
  writeFileSync(outline, '第1章细纲', 'utf-8')
  ...
  expect(msg).toContain('Confirmed:') // ⑪ 留痕 trailer
...
test('git 隐身: finalize 全链路经人话层，作者不敲 git（验收点）', () => {
  ...
  // 全经 git/exec.ts 的 addCommit（⑯ 第 3 节）。
