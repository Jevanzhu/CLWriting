...
describe('重评2-P3-1: setting-rule 移植物 × check/count 对拍（同一输入两端同判）', () => {
  ...
  const GUARD_CASES: Array<{ title: string; body: string; expect: 'report' | 'exempt' }> = [
    ...
    {
      title: '对白引导词收尾（R29-B12）→ 两端都豁免',
      body: '他低声道，「快走」二字未落，人已没入夜色。',
      expect: 'exempt',
    },
    ...
    {
      title: '动作+对白混排、span 含句读（R76-3）→ 两端都豁免',
      body: '他忽然冷下脸「住手了！」随之一刀挥落。',
      expect: 'exempt',
    },
    {
      title: '引导词表收窄：叫不豁免（R29-B12 反例）→ 两端都报',
      body: '名叫「萧策」的老者立在门前。',
      expect: 'report',
    },
    {
      title: '引导词残余面：普通词「道」收尾照豁免（两端同款取舍）→ 两端都豁免',
      body: '他切了频道，「叶秋」要来了。',
      expect: 'exempt',
    },
