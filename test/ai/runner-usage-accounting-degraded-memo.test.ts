...
import { loadProviders, persistDegraded, processProviderRuntime } from '../../src/ai/provider/store.js'
...
describe('R34D-7：降级记忆 per-key 标记在 saveProviders 成功后才置位', () => {
  it('排队段 save 失败 → 不标记；修复后下轮 persistDegraded 自然重试成功落盘', async () => {
    ...
    processProviderRuntime().__seedProvidersWriteChainForTest(ud, Promise.resolve())
