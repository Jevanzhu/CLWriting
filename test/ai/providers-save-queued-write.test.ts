/**
 * （R0916-7 收尾批：文件由批号命名 r29-ai-store-save.test.ts 改行为命名；用例与断言零改动。）
 ...
 *
 * 排队段生产不可直达（空闲快路永不入链），经 processProviderRuntime().__seedProvidersWriteChainForTest 注入
...
test('R29-2③ 排队段失败：返回 promise reject 不吞 + log.warn 留痕', async () => {
  processProviderRuntime().__seedProvidersWriteChainForTest(dir, Promise.resolve()) // 注入在途段 → 本写排队
...
test('R0913-win P3-3: 写链键折叠——case 变体寻址共享同一条串行链（折叠面），不折叠面按字面独立', async () => {
  ...
  processProviderRuntime().__seedProvidersWriteChainForTest(variant, gate)
