 ...
 *
 * 2026-09-26 终扫并入 r31a-openai-truncation.test.ts 的消息展开序臂（R31-6：混合
 * user 消息 text + tool_result → role:'tool' 先于 user 文本——OpenAI 要求 tool 消息
 * 紧跟 assistant tool_calls；断言保留、夹具收编 runOpenAI 捕获形态）。
 */
...
// ── R31-6（三十一轮，2026-09-26 终扫自 r31a-openai-truncation.test.ts 并入）：
// 混合 user 消息展开序——tool 消息先于 user 文本 ────────────────────────────────
