/**
 * （R0916-7 收尾批：文件由批号命名 r71-store-bak-plaintext.test.ts 改行为命名；用例与断言零改动。）
...
import { loadProviders, processProviderRuntime } from '../../../src/ai/provider/store.js'
...
// 不变，排队路径挂 promise then 段）。经 processProviderRuntime().__seedProvidersWriteChainForTest 注入在途段
// 触达排队路径（生产零调用测试钩子，providers-save-queued-write 同款）。
test('在途写排队窗口下的迁移 → 落盘后 bak 仍被密文覆写（不残留明文 Key）', async () => {
  let release!: () => void
  const gate = new Promise<void>((r) => {
    release = r
  })
  processProviderRuntime().__seedProvidersWriteChainForTest(dir, gate) // 迁移写排队在 gate 之后
  const conf = makeConf()
  writeFileSync(FP(), JSON.stringify({ providers: [{ ...conf }], currentId: conf.id }, null, 2), 'utf8')
