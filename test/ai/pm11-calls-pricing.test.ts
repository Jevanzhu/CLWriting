 ...
 *
 * - recordUsageCombined：task + chapter 两块记账合并为单次持锁读改写（原 runner
...
import { recordUsageCombined, recordAiCall, recordTaskUsage } from '../../src/ai/calls.js'
...
describe('PM-11 recordUsageCombined 合并记账', () => {
  it('同一笔 usage：合并 = 1 次落盘；两分身串行 = 2 次', () => {
    const merged = tempBook()
    recordUsageCombined(merged, { task: 'gen', chapter: 1, usage: USAGE, costUsd: 0.5 })
  ...
  it('同章累加形态逐字段一致', () => {
    const merged = tempBook()
    recordUsageCombined(merged, { task: 'gen', chapter: 1, usage: USAGE, costUsd: 0.5 })
    recordUsageCombined(merged, { task: 'gen', chapter: 1, usage: USAGE, costUsd: 0.25 })
    const split = tempBook()
    recordTaskUsage(split, 'gen', USAGE)
    recordAiCall(split, 1, USAGE, 0.5)
    recordTaskUsage(split, 'gen', USAGE)
  ...
  it('换章重置形态逐字段一致（tasks 保留、chapter 块重建）', () => {
    const merged = tempBook()
    recordUsageCombined(merged, { task: 'gen', chapter: 1, usage: USAGE, costUsd: 0.5 })
    recordUsageCombined(merged, { task: 'gen', chapter: 2, usage: USAGE, costUsd: 0.25 })
    const split = tempBook()
    recordTaskUsage(split, 'gen', USAGE)
    recordAiCall(split, 1, USAGE, 0.5)
    recordTaskUsage(split, 'gen', USAGE)
  ...
  it('单块退化：task-only / chapter-only 与对应分身产物一致', () => {
    ...
    recordUsageCombined(mergedTask, { task: 'rag-embed', usage: USAGE })
    ...
    recordUsageCombined(mergedCh, { chapter: 3, usage: USAGE, costUsd: 0.1 })
