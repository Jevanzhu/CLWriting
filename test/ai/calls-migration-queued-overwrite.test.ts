...
describe('R48-2: 排队迁移写不覆盖先行记账写', () => {
  it('[A, M] 队列序下 task 账目保留（修复前被 T0 快照覆盖）', async () => {
    ...
    // 再让 M 出队执行完：哨兵记账写排在 M 后（writeChains 同链 FIFO，M 在途时入队
    // 必挂其后）——其落盘可轮询观测，落盘即证 M 已出队（P3-5：固定 50ms 实睡改链序
    // 确定性观测，慢腿不赌时长）
    recordTaskUsage(root, 'sentinel', { inputTokens: 1, outputTokens: 1 })
    await waitFor(() => {
      const rec = JSON.parse(readFileSync(recPath, 'utf8')) as { tasks?: Record<string, unknown> }
      return rec.tasks?.['sentinel'] !== undefined
    })
  ...
  it('迁移写执行时文件已是新格式 → 不落盘、不抛（幂等跳过路径）', async () => {
    ...
    // 等 M 出队：跳过路径不改文件（若误写会回退成 T0 迁移态 used=2）。哨兵写排 M 后
    // （同链 FIFO）——其落盘即证 M 已出队且其读改写已吃到 M 的结论；M 若回归为 T0
    // 快照覆写，哨兵读到的 chapter.used=2 会被断言抓红（P3-5：实睡改链序确定性观测）
    const recPath = join(root, '.cache', 'ai-calls.json')
    recordTaskUsage(root, 'sentinel', { inputTokens: 1, outputTokens: 1 })
    await waitFor(() => {
      const rec = JSON.parse(readFileSync(recPath, 'utf8')) as { tasks?: Record<string, unknown> }
