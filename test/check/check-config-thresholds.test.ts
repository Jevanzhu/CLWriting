/**
 * 机检阈值五键的 runner 生效链与开关语义。
 *
 * 档源（2 档并 1，按被测行为合并；断言逐条保留、去重 0 条）：
 * - r52-check-threshold-config.test.ts（R52-E-2：book.yaml checks.* 五键直达检查器）
 * - r29-check-machine-correctness.test.ts 的 B-7 组（opening_env_chars 显式 0 = 关检）
 *
 * R52-E-2（五十二轮）：checks.word_count_tolerance / repeat_threshold /
 * repeat_chars_threshold / max_sentence_len / imagery_threshold 经 config 直达
 * checkWordCount / checkRepeat / checkImagery / checkSentenceLength（undefined 直落
 * 引擎默认参数，行为不变）；附严格短篇升红三项（repeat / sentence-length /
 * imagery-overuse）的边界钉。B-7（二十九轮批 A）：开头零环境检查的配置关闭口径。
 */
...
test('B-7 读侧: opening_env_chars 显式 0 → 无 opening-env 项（检查关闭）；未设 → 默认检查在跑', () => {
  expect(runShortForOpeningEnv({ opening_env_chars: 0 })).not.toContain('opening-env')
  expect(runShortForOpeningEnv({})).toContain('opening-env') // 未设 = 默认 300，检查照跑
})
