...
test('R33-5: 读失败（EISDIR 瞬态占用模拟）→ ok:false；无文件 → ok:true 空推进', () => {
  const root = mkdtempSync(join(tmpdir(), 'r33-lead-'))
  try {
    // 无任何文件：已读、无推进（已知态，不跳过闭合）
    expect(readChapterUpdatesForChapterChecked(root, 3)).toEqual({ ok: true, updates: [] })
    // 主文件被目录占用（readFileSync 对目录抛错）→ ok:false（修复前返回 [] 被
    // leadClosureItems 判「声明了没做」假红）
