/**
 * 机检输入解析守卫：禁词行解析 / book.yaml 空值键 / 履历空证据 / 账本标题链。
 *
 * 档源：原 r76-machine-checks.test.ts（二十四轮修复批 B）——R76-3 组按被测行为
 * 并入 roster-new-names.test.ts，余下四组成档；断言逐条保留、去重 0 条。
 *
...
function makeEmptyEvidenceBook(): string {
  const root = mkdtempTracked(join(tmpdir(), 'empty-evidence-book-'))
