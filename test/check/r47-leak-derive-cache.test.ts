 ...
 * 修复后：布线目录指纹命中直接回缓存（零 readFileSync），指纹失配（文件增/改）
 * 自动重算。
 * dev←recover 合并批收档：R47-26 与 dev 在位 R46-10 指纹缓存同题，择 dev 在位版
 * （leakDeriveCache；命中返回共享数组系 dev 口径——机检消费面只读比对零拷贝，
 * R47-26 的「返回拷贝」语义不采，其用例 5 一并删除）。
