...
describe('buildDraftPrompt: 短篇', () => {
  it('基本结构 → 含短篇要求', () => {
    const p = buildPrompt(dir, 1, 'short')
...
describe('buildDraftPrompt: 章号注入', () => {
  it('长篇 → 正确章号', () => {
    const p = buildPrompt(dir, 7, 'long')
    expect(p).toContain('第 7 章')
  })

  it('短篇 → 正确章号', () => {
    const p = buildPrompt(dir, 3, 'short')
...
describe('buildDraftPrompt: 每章字数区间（P1 接线 chapter_target_words）', () => {
  it('config 带字数目标 → 任务行用目标 ±20%（取整到百）', () => {
    const p = buildPrompt(dir, 1, 'long', cfg({ chapterTarget: 3000 }))
    expect(p).toContain('2400-3600 字')
    expect(p).not.toContain('2000-4000 字')
  })

  it('短篇同链', () => {
    const p = buildPrompt(dir, 1, 'short', cfg({ chapterTarget: 10000 }))
    expect(p).toContain('8000-12000 字')
    expect(p).not.toContain('8000-20000 字')
  })

  it('未传 config → 硬编码区间不变（直调/测试路径回落）', () => {
    expect(buildPrompt(dir, 1, 'long')).toContain('2000-4000 字')
    expect(buildPrompt(dir, 1, 'short')).toContain('8000-20000 字')
  })

  it('目标 0 = 未设 → 硬编码区间', () => {
    const p = buildPrompt(dir, 1, 'long', cfg({ chapterTarget: 0 }))
    expect(p).toContain('2000-4000 字')
  })
})
...
describe('buildDraftPrompt: 文风样章（P1 接线 style.injection）', () => {
  ...
  it('默认/轻档 → 注入 1 段（注入档接线的核心行为）', () => {
    ...
    for (const c of [undefined, cfg({}), cfg({ injection: 'light' })]) {
      const p = buildPrompt(dir, 1, 'long', c)
  ...
  it('重档 → 注入 3 段', () => {
    makeSampleLibrary()
    makeChapterOutline('战斗')
    const p = buildPrompt(dir, 1, 'long', cfg({ injection: 'heavy' }))
  ...
  it('短篇分支同链注入', () => {
    makeSampleLibrary()
    makeChapterOutline('战斗')
    const p = buildPrompt(dir, 1, 'short', cfg({ injection: 'heavy' }))
...
describe('readChapterScenes: 场景水源三级回退（① 章纲 fm → ② 正文 fm → ③ 细纲场景声明段）', () => {
  ...
  it('kk-P2 枚举门：场景声明段内非枚举引号词不收（AI 解释性引号词防串样章）', () => {
    ...
    makeDetailedOutline(1, '## 场景声明\n本章「注意铺垫」与「节奏变化」。')
    const p2 = buildPrompt(dir, 1, 'long', cfg({ injection: 'heavy' }))
    expect(p2).not.toContain('## 文风样章')
  })
})

// ── Q-5（第十五轮）：注入源清单 files——「模型可见⟺已记录」文件级溯源 ──
describe('buildDraftPrompt: Q-5 注入源清单', () => {
  /** 全源书：细纲 + 章纲 + 材料 + 世界观 + 角色 + 境界 + 样章库（战斗 1 段） */
  function makeFullBook(): void {
    mkdirSync(join(dir, '工作区'), { recursive: true })
    writeFileSync(join(dir, '工作区', '细纲.md'), '全源细纲')
    writeFileSync(join(dir, '工作区', '本章写作材料.md'), '备料内容')
    mkdirSync(join(dir, '大纲', '章纲'), { recursive: true })
    writeFileSync(join(dir, '大纲', '章纲', '0001-开篇.md'), '---\n章号: 1\n标题: 开篇\n场景: 战斗\n---\n章纲详情')
    mkdirSync(join(dir, '设定', '角色'), { recursive: true })
    writeFileSync(join(dir, '设定', '世界观.md'), '修仙世界')
    writeFileSync(join(dir, '设定', '角色', '林远.md'), '---\n姓名: 林远\n身份: 弟子\n境界: 练气\n---\n正文')
  ...
  it('全源书 → files 按注入序列出全部实际注入源（细纲→章纲→材料→设定层→样章）', () => {
    ...
    expect(d.files.slice(0, 6)).toEqual([
      '工作区/细纲.md',
      '大纲/章纲/0001-开篇.md',
      '工作区/本章写作材料.md',
      '设定/世界观.md',
  ...
  it('设定层被预算整层丢弃 → 该层源文件不计（可见⟺已记录双向闭合）', () => {
    makeFullBook()
    // 世界观撑爆预算（project 档最宽泛、超限先丢）→ 世界观层 omitted，其源不列；
    // 角色/境界（volume 档）仍注入，源保留
    writeFileSync(join(dir, '设定', '世界观.md'), '界'.repeat(12_000))
    const d = buildDraftPrompt(dir, 1, 'long')
    expect(d.prompt).toContain('（设定超预算，已省略：世界观）') // in-band 声明确认该层确实被整层丢弃
