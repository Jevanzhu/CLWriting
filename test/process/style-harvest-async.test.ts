/**
 * 文风收割异步链（style-harvest / ai-track）——异步孪生与同步版等价、链上零同步阻塞。
 *
 * 覆盖三条口径：
 * 1. 同步-异步等价（R37-5 延伸）：collectDocSignalsAsync / harvestStyleCandidatesAsync
 *    与同步孪生同输入同结果（含空书安全臂、无轨迹臂）。
 * 2. 源1 读取改道（重评-0912-2 P2-4）：裸 readFileSync 整读章正文改走 md-text-cache
 *    指纹缓存异步读 —— 判定以「缓存驻留」隔离源1（源2 零章不读盘 → 缓存有且只能有
 *    源1 的写入面）；未命中/命中两轮产物逐位一致证明改道不换输出口径。
 * 3. 轨迹枚举异步化（R44-13）：git 后端 for-each-ref 走异步 spawn、全链 0 次 spawnSync
 *    （网盘挂载 .git / 杀软锁下同步 spawn 会冻结事件循环最长 15s）。
 *
 * 判定手法：包装 spawn/spawnSync 记录调用（真实实现保留），断言子命令与实参面；
 * 前置硬化旗（`-c core.fsmonitor=false` 等）由 gitArgsOf 剥掉，形态断言不随其演进。
 */
...
/** 剥掉 git() 统一前置的 `-c key=value` 对，只留子命令与实参面。 */
...
describe('collectDocSignals / harvest 同步-异步等价', () => {
  let root = ''
  beforeEach(() => {
...
describe('收割源1 读取改道 md-text-cache（口径不变）', () => {
  let root = ''
  beforeEach(() => {
...
describe('收割链轨迹枚举异步化（全程零 spawnSync）', () => {
  let root = ''
