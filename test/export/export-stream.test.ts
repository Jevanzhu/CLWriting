/**
 * exportBook 分章读-写流水化（流式导出）的行为等价性与收口信封。
 *
 * 重构把「purified 全书数组 + join 整书大串」（峰值 ≈4-6× 全书）改为逐章净化即写即弃
 * （merged 走 atomicWriteStream 追加写），readChapterDir 不再 includeBody 全量驻留。
 * 本文件验证：
 * 1. 产物字节与原构造式 `units.map(u => `# ${t}\n\n${b}`).join('\n\n---\n\n')` 逐一恒等
 *    （同段同序同分隔符，含 %# 批注剥除路径）；
 * 2. 多章 both 导出章数/产物/分隔正确（流式单遍不丢章、不串分隔符）；
 * 3. 零可写章时按失败收口 + 逐章警告，报数口径如实（定稿过滤/无清单兜底/跳过草稿）。
 *
 * 源码锚：R73-37（二十一轮）、内存审计修复批 A1（2026-08-24）、R28-16（二十八轮）、
 * R26-53（二十六轮）。
 */
...
/** 长书夹具：book.yaml + 正文目录（定稿清单由 finalizeAll 事后登记） */
...
/** 把当前盘上全部章登记为已定稿（判据 = finalizedRevision，内容不参与） */
