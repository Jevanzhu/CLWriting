...
test('gitHealthCheck: 半提交（staged 残留）→ halfCommit 命中出人话', () => {
  ...
  expect(half).toEqual(expect.objectContaining({ kind: 'halfCommit' }))
...
test('gitHealthCheck: 合并冲突（MERGE_HEAD + 冲突标记）→ mergeConflict 命中', () => {
  ...
  expect(conflict).toEqual(expect.objectContaining({ kind: 'mergeConflict' }))
...
test('gitHealthCheck: 僵死锁（index.lock 存在无活跃进程）→ staleLock 命中', () => {
  ...
  expect(lock).toEqual(expect.objectContaining({ kind: 'staleLock' }))
...
test('gitHealthCheck: 网盘副本残留（文件 2.md）→ cloudCopy 命中', () => {
  ...
  expect(copy).toEqual(expect.objectContaining({ kind: 'cloudCopy' }))
