/**
 * 回滚「回到第 N 章」+ 兜底闭环测试 —— ⑯ 第 5 节 + ⑮ 第 4 节 + 第 4.3 节。
 *
 * 工单施工序 7-8 验证点：
...
test('兜底闭环: 正常确认（哈希一致）→ enter 复述不报警', () => {
  const root = makeGitBookWithChapters(1)
  // 第 1 章正常定稿，确认哈希是真实的（这里简化：不设细纲则不验证，verified 保持 true）
  sh('git commit --amend -m "ch:0001 第1章\n\nConfirmed: 2026-06-17T10:00 mode=manual hash=sha256:real" --no-edit', root)
  // 不放工作区细纲 → parseLastConfirm 不验证（verified=true，不误报）

  const { recap } = enter(root)
  if (recap.lastConfirm) {
    expect(recap.lastConfirm.verified).toBe(true)
  }
  expect(formatRecap(recap)).not.toContain('不一致')
  rmSync(root, { recursive: true, force: true })
})
