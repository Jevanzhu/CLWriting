...
test('停止① 预算: 宿主返回 budget trigger → paused.reason=budget', () => {
  const root = makeBookWithVolumeOutline()
  const stop = ({ chapter }: { chapter: number }): ChapterProduction | StopTrigger | null => {
    if (chapter >= 2) return { reason: 'budget', detail: '第2章调用触顶（8/8）' }
    return goodProduction(chapter)
  }
  const r = doAutoBatch({ bookRoot: root, targetCount: 3, produce: stop })
...
test('停止② 质量: isolate=true → 坏章移到 .isolated/ 不出批次', () => {
  const root = makeBookWithVolumeOutline()
  const stop = ({ chapter }: { chapter: number }): ChapterProduction | StopTrigger | null => {
...
test('停止③ 需人: 高风险章禁降级 → paused.reason=human', () => {
  const root = makeBookWithVolumeOutline()
  const stop = ({ chapter }: { chapter: number }): ChapterProduction | StopTrigger | null => {
...
test('停止④ 系统: git MERGE_HEAD → paused.reason=system + 不产章', () => {
  const root = makeBookWithVolumeOutline()
  // 造一个 MERGE_HEAD（模拟合并冲突状态）
  writeFileSync(join(root, '.git', 'MERGE_HEAD'), 'deadbeef', 'utf-8')

  const r = doAutoBatch({ bookRoot: root, targetCount: 2, produce: () => goodProduction(1) })
  expect(r.ok).toBe(true)
  if (!r.ok) return
...
test('干净章不受连累: ②隔离后同批已产章保留可审', () => {
  ...
  const r = doAutoBatch({ bookRoot: root, targetCount: 5, produce: stop })
  expect(r.ok).toBe(true)
  if (!r.ok) return
  // 第1、2章干净保留在待审清单
...
test('隔离章游标照推: 隔离后 next_chapter 不回填', () => {
  const root = makeBookWithVolumeOutline()
  const stop = ({ chapter }: { chapter: number }): ChapterProduction | StopTrigger | null => {
...
test('--resume: ②隔离章不重跑（跳过隔离章续写）', () => {
  ...
  const r1 = doAutoBatch({ bookRoot: root, targetCount: 3, produce: stop })
  expect(r1.ok).toBe(true)

  // resume：第2章已隔离（不重跑），从 next_chapter=3 续
  phase = 2
  const r2 = doAutoBatch({ bookRoot: root, targetCount: 3, produce: stop, resume: true })
