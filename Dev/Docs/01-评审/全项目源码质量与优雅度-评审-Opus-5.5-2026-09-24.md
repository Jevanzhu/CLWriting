...
- 约 35 个测试文件读生产源码文本做正则 / 包含断言（另有 6 个治理测试按源码检查依赖方向，属合理的架构门，不计入）。例如 `test/events/r0911-g-p3-4-close-cache.test.ts`；`test/desktop/r38-exit-guards.test.ts:31-49` 甚至从 ipc.ts 源码里抠出正则再测；`test/studio/webnext/sse-selfheal.test.ts:140-149` 断言 Book.vue 源码含 `useSse(() => bookName.value)` 等字符串，`web-next/src/pages/Book.vue:23-26` 注释自述 SSE 看护逻辑因此只能留在页面层——测试反过来决定了代码放在哪。这类断言对无害重构敏感、对语义破坏不敏感，是 P3-1 / P3-2 整理的直接阻力。
