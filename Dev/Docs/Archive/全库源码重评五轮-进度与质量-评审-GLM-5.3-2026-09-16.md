...
- **批内记正/加固两处**：re2-manifest-lock-reentry-async 探测点 sleep 定点 → waitFor 事件协调（本文件系注册在案负载敏感族——修复批全量首跑与 2 文件组合各 1 败、孤立复跑绿，加固后 5/5 绿入全量）；r64-switch-guards（现 finalize-switch-guard.test.ts）迁移件 spy 显式 `MockInstance` 类型（vue-tsc 视野含 webnext 测试目录，定向复验 4 过）。
