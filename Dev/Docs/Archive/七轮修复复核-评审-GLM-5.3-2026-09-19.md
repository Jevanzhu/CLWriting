...
- **红绿有效性逐一推演 ✅**（对守卫存在性与截断行为）：
  - trash 新件 4 例：撤销七轮-1（去守卫）→ 窗口用例走回收站清单查询 → 404 NOT_FOUND ≠ 409 复红；仅撤销七轮-2（留守卫去盘面校验）→ disk-deleted 用例复红——**两项修复隔离可测**；正控真跑通回收站查询路径（trash.ts:127-133 → :282 404）并证盘面校验对完好书无数伪。装置与 documents-write-bookmoved-recheck 先例逐段同构。
