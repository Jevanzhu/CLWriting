...
> **状态**:v0.3 方案已进入实现收口(2026-06-24 同步)——功能蓝图 + 编排模型(B)+ 工程方案 + prompt 工程 + UI 状态机 + driver 细节 + 横切 全部定稿;Step1 全 + Step2.1-2.5 已落地,driver resume 假会话已修,真宿主 smoke 独立入口已补,1.5/7.4/7.5 可选项已收口(.gui-active 单写者协作/场景分布/角色关系图)。**首版只支持 CC**(Codex 暂时放弃,决策 22)。本文作为方案母本/决策记录保留,当前进度以 `Dev/Plans/README.md` 为索引。
>
...
- GUI 活跃时写 `工作区/.gui-active`(PID + 时间戳);CLI 向后兼容(不强制检测,老脚本照跑,可选加轻提示) **✅ 已落地(2026-06-24,GUI 心跳 20s 续期 + finalize/check/confirm 轻提示)**
...
| 场景分布 | 5 场景占比:战斗/对话/抒情/叙事铺陈/爽点高潮 | `ChapterMeta.场景`(**路径C 已落地**:outline 声明→writer 落正文 front matter→rhythm 读) |
...
**关系图探查结论(不做)**:`RealmDoc`(定稿/设定/境界体系.md)是**境界体系**非角色文档;角色设定(`定稿/设定/角色/*.md`)无强类型(自由 MD);角色间无关系字段(关系只散在账本「关系债」类的 `欠方`/`债主`)。→ **角色关系图数据基础不足,剔除**;最多做「关系债子图」。
  > **更新(2026-06-24,7.5-B)**:角色关系图已做——角色卡 front matter 加 `关系` 字段(如 `关系: 林远(师徒);赵衡(仇敌)`),settings 返回 characterRelations,关系图加入角色关系边(绿)与关系债边(红)。通过给角色卡加关系字段解决原「数据基础不足」结论。
...
25. **可选项收口(2026-06-24)**——三项落地:① 1.5 `.gui-active` 单写者协作(GUI 心跳 20s 续期写 `工作区/.gui-active`,CLI finalize/check/confirm 检测活跃轻提示,不阻塞);② 7.4 场景分布选**路径 C**(`ChapterMeta` 加 `场景` 字段,outline 声明→writer 落正文 front matter→rhythm 读分布+场景×情绪热力矩阵;不选细纲归档——场景落正文永久保留且不动 finalize/cache schema);③ 7.5 角色关系图(角色卡 front matter 加 `关系` 字段 `林远(师徒);赵衡(仇敌)`,通过加字段解决原 7.5「数据基础不足」剔除结论)。测试 727。
