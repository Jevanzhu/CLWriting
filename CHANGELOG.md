...
> **第二批边界（留接口/归 beta）**：短篇 P1–P4 编排内核（src/process/）——靠 CLI 门面 + 确认闸兜底，不写编排代码（短篇批量自动归 M6 延伸）；短篇机检阈值 beta 校准（字数下限按题材等，列入 open-questions）；知识层知乎短篇素材平移（归 M4 既有流程，本批仅引用）；真模型 smoke（CC/Codex 跑短篇全篇）留作出口验收。
