...
function renderWbMid(){
  ...
  const SM={enter:['就绪',`第 ${state.wbChapter} ${unit} · 待生成细纲`],outline:['细纲中',`spawnRole(outline) · 生成第 ${state.wbChapter} ${unit} 细纲`],confirm:['待确认','细纲已生成 · 确认后进入备料'],prepare:['备料中','确定性步 · clwriting CLI · 备料'],draft:['写稿中',`第 ${state.wbChapter} ${unit} · 细纲已确认、备料就绪 · 流式输出中`],check:['机检中','确定性步 · clwriting CLI · 机检'],review:['三审中','镜头 / 文字 / 连续性 三视角'],finalize:['待定稿','裁决已通过 · 可定稿落盘']};
  const sm=SM[state.wbStage]||SM.draft;
  ...
  c.innerHTML=`<div class="content-scroll"><div class="bento-wrap"><div class="bento-head"><h1 class="bento-title">工作台 · 第 ${state.wbChapter} ${unit}</h1><div class="bento-sub">${sub}</div></div><div class="state-card"><span class="state-tag">【${sm[0]}】</span><span class="state-msg">${sm[1]}</span></div><nav class="stages">${stageBar}</nav><div class="card ctrl"><div class="ctrl-row"><label>${unit}号 <input type="number" id="wbCh" value="${state.wbChapter}" min="1"></label><span class="btn-cli" data-cli="outline">${state.book==='short'?'📋 篇纲':'📋 细纲'}</span><span class="btn-cli" data-cli="confirm">✓ 确认</span><span class="btn-cli" data-cli="prepare">📦 备料</span><span class="btn-fire" id="wbDraft">${state.wbRunning?'写稿中…':'✍ 写第 '+state.wbChapter+' '+unit}</span>${state.wbRunning?'<span class="btn-stop" id="wbStop">⏹ 中断</span>':''}<span class="btn-cli" data-cli="check">🔍 机检</span><span class="btn-review" data-cli="review">📝 三审</span><span class="btn-cli ${state.wbVerdict?'':'disabled'}" data-cli="finalize">✅ 定稿</span><label class="auto-toggle"><input type="checkbox" id="wbAuto" ${state.wbAuto?'checked':''}> 自动推进</label></div>${state.wbInterrupted?'<div class="interrupt-bar"><span class="interrupt-tip">写稿已中断，正文已保留（'+state.wbTextOut.length+' 字）</span><span class="btn" id="wbDiscard">🗑 弃稿</span><span class="btn primary" id="wbRewrite">🔄 重写</span></div>':''}</div>${mainBody}</div></div>`;
}
...
function wbCli(step){
  ...
  if(state.wbAuto&&step==='confirm'){wbPush('spawn','→ 自动备料');setTimeout(()=>wbCli('prepare'),700);}
...
function bindWb(){
  ...
  const st=el('wbStop');if(st)st.onclick=()=>{state.wbRunning=false;state.wbInterrupted=true;wbPush('error','⏹ 已中断——正文已保留，可弃稿或重写');renderWbMid();bindWb();renderWbCtx();};
  const v=el('wbVerdict');if(v)v.onclick=()=>{state.wbVerdict=true;wbPush('saved','裁决：通过（可定稿）');renderWbMid();renderWbCtx();showHint('裁决通过 · 可定稿');if(state.wbAuto){wbPush('spawn','→ 自动定稿');setTimeout(()=>wbCli('finalize'),700);}};
  ...
  const dc=el('wbDiscard');if(dc)dc.onclick=()=>{state.wbTextOut='';state.wbInterrupted=false;wbPush('error','已弃稿 · 清空正文');renderWbMid();bindWb();renderWbCtx();};
  const rw=el('wbRewrite');if(rw)rw.onclick=()=>{state.wbInterrupted=false;wbDraft();};
