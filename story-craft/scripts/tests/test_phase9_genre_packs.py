...
class Phase9GenrePackTests(unittest.TestCase):
    def test_all_phase9_genre_packs_exist(self) -> None:
        self.assertTrue(GENRE_PACKS_DIR.is_dir(), "missing genres dir")
        actual = {path.name for path in GENRE_PACKS_DIR.iterdir() if path.is_dir()}
        self.assertEqual(set(EXPECTED_PACKS), actual)

    def test_each_genre_pack_has_required_files_and_content(self) -> None:
        for pack, phrases in EXPECTED_PACKS.items():
            with self.subTest(pack=pack):
                ...
                self.assertEqual(f"# {pack}", readme.splitlines()[0])
                ...
                self.assertGreater(len(readme.strip()), 200)
                self.assertGreater(len(patterns.strip()), 300)
                self.assertGreater(len(checklist.strip()), 200)

    def test_genre_packs_link_to_existing_templates(self) -> None:
        ...
        for pack, template_names in EXPECTED_PACKS.items():
            for template_name in template_names:
                ...
                with self.subTest(pack=pack, template=template_name):
                    self.assertTrue((template_dir / f"{template_name}.md").is_file())
