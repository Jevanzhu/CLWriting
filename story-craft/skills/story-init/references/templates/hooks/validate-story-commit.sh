#!/bin/bash
# validate-story-commit.sh - advisory checks for git commit commands.
set -euo pipefail
...
while IFS= read -r -d '' file; do
  ...
  case "$file" in
    正文.md|*/正文.md|正文/*|*/正文/*)
      ...
      if [ -n "$HARDCODED" ]; then
        WARNINGS="$WARNINGS\n[WARN] $file: hardcoded character attributes found; prefer 设定/ files.\n$HARDCODED"
      fi
      ;;
  esac
  ...
  case "$file" in
    设定/*|*/设定/*)
      if ! grep -qE "^[[:space:]]*(名字|姓名|名称|name|Name)[[:space:]]*[：:]" "$FULL_PATH" 2>/dev/null; then
        WARNINGS="$WARNINGS\n[WARN] $file: setting file missing required fields (name/名字: ...)"
      fi
      ;;
  esac
