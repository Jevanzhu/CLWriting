...
python -X utf8 "${SCRIPTS_DIR}/story_craft.py" --project-root "${PROJECT_ROOT}" query impact --chapter "${CHAPTER}"
python -X utf8 "${SCRIPTS_DIR}/story_craft.py" --project-root "${PROJECT_ROOT}" query impact --chapter "${CHAPTER}" --format markdown
...
- 章节影响分析：调用 `query impact --chapter N`，只读汇总目标章 commit、角色、伏笔、时间线和后续章节引用；需要改稿复查清单时追加 `--format markdown`。
